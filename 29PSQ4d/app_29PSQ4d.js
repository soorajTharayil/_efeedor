...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculateNearMissesRate = function () {
      // Get the number of near misses reported
      var nearMisses = parseInt(document.getElementById("formula_para1").value);

      // Get the number of incidents reported
      var incidentsReported = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for nearMisses and incidentsReported
      if (isNaN(nearMisses) || nearMisses < 0) {
        alert("Enter the number of near misses reported.");
        return;
      }

      if (isNaN(incidentsReported) || incidentsReported < 0) {
        alert("Enter the number of incidents reported.");
        return;
      }

      if (nearMisses > incidentsReported) {
        alert(
          "The no. of near misses reported must be less than the no. of incidents reported."
        );
        return;
      }

      if (nearMisses === 0 && incidentsReported === 0) {
        $scope.calculatedResult = 0;
      } else {
        // Calculate the percentage of near misses
        $scope.calculatedResult = (
          (nearMisses / incidentsReported) *
          100
        ).toFixed(2);
      }
      ...
      console.log("Calculated Near Misses Rate", $scope.calculatedResult);
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi29.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
