...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.calculateTimeFormat = function () {
		...
		var admissionDate = new Date($scope.feedback.initial_assessment_hr2);
		var assessmentDate = new Date($scope.feedback.initial_assessment_hr3);
		...
		$scope.feedback.initial_assessment_hr2 = $scope.formattedAdmissionDateTime;
		$scope.feedback.initial_assessment_hr3 = $scope.formattedAssessmentDateTime;
	...
	$scope.savefeedback = function () {
		...
		if (!$scope.feedback.initial_assessment_hr2 || isNaN(admissionDate.getTime())) {
			alert("Please enter bill prepared time.");
			return;
		}

		if (!$scope.feedback.initial_assessment_hr3 || isNaN(assessmentDate.getTime())) {
			alert("Please enter sample received time.");
			return;
		}
		...
		$http.post($rootScope.baseurl_main + '/savepatientfeedback_return_to_i.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
