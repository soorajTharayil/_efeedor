<?php
$Subject = 'Ticket Ageing Alert ' . date('d M, Y');
...
$query = 'SELECT * FROM  ticketsop  WHERE status = "Open" AND created_on <= "' . $tdate . '" AND created_on  > "' . $tdate7day . '"';
...
while ($t = mysqli_fetch_object($tickets)) {
	...
	$query = 'SELECT * FROM  bf_opatients  WHERE patient_id = "' . $t->created_by . '"';
	...
	$query = 'SELECT * FROM  ticketsop  inner JOIN department ON department.dprt_id = ticketsop.departmentid   WHERE  tickets.id = ' . $t->id;
...
$query = 'SELECT * FROM  ticketsop  WHERE status = "Open" AND created_on <= "' . $tdate7day . '" AND created_on  > "' . $tdate15day . '"';
...
$query = 'SELECT * FROM  ticketsop  WHERE status = "Open" AND  created_on  < "' . $tdate15day . '"';
