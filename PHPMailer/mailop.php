...
$Subject = 'HID: ' . $HID . ' NEW OUTPATIENT FEEDBACK TICKET- EFEEDOR';

$query = 'SELECT * FROM  bf_outfeedback  WHERE emailstatus = 0';
...
while ($r = mysqli_fetch_object($flore)) {
	...
	while ($t = mysqli_fetch_object($ticket)) {
		...
		$query = 'SELECT * FROM  bf_departmentop  WHERE title = "' . $p->ward . '"';

		$patient = mysqli_query($con, $query);

		$f = mysqli_fetch_object($patient);

		$tlink = $config_set['BASE_URL'] . 'opf/track/' . $t->id;

		$message .= 'Feedback ticket from <br />Patient: ' . $p->name . ', <br />PID: ' . $p->patient_id . ', <br />Speciality: ' . $f->title . ' <br />Department: ' . $t->description;
	...
	if ($rowcount > 1) {
		...
		$messages .= 'Feedback ticket from <br />Patient:  ' . $p->name . ',<br /> PID: ' . $p->patient_id . ',<br /> Speciality: ' . $f->title;
	...
	$quiery = 'Update bf_outfeedback set emailstatus = 1 WHERE id=' . $r->id;
