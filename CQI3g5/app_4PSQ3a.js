  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (opportunitiesForErrors < 0) {
        alert(
          "Negative values are not allowed for 'Total number of babies admitted in NICU'."
        );
        return;
      }
      ...
      if (opportunitiesForErrors === 0) {
        if (medicationErrors === 0) {
          ...
          alert(
            "Total number of babies admitted in NICU cannot be zero unless both values are zero."
          );
          return;
        }
      }
      ...
      // Auto-format result (whole → as-is, decimal → 2 places)
