  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter total number of cases where in Initial assessment done in physiotherapy with in 24 hours "
        );
        return;
      }
      ...
      if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
        alert(
          "Please enter total number of out-patients requiring physiotherapy"
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter total number of cases where in Initial assessment done in physiotherapy with in 24 hours  be less than total number of out-patients requiring physiotherapy"
        );
        return;
      }
