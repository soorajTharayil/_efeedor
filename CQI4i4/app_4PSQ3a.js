...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter number of executed food waste collection");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total No.of Scheduled food waste collection ");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter  number of executed food waste collection  be less than total No.of Scheduled food waste collection");
			return;
		}
