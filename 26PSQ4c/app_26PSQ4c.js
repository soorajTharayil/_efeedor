...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculateStockOutRate = function () {
      // Get the number of stock outs of emergency drugs
      var stockOuts = parseInt(document.getElementById("formula_para1").value);

      // Get the number of drugs listed as emergency drugs in hospital formulary
      var emergencyDrugsListed = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for stockOuts and emergencyDrugsListed
      if (isNaN(stockOuts) || stockOuts < 0) {
        alert("Enter the number of stock outs of emergency drugs.");
        return;
      }

      if (isNaN(emergencyDrugsListed) || emergencyDrugsListed < 0) {
        alert(
          "Enter the number of drugs listed as emergency drugs in hospital formulary."
        );
        return;
      }

      if (stockOuts > emergencyDrugsListed) {
        alert(
          "The no. of stock outs of emergency drugs must be less than the no. of drugs listed as emergency drugs."
        );
        return;
      }

      if (stockOuts === 0 && emergencyDrugsListed === 0) {
        $scope.calculatedResult = 0;
      } else {
        // Calculate the percentage of stock out rate for emergency medications
        $scope.calculatedResult = (
          (stockOuts / emergencyDrugsListed) *
          100
        ).toFixed(2);
      }
      ...
      console.log("Calculated Stock Out Rate", $scope.calculatedResult);
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi26.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
