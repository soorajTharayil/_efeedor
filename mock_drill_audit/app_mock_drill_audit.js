...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
        ...
        //alert(id);
        $http.get($rootScope.baseurl_main + '/audit_load_emergency_code.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
            $scope.emergency_code = responsedata.data;
	...
	$scope.feedback = {
		checklist: 'Select Emergency Code'
	};
	...
	$scope.savefeedback = function () {
		...
		function isFeedbackValid() {
			if ($scope.feedback.location == '' || $scope.feedback.location == undefined) {
				alert("Please enter location");
				return false;
			}
			if ($scope.feedback.checklist == '' || $scope.feedback.checklist == undefined || $scope.feedback.checklist == 'Select Emergency Code') {
				alert("Please select emergency code");
				return false;
			}
		...
		var activationDate = new Date($scope.feedback.initial_assessment_hr1);
		var announcementDate = new Date($scope.feedback.initial_assessment_hr2);
		var arrivalDate = new Date($scope.feedback.initial_assessment_hr3);
		var clearanceDate = new Date($scope.feedback.initial_assessment_hr4);
		var closeDate = new Date($scope.feedback.initial_assessment_hr4);


		if (!$scope.feedback.initial_assessment_hr1 || isNaN(activationDate.getTime())) {
			alert("Please enter valid date & time.");
			return;
		}

		if (!$scope.feedback.initial_assessment_hr2 || isNaN(announcementDate.getTime())) {
			alert("Please enter valid date & time.");
			return;
		}

		if (!$scope.feedback.initial_assessment_hr3 || isNaN(arrivalDate.getTime())) {
			alert("Please enter valid date & time.");
			return;
		}

		if (!$scope.feedback.initial_assessment_hr4 || isNaN(clearanceDate.getTime())) {
			alert("Please enter valid date & time.");
			return;
		}

		if (!$scope.feedback.initial_assessment_hr5 || isNaN(closeDate.getTime())) {
			alert("Please enter valid date & time.");
			return;
		}
		...
		$scope.feedback.initial_assessment_hr4 = formatDateToLocalString($scope.feedback.initial_assessment_hr4);
		$scope.feedback.initial_assessment_hr5 = formatDateToLocalString($scope.feedback.initial_assessment_hr5);
		...
		$http.post($rootScope.baseurl_main + '/savepatientfeedback_mock_drill_audit.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
