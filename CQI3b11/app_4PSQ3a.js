...
$scope.user_id = ehandor.userid;
    ...
    if (medicationErrors < 0 || isNaN(medicationErrors)) {
        alert("Please enter the reporting errors identified before dispatch.");
        return;
    }

    if (opportunitiesForErrors < 0 || isNaN(opportunitiesForErrors)) {
        alert("Please enter Number of tests performed.");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3b11')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3b11.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
