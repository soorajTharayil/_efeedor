...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      // ⚠️ Denominator zero but numerator not zero → block
      if (opportunitiesForErrors === 0) {
        alert(
          "Number of discharges and deaths in that month cannot be zero unless both values are zero"
        );
        return;
      }
