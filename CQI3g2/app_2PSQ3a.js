  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateErrorRate = function () {
      ...
      if (isNaN(reportingErrors) || reportingErrors < 0) {
        alert(
          "Please enter number of ventilator associated pneumonias in a month."
        );
        return;
      }
      ...
      if (reportingErrors > testsPerformed) {
        alert(
          "number of ventilator associated pneumonias in a month less than number of ventilator days in that month."
        );
        return;
      }
