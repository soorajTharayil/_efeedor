...
while ($welcome_message_object = mysqli_fetch_object($welcome_message_result)) {
    ...
    $interim_link = 's.efeedor.com/patpcrf/?p=' . $uuid;   //pointing to public_html/pat 
...
while ($welcome_message_object = mysqli_fetch_object($welcome_message_result)) {
    ...
    $op_link = 's.efeedor.com/opopfb/?p=' . $uuid;   //pointing to public_html/pat 
...
while ($discharge_message_object = mysqli_fetch_object($discharge_message_result)) {
    ...
    $ip_link = 's.efeedor.com/dipipfb/?p=' . $uuid;    //pointing to public_html/dip
...
while ($feedback_int_object = mysqli_fetch_object($feedback_int_result)) {
    ...
    while ($tickets_int_object = mysqli_fetch_object($tickets_int_result)) {
        ...
        $patient_tracking_link = 's.efeedor.com/tkt/?p=' . $uuid;     //pointing to public_html/ticket
        $patient_tracking_link_whatsapp = 's.efeedor.com/tkts/?p=' . $uuid;     //pointing to public_html/ticket
        ...
        VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'inpatientacknowledgmentsms_on_compliantsubmission', '" . json_encode([$patient_name, $hospitalname, $patient_tracking_link_whatsapp, $hospitalname]) . "', 
...
while ($feedback_object = mysqli_fetch_object($feedback_result)) {
    ...
    VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'inpatient_sms_to_satisfiedpatient', '" . json_encode([$name, $hospitalname, $slink, $hospitalname]) . "', 
    'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending')";
        ...
        // Second query: Insert data into notifications table
        $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status) 
    VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'inpatientsms_to_unsatisfiedpatient', '" . json_encode([$name, $hospitalname, $hospitalname]) . "', 
    'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending')";
...
while ($outfeedback_object = mysqli_fetch_object($outfeedback_result)) {
    ...
    if ($tickets_generate == false) {
        ...
        // Second query: Insert data into notifications table
        $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status) 
    VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'outpatientsms_to_satisfiedpatient', '" . json_encode([$name, $hospitalname, $slink, $hospitalname]) . "', 
    'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending')";
        ...
        // Second query: Insert data into notifications table
        $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status) 
    VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'outpatientsms_to_unsatisfiedpatient', '" . json_encode([$name, $hospitalname, $hospitalname]) . "', 
    'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending')";
...
while ($feedback_int_object = mysqli_fetch_object($feedback_int_result)) {
    ...
    while ($ticket_close_object = mysqli_fetch_object($ticket_close_result)) {
        ...
        // Second query: Insert data into notifications table
        $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status) 
    VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'inpatientacknowledgmentsms_on_compliant_closedd', '" . json_encode([$name, $hospitalname, $department_head_link, $hospitalname]) . "', 
    'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending')";
