...
while ($feedback_isr_object = mysqli_fetch_object($feedback_isr_result)) {
    ...
    while ($tickets_isr_object = mysqli_fetch_object($tickets_isr_result)) {
        ...
        $employee_tracking_link = 'h.efeedor.com/tkt/?p=' . $uuid;     //pointing to public_html/ticket
        $employee_tracking_link_whatsapp = 'h.efeedor.com/tkts/?p=' . $uuid;     //pointing to public_html/ticket
        ...
        VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'staffacknowledgementsms_on_requestsubmission', '" . json_encode([$emp_name, $hospitalname, $employee_tracking_link_whatsapp, $hospitalname]) . "', 
...
while ($feedback_isr_object = mysqli_fetch_object($feedback_isr_result)) {
    ...
    while ($tickets_isr_object = mysqli_fetch_object($tickets_isr_result)) {
        ...
        foreach ($assign_to_users as $user_id) {
            ...
            $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status,meta,uuid) 
        VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'assigning_servicerequest_to_staffsms', '" . json_encode([$hospitalname, $Concern_Category, $Concern_Area, $emp_ward, $emp_bed_no, $emp_name, $emp_contactnumber, $employee_tracking_link_whatsapp, $hospitalname]) . "', 
        'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending','" . mysqli_real_escape_string($con, json_encode($meta_data)) . "','" . $uuid . "')";
            // Execute the second query
            if ($conn_g->query($insert_notification_query) === TRUE) {
                echo "Data inserted into notifications table successfully.<br>";
...
while ($feedback_incident_object = mysqli_fetch_object($feedback_incident_result)) {
    ...
    while ($tickets_incident_object = mysqli_fetch_object($tickets_incident_result)) {
        ...
        $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status,meta,uuid) 
        VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'staffacknowledgementsms_on_incidentreporting', '" . json_encode([$emp_name, $hospitalname, $employee_tracking_link_whatsapp, $hospitalname]) . "', 
        'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending','" . mysqli_real_escape_string($con, json_encode($meta_data)) . "','" . $uuid . "')";
...
while ($feedback_incident_object = mysqli_fetch_object($feedback_incident_result)) {
    ...
    while ($tickets_incident_object = mysqli_fetch_object($tickets_incident_result)) {
        ...
        foreach ($assign_to_users as $user_id) {
            ...
            $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status,meta,uuid) 
        VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'staffalert_for_incidentassigning', '" . json_encode([$hospitalname, $Concern_Category, $Concern_Area, $incident_type, $priority, $risk_matrix, $emp_ward, $emp_bed_no, $employee_tracking_link_whatsapp]) . "', 
        'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending','" . mysqli_real_escape_string($con, json_encode($meta_data)) . "','" . $uuid . "')";
            // Execute the second query
            if ($conn_g->query($insert_notification_query) === TRUE) {
                echo "Data inserted into notifications table successfully.<br>";
...
while ($feedback_incident_object = mysqli_fetch_object($feedback_incident_result)) {
    ...
    while ($tickets_incident_object = mysqli_fetch_object($tickets_incident_result)) {
        ...
        foreach ($assign_to_users as $user_id) {
            ...
            $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status,meta,uuid) 
        VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'staffalert_for_incident_reassigning', '" . json_encode([$hospitalname, $Concern_Category, $Concern_Area, $priority, $risk_matrix, $incident_type, $emp_ward, $emp_contactnumber, $employee_tracking_link_whatsapp]) . "', 
        'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending','" . mysqli_real_escape_string($con, json_encode($meta_data)) . "','" . $uuid . "')";
            // Execute the second query
            if ($conn_g->query($insert_notification_query) === TRUE) {
                echo "Data inserted into notifications table successfully.<br>";
        ...
        //     }
        //     $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status,meta,uuid) 
        // VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'staffalert_for_incident_reassigning', '" . json_encode([$hospitalname, $Concern_Category, $Concern_Area, $priority, $risk_matrix, $incident_type, $emp_ward, $emp_bed_no, $emp_name, $emp_contactnumber, $employee_tracking_link_whatsapp]) . "', 
        // 'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending','" . mysqli_real_escape_string($con, json_encode($meta_data)) . "','" . $uuid . "')";
        //     // Execute the second query
        //     if ($conn_g->query($insert_notification_query) === TRUE) {
        //         echo "Data inserted into notifications table successfully.<br>";
    ...
    while ($tickets_incident_object = mysqli_fetch_object($tickets_incident_result)) {
        ...
        foreach ($assign_to_users as $user_id) {
            ...
            if ($user_row = mysqli_fetch_object($user_result)) {
              ...
              $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status,meta,uuid) 
        VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'staffalert_for_incidentdescription', '" . json_encode([$hospitalname, $Concern_Category, $Concern_Area, $priority, $risk_matrix, $incident_type, $emp_ward, $emp_bed_no, $employee_tracking_link_whatsapp]) . "', 
        'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending','" . mysqli_real_escape_string($con, json_encode($meta_data)) . "','" . $uuid . "')";
              // Execute the second query
            if ($conn_g->query($insert_notification_query) === TRUE) {
                echo "Data inserted into notifications table successfully.<br>";
