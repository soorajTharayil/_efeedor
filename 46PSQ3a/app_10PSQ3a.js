  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert(
          "Enter no. of new oncology patients who had treatment initiated following tumour board."
        );
        return;
      }
      ...
      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert(
          "Enter no. of new oncology cases (all disciplines) treated in the month."
        );
        return;
      }
      ...
      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "No. of oncology patients treated post-tumour board must be less than the no. of new oncology cases treated."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi47.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
