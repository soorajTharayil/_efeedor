...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate return to emergency department within 72 hours function
    $scope.calculateReturnToEmergencyKPI = function () {
      // Get the number of returns to emergency within 72 hours
      var returnsToEmergency = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of patients who have come to the emergency
      var totalPatients = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for returns to emergency and total patients
      if (isNaN(returnsToEmergency) || returnsToEmergency < 0) {
        alert("Enter the no. of returns to emergency within 72 hours.");
        return;
      }

      if (isNaN(totalPatients) || totalPatients <= 0) {
        alert("Enter the no. of patients who have come to the emergency.");
        return;
      }

      if (returnsToEmergency > totalPatients) {
        alert(
          "Number of returns to emergency within 72 hours must be less than the total no. of patients who have come to the emergency."
        );
        return;
      }

      // Calculate the KPI: Return to the emergency department within 72 hours
      var returnToEmergencyKPI = (returnsToEmergency / totalPatients) * 100;

      // Format the result: if it's a whole number, show it without decimal places; otherwise, use two decimal places
      if (returnToEmergencyKPI % 1 === 0) {
        $scope.calculatedResult = returnToEmergencyKPI.toString();
      } else {
        $scope.calculatedResult = returnToEmergencyKPI.toFixed(2);
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi11.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
