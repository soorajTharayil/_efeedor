...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
	...
	$scope.kpiDeadline = computeDeadline($scope.selectedMonths, $scope.selectedYears);
	...
	$scope.setupapplication1 = function () {
		...
		$http.get($rootScope.baseurl_main + '/qualitybenchmark.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
			$scope.benchmarklist = responsedata.data;

			$scope.benchmarkTimes = ($scope.benchmarklist.benchmark || []).map(function (item) {
				return item.bedno && item.bedno.length > 0 ? item.bedno[0] : null;
			});

			$scope.feedback.benchmark = $scope.benchmarkTimes[1]
...
$scope.calculateTimeFormat = function () {
    // Combine input values into a readable format
    ...
    // Get input values as floats (accept decimals)
    ...
    if (hoursInput < 0 || minutesInput < 0 || secondsInput < 0 || totalAdmissions < 0) {
        alert("Negative values are not allowed. Please enter positive or zero values.");
        return;
    }
    ...
    // ✅ Handle zero and both-zero cases
    ...
    // 🚫 Block denominator = 0 when numerator > 0
    if (totalAdmissions === 0 && assessmentSeconds > 0) {
        alert("Total admissions cannot be 0 when time is greater than 0.");
        return;
    }

    // ✅ Normal calculation (allow decimals)
    ...
    // 🕒 Format output properly — include up to 2 decimals if needed
    avgSeconds = parseFloat(avgSeconds.toFixed(2));
    if (avgSeconds >= 60) { // Adjust overflow just in case
        avgSeconds -= 60;
        avgMinutes++;
    }

    $scope.calculatedResult = `${avgHours}:${('0' + avgMinutes).slice(-2)}:${avgSeconds < 10 ? '0' + avgSeconds : avgSeconds}`;
...
// Menu bar start
	...
	$scope.getBenchmarkInSeconds = function () {
		const benchmarkStr = $scope.benchmarkTimes[1]; // pick the relevant one
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4h11')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4h11.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
