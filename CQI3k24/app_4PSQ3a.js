...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter total number of test with in the Z score limit ");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total number of test participated in EQAS programme ");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter total number of test with in the Z score limit be less than total number of test participated in EQAS programme ");
			return;
		}
