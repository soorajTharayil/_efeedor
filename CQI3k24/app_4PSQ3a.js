...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of tests within Z score limit (default 0 if empty)
    var testsWithinZLimit = parseFloat(document.getElementById('formula_para1').value || 0);

    // Get the total number of tests participated in EQAS (default 0 if empty)
    var totalTests = parseFloat(document.getElementById('formula_para2').value || 0);

    // Validate inputs (negative blocked)
    if (testsWithinZLimit < 0) {
        alert("Please enter total number of test within the Z score limit");
        return;
    }

    if (totalTests < 0) {
        alert("Please enter total number of tests participated in EQAS programme");
        return;
    }
    ...
    var percentage = totalTests === 0 ? 0 : (testsWithinZLimit / totalTests) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k24'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k24.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
