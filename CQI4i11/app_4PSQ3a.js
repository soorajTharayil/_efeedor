...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter quantity of biodegradable cleaning products used in the month");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total cleaning materials used ");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter quantity of biodegradable cleaning products used in the month be less than total cleaning materials used ");
			return;
		}
