...
$scope.user_id = ehandor.userid;
    ...
    var biodegradableUsed = parseFloat(document.getElementById('formula_para1').value || 0);
    var totalCleaningMaterials = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (biodegradableUsed < 0) {
        alert("Please enter quantity of biodegradable cleaning products used in the month (cannot be negative)");
        return;
    }

    if (totalCleaningMaterials < 0) {
        alert("Please enter total cleaning materials used (cannot be negative)");
        return;
    }

    

    // Calculate percentage (0/0 allowed → result is 0)
    var percentage = (biodegradableUsed === 0 && totalCleaningMaterials === 0)
        ? 0
        : (biodegradableUsed / totalCleaningMaterials) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4i11'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4i11.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
