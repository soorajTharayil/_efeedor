...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/audit_load_np_ratio_ward.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
			$scope.ratio_ward = responsedata.data;
	...
	$scope.savefeedback = function () {
		...
		function isFeedbackValid() {
			...
			if ($scope.feedback.ward == '' || $scope.feedback.ward == undefined || $scope.feedback.ward == 'Select Ward') {
		...
		$http.post($rootScope.baseurl_main + '/savepatientfeedback_nurse_patients_ratio_ward.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
