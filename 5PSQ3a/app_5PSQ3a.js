...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate medication error rate function
    $scope.calculateAbbreviationErrorRate = function () {
      // Get the number of medication charts with error-prone abbreviations
      var chartsWithErrors = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of medication charts reviewed
      var chartsReviewed = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for error-prone charts and total charts reviewed
      if (isNaN(chartsWithErrors) || chartsWithErrors < 0) {
        alert(
          "Enter the number of medication charts with error-prone abbreviations."
        );
        return;
      }

      if (isNaN(chartsReviewed) || chartsReviewed <= 0) {
        alert("Enter the number of medication charts reviewed.");
        return;
      }

      if (chartsWithErrors > chartsReviewed) {
        alert(
          "Number of medication charts with error-prone abbreviations must be less than the number of medication charts reviewed."
        );
        return;
      }

      // Calculate the percentage of medication charts with error-prone abbreviations
      var abbreviationErrorRate = (chartsWithErrors / chartsReviewed) * 100;

      // Format: if it's a whole number, show it without decimal places; otherwise, use two decimal places
      if (abbreviationErrorRate % 1 === 0) {
        $scope.calculatedResult = abbreviationErrorRate.toString();
      } else {
        $scope.calculatedResult = abbreviationErrorRate.toFixed(2);
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi5.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
