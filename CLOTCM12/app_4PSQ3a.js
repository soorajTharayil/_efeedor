...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    // Get the number of medication errors (accept decimals)
    var medicationErrors = parseFloat(document.getElementById('formula_para1').value);
    // Get the number of opportunities for medication errors (accept decimals)
    var opportunitiesForErrors = parseFloat(document.getElementById('formula_para2').value);

    // 🔹 Validate for NaN or negative values
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter a valid non-negative number for total number of in-patient days for the month");
        return;
    }

    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter a valid non-negative number for number of discharges and deaths in that month");
        return;
    }
    ...
    // 🔹 Denominator zero but numerator > 0 → not allowed
    ...
    // 🔹 Auto-format: whole number or 2 decimal places
    if (averageValue % 1 === 0) {
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CLOTCM12'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CLOTCM12.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
