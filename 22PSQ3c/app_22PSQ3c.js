...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
      ...
      $http
        ...
        .then(
          function (responsedata) {
            ...
            $scope.feedback.benchmark = $scope.benchmarkTimes[2];
    ...
    $scope.calculateTimeFormat = function () {
      ...
      // Get total admission
      var totalOutPatients = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate the total admissions value
      if (isNaN(totalOutPatients) || totalOutPatients <= 0) {
        alert("Please enter the above value to calculate");
        return;
      }

      var averageSeconds = Math.floor(assessmentSeconds / totalOutPatients);
    ...
    $scope.getBenchmarkInSeconds = function () {
      const benchmarkStr = $scope.benchmarkTimes[2]; // pick the relevant one
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi22.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
