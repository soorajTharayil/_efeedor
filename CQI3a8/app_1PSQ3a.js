...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTimeFormat = function () {
      // Combine inputs for display (safe defaults)
      ...
      // Get numeric values (allow decimals)
      ...
      // ✅ Allow all zero (0 hr, 0 min, 0 sec, 0 admissions)
      if (
        hoursInput === 0 &&
        minutesInput === 0 &&
        secondsInput === 0 &&
        ...
        $scope.calculatedResult = "00:00:00";
        $scope.feedback.calculatedResult = $scope.calculatedResult;
        console.log("Calculated result", $scope.calculatedResult);
        return;
      }

      // Validate total admissions (must be number)
      if (isNaN(totalAdmissions)) {
      ...
      // If denominator is zero but numerator > 0 → take raw input
      ...
      // Convert the average back to hr:min:sec
