...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter  total No of patients treated with self harm/suicidal risk adhering with the hospital policies, protocols, & environment safety measures implemented ");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter  Total No of patients with self harm/suicidal risk  treated/admitted in the month ");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter total No of patients treated with self harm/suicidal risk adhering with the hospital policies, protocols, & environment safety measures implemented  be less than total No of patients with self harm/suicidal risk treated/admitted in the month");
			return;
		}
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4g5'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4g5.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
