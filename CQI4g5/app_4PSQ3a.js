  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter  total No of patients treated with self harm/suicidal risk adhering with the hospital policies, protocols, & environment safety measures implemented "
        );
        return;
      }
      ...
      if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
        alert(
          "Please enter  Total No of patients with self harm/suicidal risk  treated/admitted in the month "
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter total No of patients treated with self harm/suicidal risk adhering with the hospital policies, protocols, & environment safety measures implemented  be less than total No of patients with self harm/suicidal risk treated/admitted in the month"
        );
        return;
      }
