...
// 	$scope.setupapplication();
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter number of serious adverse events reported within the defined timeframe ");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter number of serious adverse events reported within and outside the defined timeframe");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter number of serious adverse events reported within the defined timeframe be less than number of serious adverse events reported within and outside the defined timeframe");
			return;
		}
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k58'
		)
