...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculateRescheduledSurgeriesRate = function () {
      // Get the number of cases rescheduled
      var casesRescheduled = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of surgeries planned
      var surgeriesPlanned = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for casesRescheduled and surgeriesPlanned
      if (isNaN(casesRescheduled) || casesRescheduled < 0) {
        alert("Enter the number of cases rescheduled.");
        return;
      }

      if (isNaN(surgeriesPlanned) || surgeriesPlanned < 0) {
        alert("Enter the number of surgeries planned.");
        return;
      }

      if (casesRescheduled > surgeriesPlanned) {
        alert(
          "The number of cases rescheduled must be less than the number of surgeries planned."
        );
        return;
      }

      if (casesRescheduled === 0 && surgeriesPlanned === 0) {
        $scope.calculatedResult = 0;
      } else {
        // Calculate the rescheduled surgeries rate
        $scope.calculatedResult = (
          (casesRescheduled / surgeriesPlanned) *
          100
        ).toFixed(2);
      }
      ...
      console.log(
        "Calculated Rescheduled Surgeries Rate",
        $scope.calculatedResult
      );
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi19.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
