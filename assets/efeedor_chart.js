...
function patient_feedback_analysis(callback) {
  ...
  var apiUrl = baseUrl +  "/analytics/patient_feedback_analysis"; // Replace with your API endpoint
    ...
    options: {
      onClick: function (event, elements) {
        ...
        if (elements.length) {
          ...
          // For example, calling a function and passing the clicked data
...
function handleBarClick(clickData) {
  ...
  var url = baseUrl +  "/ipd/trend_analytic/" + clickData.type;
  window.location = url;
...
function patient_satisfaction_analysis(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = baseUrl +  "/analytics/patient_satisfaction_analysis"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
      ...
      tooltips: {
        ...
        callbacks: {
          label: function (tooltipItems, data) {
            ...
            var all_detail = apiData[dataIndex].all_detail;
            multistringText.push(
              "Satisfied Patients: " + all_detail.rating_bar_positive
            );
            multistringText.push(
              "Unatisfied Patients: " + all_detail.rating_bar_negative
            );
...
function net_permoter_analysis(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = baseUrl +  "/analytics/net_permoter_analysis"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
function why_patient_choose(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = baseUrl +  "/analytics/why_patient_choose"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
function tickets_recived_by_department(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = baseUrl +  "/analytics/tickets_recived_by_department"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
function ticketsRecivedByDepartment(apiData) {
  ...
  console.log(myFeedbackChart);
	if (myFeedbackChart) {
	
	  myFeedbackChart.destroy();
	}
  // Create the Chart.js chart
  myFeedbackChart = new Chart(ctx, {
    type: "pie",
    data: {
      labels: labels,
      datasets: [
...
function tickets_recived_by_department_set(type,value) {
  ...
  var xhr = new XMLHttpRequest();
  var apiUrl = baseUrl +  "/analytics/tickets_recived_by_department_set?settype="+type; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleDropdownChange(value) {
    if (value) {
      var url = " baseUrl +  "/ipd/comments?set=" + value;
      window.location.href = url;
      //renderWordCloud(value);
    } else {
      // baseUrl +  "/ipd/comments";
      var url = " baseUrl +  "/ipd/comments";
      window.location.href = url;
...
function renderWordCloud(type = null) {
  ...
  var apiUrlBase = baseUrl +  "/wordcloud/ip_comment";
