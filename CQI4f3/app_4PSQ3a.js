  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter number of sentinel events, reported, collected and analysed within the defined time frame "
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter number of sentinel events, reported, collected and analysed within the defined time frame be less than number of sentinel events reported and collected "
        );
        return;
      }
