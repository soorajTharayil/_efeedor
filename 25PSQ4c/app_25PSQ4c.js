...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculateIncompleteConsentRate = function () {
      // Get the number of medical records having incomplete and/or improper consent
      var incompleteConsentRecords = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of discharges and deaths
      var dischargesAndDeaths = parseInt(
      ...
      if (isNaN(incompleteConsentRecords) || incompleteConsentRecords < 0) {
        alert(
          "Enter the number of medical records having incomplete and/or improper consent."
        );
        return;
      }
      ...
      if (incompleteConsentRecords > dischargesAndDeaths) {
        alert(
          "Number of medical records having incomplete/improper consent must be less than the number of discharges and deaths."
        );
        return;
      }
      ...
      if (incompleteConsentRecords === 0 && dischargesAndDeaths === 0) {
        ...
        $scope.calculatedResult = (
          (incompleteConsentRecords / dischargesAndDeaths) *
          100
        ).toFixed(2);
      }
      ...
      console.log(
        "Calculated Incomplete Consent Rate",
        $scope.calculatedResult
      );
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi25.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
