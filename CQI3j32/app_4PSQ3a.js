...
$scope.calculateMedicationErrorRate = function () {
    ...
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter No of Safety, security incidents related to Work place violence");
        return;
    }

    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter No of safety, security related incidents reported in the month");
        return;
    }

    // ✅ Allow both zeros — result will be 0%
    ...
    // 🚫 Block division by zero if denominator = 0 and numerator > 0
    if (opportunitiesForErrors === 0 && medicationErrors > 0) {
        alert("No of safety, security related incidents reported in the month cannot be 0 when incidents are reported.");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3j32')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3j32.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
