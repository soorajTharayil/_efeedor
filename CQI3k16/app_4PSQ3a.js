...
$scope.user_id = ehandor.userid;
    ...
    // Get input values
    var documentedDietOrders = parseFloat(document.getElementById('formula_para1').value || 0);
    var totalInpatients = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative values
    if (documentedDietOrders < 0) {
        alert("Number of inpatient case records wherein the diet order has been documented cannot be negative");
        return;
    }
    ...
    var documentationRate = (documentedDietOrders === 0 && totalInpatients === 0)
        ? 0
        : (documentedDietOrders / totalInpatients) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k16'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQIk16.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
