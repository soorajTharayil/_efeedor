...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
	...
	$scope.calculateErrorRate = function () {
		...
		// Format the result and append unit
		if (errorsPerThousand % 1 === 0) {
			$scope.calculatedResult = errorsPerThousand.toString() + " per 1,000 in-patient days";
		} else {
			$scope.calculatedResult = errorsPerThousand.toFixed(2) + " per 1,000 in-patient days";
		}

		// Store result in the feedback object
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4f5')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4f5.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
