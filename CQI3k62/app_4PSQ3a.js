...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of code blues where ACLS protocols were initiated within 5 min
    var aclsInitiated = parseFloat(document.getElementById('formula_para1').value);

    // Get the total number of code blues activated in the month
    var totalCodeBlue = parseFloat(document.getElementById('formula_para2').value);

    // Validate inputs: negative values blocked
    if (isNaN(aclsInitiated) || aclsInitiated < 0) {
        alert("Please enter total no of code blue where ACLS protocols initiated within 5 min (cannot be negative)");
        return;
    }

    if (isNaN(totalCodeBlue) || totalCodeBlue < 0) {
        alert("Please enter total no of code blue activated in the month (cannot be negative)");
        return;
    }
    ...
    var percentage;
    if (aclsInitiated === 0 && totalCodeBlue === 0) {
        percentage = 0; // both zero → 0%
    } else {
        percentage = (aclsInitiated / totalCodeBlue) * 100;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k62'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k62.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
