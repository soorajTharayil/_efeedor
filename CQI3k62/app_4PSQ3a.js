  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter total no of code blue where ACLS protocols initiated within 5 min"
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter total no of code blue where ACLS protocols initiated within 5 min be less than total No of code blue activated in the month "
        );
        return;
      }
