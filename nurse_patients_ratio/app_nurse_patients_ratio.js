...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/audit_load_np_ratio_icu.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
			$scope.ratio_icu = responsedata.data;
	...
	$scope.setupapplication1 = function () {
		...
		var id = url.substring(url.lastIndexOf('=') + 1);
		//alert(id);
		$http.get($rootScope.baseurl_main + '/audit_load_patient_status.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
			$scope.patient_status = responsedata.data;
			console.log($scope.auditdept);
		},
			function myError(response) {
				$rootScope.loader = false;
	...
	$scope.savefeedback = function () {
		...
		function isFeedbackValid() {
			...
			if ($scope.feedback.icu == '' || $scope.feedback.icu == undefined || $scope.feedback.icu == 'Select ICU') {
				alert("Please select ICU");
				return false;
			}

			if ($scope.feedback.department == '' || $scope.feedback.department == undefined || $scope.feedback.icu == 'Select Patient Status') {
				alert("Please select patient status");
				return false;
			}

			if ($scope.feedback.beds === undefined || $scope.feedback.beds === null || $scope.feedback.beds === '') {
