...
$scope.user_id = ehandor.userid;
    ...
    // Get inputs as float to accept decimals
    var staffAdhering = parseFloat(document.getElementById('formula_para1').value || 0);
    var staffAudited = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (staffAdhering < 0) {
        alert("Please enter the number of staff adhering to safety precautions (cannot be negative).");
        return;
    }

    if (staffAudited < 0) {
        alert("Please enter the number of staff audited (cannot be negative).");
        return;
    }
    ...
    var adherenceRate = (staffAudited === 0) ? 0 : (staffAdhering / staffAudited) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3b8')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3b8.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
