  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors <= 0) {
        alert(
          "Please enter number of women having cesarean section for first time"
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      if (
        $scope.feedback.initial_assessment_hr === undefined ||
        $scope.feedback.initial_assessment_hr === null ||
        $scope.feedback.initial_assessment_hr === ""
      ) {
        alert(
          "Please enter number of patients who develop device associated pressure ulcer."
        );
        return false;
      }
