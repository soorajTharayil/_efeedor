...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_2PSQ3a.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
			...
			console.log($scope.diagnostic_errors_count);
...
$scope.user_id = ehandor.userid;
    ...
    if (reportingErrors < 0) {
        alert("Please enter Number of falls in a given month (cannot be negative)");
        return;
    }
    if (testsPerformed < 0) {
        alert("Please enter Total number of patient days for the month (cannot be negative)");
        return;
    }
    ...
    var errorsPerThousand = (reportingErrors === 0 && testsPerformed === 0) ? 0 : (reportingErrors / testsPerformed) * 1000;

    // Format result: whole number if exact, else 2 decimals
    $scope.calculatedResult = (errorsPerThousand % 1 === 0) ? errorsPerThousand.toString() : errorsPerThousand.toFixed(2);
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4b2')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4b2.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
