...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter a valid (non-negative) total number of medication errors");
        return;
    }

    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter a valid (non-negative) total number of opportunities for medication errors");
        return;
    }
    ...
    // Denominator cannot be zero alone
    if (opportunitiesForErrors === 0) {
        alert("Opportunities for medication errors cannot be zero unless both are zero");
        return;
    }
    ...
    if (averageValue % 1 === 0) {
        $scope.calculatedResult = averageValue.toString();
    } else {
        $scope.calculatedResult = averageValue.toFixed(2);
    }
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CLOTCM8'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CLOTCM8.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
