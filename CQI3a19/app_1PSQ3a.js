...
$scope.user_id = ehandor.userid;
    ...
    // Get hours, minutes, and seconds input (allow decimals)
    ...
    var assessmentSeconds = Math.floor(hoursInput * 3600 + minutesInput * 60 + secondsInput);

    // Get total admissions
    var totalAdmissions = parseFloat(document.getElementById('formula_para2').value || 0);

    // Validate negative admissions
    if (totalAdmissions < 0) {
        alert("Total admissions cannot be negative");
        return;
    }

    // Calculate average seconds; allow zero numerator/denominator → result 0:00:00
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3a19')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3a19.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
