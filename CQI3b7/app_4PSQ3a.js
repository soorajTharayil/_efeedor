...
// 	$scope.setupapplication();
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter the number of test reports correlating with clinical diagnosis.");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter the of tests performed.");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("The no. of test reports correlating with clinical diagnosis must be less than the no. of tests performed.");
			return;
		}
