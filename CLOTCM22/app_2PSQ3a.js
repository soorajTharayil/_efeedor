...
$scope.calculateErrorRate = function () {
    ...
    // Validate negative or invalid inputs
    if (isNaN(reportingErrors) || reportingErrors < 0) {
        alert("Please enter number of central line associated blood stream infections in a month");
        return;
    }

    if (isNaN(testsPerformed) || testsPerformed < 0) {
        alert("Please enter number of central line days in that month");
        return;
    }

    // Allow numerator > denominator and both zero
    var errorsPerThousand = 0;
    if (testsPerformed === 0) {
        // Both zero or denominator zero → define result as 0
        errorsPerThousand = 0;
    } else {
    ...
    // Format output — whole → no decimals, else → 2 decimals
    if (Number.isInteger(errorsPerThousand)) {
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CLOTCM22')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CLOTCM22.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
