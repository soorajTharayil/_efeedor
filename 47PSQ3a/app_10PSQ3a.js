  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert("Enter no. of patients who developed adverse reaction(s).");
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert("Enter total no. of patients receiving the radiopharmaceutical.");
        return;
      }

      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "No. of patients who developed adverse reaction(s) must be less than the no. of radiopharmaceutical patients."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi48.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
