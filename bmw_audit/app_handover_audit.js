...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //     ($scope.feedback.department + "").trim() === ""
    //   ) {
    ...
    //   ) {
    //     alert("Please select Building/Area");
    //     return;
    //   }
    //   if (
    //     !$scope.feedback.mid_no ||
    //     ($scope.feedback.mid_no + "").trim() === ""
    //   ) {
    ...
    //   if (
    //   !$scope.feedback.supervisor_name ||
    //   ($scope.feedback.supervisor_name + "").trim() === ""
    // ) {
    ...
    // if (
    //   !$scope.feedback.driver_name ||
    //   ($scope.feedback.driver_name + "").trim() === ""
    // ) {
    ...
    // if (
    //   !$scope.feedback.picker_name ||
    //   ($scope.feedback.picker_name + "").trim() === ""
    // ) {
    ...
    $scope.setupapplication2 = function () {
      ...
      //alert(id);
      $http
        .get($rootScope.baseurl_main + "/audit_load_area.php?patientid=" + id, {
          timeout: 20000,
        })
        .then(
          function (responsedata) {
    ...
    $scope.$watchGroup(
  [
    'feedback.yellow_bags',
    'feedback.red_bags',
    'feedback.white_bags',
    'feedback.brownish_yellow_bags',
    'feedback.blue_bags'
  ],
  function (newValues) {
    let total = 0;
    newValues.forEach(v => total += parseInt(v) || 0);
    $scope.feedback.total_bags = total;
  }
);
    ...
    $scope.calculateTimeFormat = function () {
      ...
      // Get admission time
      var admissionHours = parseInt(
        document.getElementById("formula_para1_hr").value || 0
      );
      var admissionMinutes = parseInt(
        document.getElementById("formula_para1_min").value || 0
      );
      var admissionSeconds = parseInt(
        document.getElementById("formula_para1_sec").value || 0
      );

      // Get assessment completion time
      var assessmentHours = parseInt(
        document.getElementById("formula_para2_hr").value || 0
      );
      var assessmentMinutes = parseInt(
        document.getElementById("formula_para2_min").value || 0
      );
      var assessmentSeconds = parseInt(
        document.getElementById("formula_para2_sec").value || 0
      );

      // Convert times to total seconds for easier comparison
      var totalAdmissionSeconds = convertToTotalSeconds(
        admissionHours,
        admissionMinutes,
        admissionSeconds
      );
      var totalAssessmentSeconds = convertToTotalSeconds(
        assessmentHours,
        assessmentMinutes,
        assessmentSeconds
      );
      ...
      $scope.calculatedResult = `${diffHours}:${("0" + diffMinutes).slice(
        -2
    ...
    $scope.calculateDoctorAdviceToBillPaid = function () {
      ...
      var doctorAdviceHours = parseInt(
        document.getElementById("formula_para3_hr").value || 0
      );
      var doctorAdviceMinutes = parseInt(
        document.getElementById("formula_para3_min").value || 0
      );
      var doctorAdviceSeconds = parseInt(
        document.getElementById("formula_para3_sec").value || 0
      );

      // Get the time when the bill was paid
      var billPaidHours = parseInt(
        document.getElementById("formula_para4_hr").value || 0
      );
      var billPaidMinutes = parseInt(
        document.getElementById("formula_para4_min").value || 0
      );
      var billPaidSeconds = parseInt(
        document.getElementById("formula_para4_sec").value || 0
      );

      // Convert times to total seconds for easier comparison
      var totalDoctorAdviceSeconds = convertToTotalSeconds(
        doctorAdviceHours,
        doctorAdviceMinutes,
        doctorAdviceSeconds
      );
      var totalBillPaidSeconds = convertToTotalSeconds(
        billPaidHours,
        billPaidMinutes,
        billPaidSeconds
      );
      ...
      // Format the result to "hr:min:sec"
      $scope.calculatedDoctorAdviceToBillPaid = `${diffHours}:${(
        "0" + diffMinutes
      ).slice(-2)}:${("0" + diffSeconds).slice(-2)}`;

      // Store the result in the feedback object for further use
      $scope.feedback.calculatedDoctorAdviceToBillPaid =
        $scope.calculatedDoctorAdviceToBillPaid;

      console.log(
        "Calculated Doctor Advice to Bill Paid:",
        $scope.calculatedDoctorAdviceToBillPaid
      );
    };
    ...
    $scope.prev = function () {
      window.location.href = "/audit_forms#step2";
    };
    ...
    $scope.savefeedback = function () {
      ...
      $scope.feedback.initial_assessment_hr6 = formatDateToLocalString(
        $scope.feedback.initial_assessment_hr6
      );
      $scope.feedback.discharge_date_time = formatDateToLocalString(
        $scope.feedback.discharge_date_time
      );
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/save_ma_bmw_audit.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId,
          $scope.feedback
