...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of patients treated inappropriately
    var patientsInappropriatelyTreated = parseFloat(document.getElementById('formula_para1').value || 0);

    // Get the total number of patients with bronchiolitis
    var totalPatientsBronchiolitis = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (patientsInappropriatelyTreated < 0) {
        alert("Number of patients treated inappropriately cannot be negative.");
        return;
    }

    if (totalPatientsBronchiolitis < 0) {
        alert("Total number of patients with bronchiolitis cannot be negative.");
        return;
    }
    ...
    var errorRatePercentage = totalPatientsBronchiolitis === 0 ? 0 : (patientsInappropriatelyTreated / totalPatientsBronchiolitis) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3a16')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3a16.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
