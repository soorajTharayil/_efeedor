...
// 	$scope.setupapplication();
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter Number of patients treated inappropriately");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter Total number of patients with bronchiolitis");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Number of patients treated inappropriately less than Total number of patients with bronchiolitis.");
			return;
		}
