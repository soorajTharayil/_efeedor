...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculateSSIRate = function () {
      // Get the number of surgical site infections in a month
      var surgicalSiteInfections = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of surgeries performed in that month
      var surgeriesPerformed = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for surgicalSiteInfections and surgeriesPerformed
      if (isNaN(surgicalSiteInfections) || surgicalSiteInfections < 0) {
        alert("Enter the number of surgical site infections in a month.");
        return;
      }

      if (isNaN(surgeriesPerformed) || surgeriesPerformed <= 0) {
        alert("Enter the number of surgeries performed in that month.");
        return;
      }

      if (surgicalSiteInfections > surgeriesPerformed) {
        alert(
          "Number of surgical site infections must be less than the number of surgeries performed."
        );
        return;
      }

      if (surgicalSiteInfections === 0 && surgeriesPerformed === 0) {
        $scope.calculatedResult = 0;
      } else {
        // Calculate the SSI rate
        $scope.calculatedResult = (
          (surgicalSiteInfections / surgeriesPerformed) *
          100
        ).toFixed(2);
      }
      ...
      console.log("Calculated SSI rate", $scope.calculatedResult);
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi16.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
