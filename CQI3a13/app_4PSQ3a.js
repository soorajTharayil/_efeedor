...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
	...
	$scope.calculateMedicationErrorRate = function () {
		var medicationErrors = document.getElementById('formula_para1').value.trim();
		var opportunitiesForErrors = document.getElementById('formula_para2').value.trim();

		if (medicationErrors === '' || isNaN(medicationErrors) || parseFloat(medicationErrors) < 0) {
			alert("Please enter Number of cases where a spontaneous perineal tear occurs.");
			return;
		}
		if (opportunitiesForErrors === '' || isNaN(opportunitiesForErrors) || parseFloat(opportunitiesForErrors) < 0) {
			alert("Please enter Total number of Vaginal deliveries.");
			return;
		}

		var medErr = parseFloat(medicationErrors);
		var oppErr = parseFloat(opportunitiesForErrors);

		var errorRatePercentage = (oppErr > 0) ? (medErr / oppErr) * 100 : 0;
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3a13')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3a13.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
