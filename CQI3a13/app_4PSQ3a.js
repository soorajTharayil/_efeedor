  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter Number of cases where a spontaneous perineal tear occurs."
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Number of cases where a spontaneous perineal tear occurs be less than Total number of Vaginal deliveries."
        );
        return;
      }
