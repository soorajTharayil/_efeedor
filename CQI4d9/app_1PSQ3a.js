...
$scope.calculateTimeFormat = function () {
    // Display initial total time from input
    ...
    // Parse all values (allow decimals)
    ...
    // ✅ Handles 0 denominator gracefully
    var averageSeconds = 0;
    if (assessmentSeconds === 0 && totalAdmissions === 0) {
        averageSeconds = 0;
    } else if (totalAdmissions === 0) {
        averageSeconds = assessmentSeconds; // avoid divide-by-zero
    } else {
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4d9')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4d9.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
