...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    // Get input values
    var documentedCases = parseFloat(document.getElementById('formula_para1').value || 0);
    var totalInpatients = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative values
    if (documentedCases < 0) {
        alert("Number of case sheets where nutritional assessment is documented cannot be negative");
        return;
    }

    if (totalInpatients < 0) {
        alert("Total number of in-patients for the month cannot be negative");
        return;
    }
    ...
    var documentationRate = (documentedCases === 0 && totalInpatients === 0) 
        ? 0 
        : (documentedCases / totalInpatients) * 100;

    // Auto-format result: whole number or 2 decimals
    $scope.calculatedResult = (documentationRate % 1 === 0)
        ? documentationRate.toString()
        : documentationRate.toFixed(2);
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3a5')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3a5.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
