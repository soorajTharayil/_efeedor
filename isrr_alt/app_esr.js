...
var ehandor = JSON.parse($window.localStorage.getItem('ehandor'));
  ...
  $scope.encodeFiles = function (element) {
    var files = Array.from(element.files); // Convert FileList to an array
    ...
    files.forEach(function (file) {
      ...
      $http.post($rootScope.baseurl_main + '/upload_file.php', formData, {
        ...
        if (response.data.file_url) {
          ...
          // Add each uploaded file's information to the feedback.files array
          $scope.feedback.files.push({
            url: fileUrl,
            name: file.name
          });
        }
