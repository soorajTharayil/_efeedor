...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		// Validate inputs (allow 0, but not negative or blank)
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter total number of days (MRF submission date - Staff onboarded date)");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
			alert("Please enter total number of Manpower requisition form received");
			return;
		}
		...
		if (opportunitiesForErrors !== 0) {
			errorRatePercentage = medicationErrors / opportunitiesForErrors;
		}

		// Format: keep whole number as is, else 2 decimals, then add unit
		if (errorRatePercentage % 1 === 0) {
			$scope.calculatedResult = errorRatePercentage.toString() + " days/MRF";
		} else {
			$scope.calculatedResult = errorRatePercentage.toFixed(2) + " days/MRF";
		}

		// Store result
		$scope.feedback.calculatedResult = $scope.calculatedResult;

		console.log("Calculated average result", $scope.calculatedResult);
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4h16'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4h16.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
