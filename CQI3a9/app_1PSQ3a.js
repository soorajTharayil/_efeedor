...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTimeFormat = function () {
      // Get user input values (default to 0 if empty)
      let hr = parseFloat($scope.feedback.initial_assessment_hr || 0);
      let min = parseFloat($scope.feedback.initial_assessment_min || 0);
      let sec = parseFloat($scope.feedback.initial_assessment_sec || 0);
      ...
      $scope.feedback.initial_assessment_total = `${hr}:${min}:${sec}`;
      ...
      if (isNaN(totalAdmissions) || totalAdmissions < 0) {
        alert("Please enter a valid total admission value (0 or positive).");
        return;
      }
      ...
      // Handle zero input safely (both zero allowed)
      ...
      // Format the result
      const formatTwo = (num) => (num % 1 === 0 ? num : num.toFixed(2)); // 2 decimals if fractional

      $scope.calculatedResult = `${formatTwo(avgHours)}:${(
        "0" + formatTwo(avgMinutes)
      ).slice(-2)}:${("0" + formatTwo(avgSeconds)).slice(-2)}`;

      // Save result to scope
      $scope.feedback.calculatedResult = $scope.calculatedResult;
      $scope.valuesEdited = false;

      console.log("Calculated Average:", $scope.calculatedResult);
    };
