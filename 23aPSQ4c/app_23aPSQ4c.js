...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
      ...
      $http
        ...
        .then(
          function (responsedata) {
            ...
            $scope.feedback.benchmark = $scope.benchmarkTimes[3];
    ...
    $scope.getBenchmarkInSeconds = function () {
      const benchmarkStr = $scope.benchmarkTimes[3]; // pick the relevant one
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi23a.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
