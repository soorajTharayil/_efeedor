...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculatePatientFallRate = function () {
      // Get the number of patient falls
      var patientFalls = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the total number of patient days
      var patientDays = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for patientFalls and patientDays
      if (isNaN(patientFalls) || patientFalls < 0) {
        alert("Enter the number of patient falls.");
        return;
      }

      if (isNaN(patientDays) || patientDays < 0) {
        alert("Enter the total number of patient days.");
        return;
      }

      if (patientFalls > patientDays) {
        alert(
          "The number of patient falls must be less than the total number of patient days."
        );
        return;
      }

      if (patientFalls === 0 && patientDays === 0) {
        $scope.calculatedResult = 0;
      } else {
        // Calculate the patient fall rate per 1000 patient days
        $scope.calculatedResult = Math.round(
          (patientFalls / patientDays) * 1000
        );
      }
      ...
      console.log("Calculated Patient Fall Rate", $scope.calculatedResult);
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi28.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
