...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
        ...
        .then(
          function (responsedata) {
            $scope.feedback.initial_assessment_hr =
              responsedata.data.staff_adhering_to_safety_precautions;
            $scope.feedback.total_admission =
              responsedata.data.staff_audited_count;
    ...
    $scope.calculateAdherenceRate = function () {
      // Get the number of staff adhering to safety precautions
      var adheringStaff = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of staff audited
      var auditedStaff = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for staff adherence and staff audited
      ...
      if (adheringStaff > auditedStaff) {
        alert(
          "The no. of staff adhering to safety precautions must be less than the number of staff audited."
        );
        return;
      }

      // Calculate the percentage of adherence to safety precautions
      var adherencePercentage = (adheringStaff / auditedStaff) * 100;

      if (adherencePercentage % 1 === 0) {
        $scope.calculatedResult = adherencePercentage.toString();
      } else {
        $scope.calculatedResult = adherencePercentage.toFixed(2);
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi3.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
