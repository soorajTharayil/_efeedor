...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $window, $timeout) {
	...
	$scope.login = function () {
		...
		grecaptcha.ready(function () {
			grecaptcha.execute('6Lc8CkcqAAAAACv2WebgYioIVJhljcDhqJk5AbAz', { action: 'login' }).then(function (token) {
				...
				$http.post($rootScope.baseurl_main + '/login.php', $scope.loginvar, { timeout: 20000 }).then(function (responsedata) {
					...
					if (responsedata.status == 200) {
						...
						// $timeout(function() {  // THEN, use $timeout to trigger digest
						// 	console.log("adminId after timeout:", $rootScope.adminId); 
						// });
