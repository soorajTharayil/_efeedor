  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
        alert(
          "Please enter total number of in patients requiring physiotherapy"
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter total number of cases where in Initial assessment done in physiotherapy with in 24 hours be less than total number of in patients requiring physiotherapy"
        );
        return;
      }
