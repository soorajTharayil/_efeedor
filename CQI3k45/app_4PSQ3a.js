...
$scope.calculateMedicationErrorRate = function () {
    // Parse inputs (accept integers and decimals)
    ...
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter total number of cases where Initial assessment done in physiotherapy within 24 hours");
        return;
    }
    ...
    // Denominator zero but numerator > 0 → blocked
        ...
        alert("Total number of in patients requiring physiotherapy cannot be zero when Initial assessment cases are greater than zero");
        return;
    } 
