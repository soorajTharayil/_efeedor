...
$scope.user_id = ehandor.userid;
    ...
    // Get inputs (parseFloat allows decimals)
    ...
    if (medicationErrors < 0) {
        alert("Negative values are not allowed for 'staff adhering to safety precautions'.");
        return;
    }

    if (opportunitiesForErrors < 0) {
        alert("Negative values are not allowed for 'number of staff audited'.");
        return;
    }
    ...
    if (opportunitiesForErrors === 0) {
        if (medicationErrors === 0) {
            ...
            alert("Number of staff audited cannot be zero unless both values are zero.");
            return;
        }
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3b10')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3b10.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
