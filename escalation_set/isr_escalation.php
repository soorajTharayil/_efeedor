...
$query = 'SELECT * FROM `escalation` WHERE section="ESR"';
$escilation = mysqli_fetch_object(mysqli_query($con, $query));
if ($escilation->status != 'ACTIVE') {
	///exit;
	$query = 'SELECT * FROM  bf_feedback_esr  WHERE escilationstatus = 0';
	$flore = mysqli_query($con, $query);
	while ($r = mysqli_fetch_object($flore)) {
		$quiery = 'Update bf_feedback_esr set escilationstatus = -1 WHERE id=' . $r->id;
...
while ($r = mysqli_fetch_object($flore)) {
	...
	$query = 'SELECT * FROM  tickets_esr  inner JOIN department ON department.dprt_id = tickets_esr.departmentid   WHERE  feedbackid = ' . $r->id . ' GROUP BY  department.description';
	...
	while ($t = mysqli_fetch_object($ticket)) {
		...
		$query = 'SELECT * FROM  tickets_esr  inner JOIN department ON department.dprt_id = tickets_esr.departmentid   WHERE  feedbackid = ' . $r->id . ' AND department.description="' . $t->description . '"';
		...
		$department_head_link = $config_set['BASE_URL'] . 'isr/track/' . $TID;
		...
		foreach ($escilate_to_department as $e3) {
			...
			$setArray = explode(',', $e3->department['esr'][$t->setkey]);
			...
			if (in_array($t->slug, $setArray)) {
				...
				if ($escilation->dept_level_whatsapp_alert == 'YES') {
					...
					$campaignName = 'service_request_level1_escalationalert';
					$templateParams = json_encode([$hospitalname,$Concern_Category, $Concern_Area,  $patient_ward, $patient_bedno, $patient_name, $patient_contact, $department_head_link, $hospitalname], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
		...
		foreach ($escilate_to_one as $e1) {
			...
			if ($escilation->level1_whatsapp_alert == 'YES') {
				$destination = '91' . $number;
				$userName = 'ITATONE POINT CONSULTING LLP 7345';
				$campaignName = 'level2_service_request_escalationalert';
				$templateParams = json_encode([$hospitalname,$Concern_Category, $Concern_Area,  $patient_ward, $patient_bedno, $patient_name, $patient_contact, $department_head_link, $hospitalname], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
				$source = 'new-landing-page form';
				$media = '{}';
				$buttons = '[]';
		...
		foreach ($escilate_to_two as $e2) {
			...
			if ($escilation->level2_whatsapp_alert == 'YES') {
				$destination = '91' . $number;
				$userName = 'ITATONE POINT CONSULTING LLP 7345';
				$campaignName = 'level3_service_request_escalationalert';
				$templateParams = json_encode([$hospitalname,$Concern_Category, $Concern_Area,  $patient_ward, $patient_bedno, $patient_name, $patient_contact, $department_head_link, $hospitalname], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
				$source = 'new-landing-page form';
				$media = '{}';
				$buttons = '[]';
	...
	$quiery = 'Update bf_feedback_esr set escilationstatus = 5 WHERE id=' . $r->id;
