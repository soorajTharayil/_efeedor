...
if ($escilation->status != 'ACTIVE') {
	///exit;
	$query = 'SELECT * FROM  bf_feedback_incident  WHERE escilationstatus_assigned_explanation = 0';
	$flore = mysqli_query($con, $query);
	while ($r = mysqli_fetch_object($flore)) {
		$quiery = 'Update bf_feedback_incident set escilationstatus_assigned_explanation = -1 WHERE id=' . $r->id;
...
while ($r = mysqli_fetch_object($flore)) {
	...
	$query = 'SELECT * FROM  bf_employees_esr WHERE id = "' . $r->pid . '"';
	...
	while ($t = mysqli_fetch_object($ticket)) {
		...
		foreach ($escilate_to_department as $e3) {
			...
			$setArray = explode(',', $e3->department['incident'][$t->setkey]);
				...
				if ($escilation->dept_level_whatsapp_alert == 'YES') {
						...
						$campaignName = 'level1_incident_escalationalert';
	...
	$quiery = 'Update bf_feedback_incident set escilationstatus_assigned_explanation = 5 WHERE id=' . $r->id;
