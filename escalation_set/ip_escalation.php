...
$query = 'SELECT * FROM `escalation` WHERE section="IP"';
$escilation = mysqli_fetch_object(mysqli_query($con, $query));
if ($escilation->status != 'ACTIVE') {
	///exit;
	$query = 'SELECT * FROM  bf_feedback  WHERE escilationstatus = 0';
	$flore = mysqli_query($con, $query);
	while ($r = mysqli_fetch_object($flore)) {
		$quiery = 'Update bf_feedback set escilationstatus = -1 WHERE id=' . $r->id;
...
while ($r = mysqli_fetch_object($flore)) {
	...
	while ($t = mysqli_fetch_object($ticket)) {
		...
		$department_head_link = $config_set['BASE_URL'] . 'ipd/track/' . $TID;
		...
		$message = "URGENT!%0aLevel 1 Escalation.%0aInpatient issue not addressed at " . trim($hospitalname) . ".%0aLogin to view details:%0a" . trim($department_head_link) . "%0a-EFEEDOR";
		$message = str_replace('&', 'and', str_replace(' ', '%20', $message));
		foreach ($escilate_to_department as $e3) {
			$number = $e3->mobile;

			$setArray = explode(',', $e3->department['inpatient'][$t->setkey]);
			...
			if (in_array($t->slug, $setArray)) {
				...
				if ($escilation->dept_level_sms_alert == 'YES') {
					...
					// 		VALUES 
					// 		("message","' . $message . '",0,"' . $e3->mobile . '","1607100000000336092","' . $HID . '","' . $escilate_level_departmnet_time . '","' . mysqli_real_escape_string($con, json_encode($meta_data)) . '")';
				...
				if ($escilation->dept_level_whatsapp_alert == 'YES') {
					...
					$campaignName = 'inpatient_level1_negativefeedback_escalationalert';
					$templateParams = json_encode([$hospitalname, $patient_name, $patient_ward, $patient_bedno, $Concern_Category, $Concern_Area, $department_head_link, $hospitalname], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
		...
		foreach ($escilate_to_one as $e1) {
			...
			if ($escilation->level1_whatsapp_alert == 'YES') {
				$destination = '91' . $number;
				$userName = 'ITATONE POINT CONSULTING LLP 7345';
				$campaignName = 'inpatient_level2_negativefeedback_escalationalert';
				$templateParams = json_encode([$hospitalname, $patient_name, $patient_ward, $patient_bedno, $Concern_Category, $Concern_Area, $department_head_link, $hospitalname], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
				$source = 'new-landing-page form';
				$media = '{}';
				$buttons = '[]';
		...
		foreach ($escilate_to_two as $e2) {
			...
			if ($escilation->level2_whatsapp_alert == 'YES') {
				$destination = '91' . $number;
				$userName = 'ITATONE POINT CONSULTING LLP 7345';
				$campaignName = 'inpatient_level3_negativefeedback_escalationalert';
				$templateParams = json_encode([$hospitalname, $patient_name, $patient_ward, $patient_bedno, $Concern_Category, $Concern_Area, $department_head_link, $hospitalname], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
				$source = 'new-landing-page form';
				$media = '{}';
				$buttons = '[]';
	...
	$quiery = 'Update bf_feedback set escilationstatus = 5 WHERE id=' . $r->id;
