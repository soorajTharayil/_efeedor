<?php
	...
	if($escilation->status != 'ACTIVE'){
		//exit;
		$query = 'SELECT * FROM  bf_outfeedback  WHERE escilationstatus_email = 0';
	$flore = mysqli_query($con, $query);
	while ($r = mysqli_fetch_object($flore)){
		$quiery = 'Update bf_outfeedback set escilationstatus_email = -1 WHERE id=' . $r->id;
	...
	while ($r = mysqli_fetch_object($flore)){
		...
		$query = 'SELECT * FROM  bf_opatients  WHERE id = "' . $r->pid . '"';
...
$meta_data = array();
		while ($t = mysqli_fetch_object($ticket)){$meta_data['ticket'] = $t;		$meta_data['config_set_url'] = $config_set['BASE_URL'];			$meta_data['config_set_domain'] = $config_set['DOMAIN'];			
			...
			$subject_1 = 'Level 1 ESCALATION- HID: '.$HID.'- COMPLAINT FROM OUTPATIENT FEEDBACK TICKET- EFEEDOR';
			$subject_2 = 'Level 2 ESCALATION- HID: '.$HID.'- COMPLAINT FROM OUTPATIENT FEEDBACK TICKET- EFEEDOR';
