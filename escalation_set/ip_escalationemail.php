<?php
	...
	if($escilation->status != 'ACTIVE'){
		//exit;
		$query = 'SELECT * FROM  bf_feedback  WHERE escilationstatus_email = 0';
	$flore = mysqli_query($con, $query);
	while ($r = mysqli_fetch_object($flore)){
		$quiery = 'Update bf_feedback set escilationstatus_email = -1 WHERE id=' . $r->id;
...
$meta_data['ticket'] = $t;
		...
		$meta_data['config_set_domain'] = $config_set['DOMAIN'];		
			...
			$subject_1 = 'Level 1 ESCALATION- HID: '.$HID.'- COMPLAINT FROM INPATIENT FEEDBACK TICKET- EFEEDOR';
			$subject_2 = 'Level 2 ESCALATION- HID: '.$HID.'- COMPLAINT FROM INPATIENT FEEDBACK TICKET- EFEEDOR';
