...
$query = 'SELECT * FROM `escalation` WHERE section="INCIDENT"';
$escilation = mysqli_fetch_object(mysqli_query($con, $query));
if ($escilation->status != 'ACTIVE') {
	///exit;
	$query = 'SELECT * FROM  bf_feedback_incident  WHERE escilationstatus = 0';
	$flore = mysqli_query($con, $query);
	while ($r = mysqli_fetch_object($flore)) {
		$quiery = 'Update bf_feedback_incident set escilationstatus = -1 WHERE id=' . $r->id;
...
while ($r = mysqli_fetch_object($flore)) {
	...
	$patient_contact = $parameter->contactnumber;
	$patient_serverity = $parameter->incident_type;
	...
	$query = 'SELECT * FROM  tickets_incident  inner JOIN department ON department.dprt_id = tickets_incident.departmentid   WHERE  feedbackid = ' . $r->id . ' GROUP BY  department.description';
	...
	$query = 'SELECT * FROM  bf_employees_incident WHERE id = "' . $r->pid . '"';
	...
	while ($t = mysqli_fetch_object($ticket)) {
		...
		$query = 'SELECT * FROM  tickets_incident  inner JOIN department ON department.dprt_id = tickets_incident.departmentid   WHERE  feedbackid = ' . $r->id . ' AND department.description="' . $t->description . '"';


		$department = $t->description;
		$status_assigned = $t->status;
		...
		$query = 'SELECT * FROM  bf_ward_esr  WHERE title = "' . $p->ward . '"';
		...
		$message = "URGENT!%0aLevel 1 Escalation.%0aInpatient issue not addressed at " . $hospitalname . ".%0aLogin to view details:%0a" . $department_head_link . "%0a-EFEEDOR";
		...
		foreach ($escilate_to_one as $e1) {
			...
			if ($escilation->level1_sms_alert == 'YES') {
				...
				// 				VALUES 
				// 				("message","' . $message . '",0,"' . $e1->mobile . '","1607100000000336092","' . $HID . '","' . $escilate_level_one_time . '","' . mysqli_real_escape_string($con, json_encode($meta_data)) . '")';
			...
			if ($escilation->level1_whatsapp_alert == 'YES') {
				...
				$campaignName = 'level2_incident_escalationalert';
				$templateParams = json_encode([$hospitalname, $Concern_Category, $Concern_Area, $patient_serverity, $patient_ward, $patient_bedno, $patient_name, $patient_contact, $department_head_link, $hospitalname], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
		...
		foreach ($escilate_to_two as $e2) {
			...
			if ($escilation->level2_whatsapp_alert == 'YES') {
				$destination = '91' . $number;
				$userName = 'ITATONE POINT CONSULTING LLP 7345';
				$campaignName = 'level3_incident_escalationalert';
				$templateParams = json_encode([$hospitalname, $Concern_Category, $Concern_Area, $patient_serverity, $patient_ward, $patient_bedno, $patient_name, $patient_contact, $department_head_link, $hospitalname], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
				$source = 'new-landing-page form';
				$media = '{}';
				$buttons = '[]';
	...
	$quiery = 'Update bf_feedback_incident set escilationstatus = 5 WHERE id=' . $r->id;
