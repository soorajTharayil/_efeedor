...
$scope.calculateMedicationErrorRate = function () {
	...
	// 🚫 Denominator = 0 (but numerator > 0) → blocked
	...
	// ✅ Numerator > denominator → allowed, calculate normally
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		if (($scope.feedback.initial_assessment_hr === '' || $scope.feedback.initial_assessment_hr === undefined)) {
			alert('Please enter single value - Total Number of fire incidents');
			return false;
		}
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4b5')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4b5.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
