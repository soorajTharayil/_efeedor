...
if (count($data) > 1) {
 ...
 $query = 'INSERT INTO `bf_feedback_esr`(`datetime`,`datet`,`remarks`, `nurseid`, `patientid`, `dataset`, `source`,`ward`,`bed_no`,`pid`,`image`,`file`) 
    ...
    while ($r = mysqli_fetch_object($result)) {
        ...
        foreach ($data['reason'] as $key => $value) {
            if ($r->slug == $key) {
                //var_dump($value);
                if ($value == true) {
                    $query = 'INSERT INTO `tickets_esr`(`created_by`, `departmentid`, `rating`, `anymessage`, `feedbackid`,`ward`,`priority`) VALUES ("' . $patinet_id . '","' . $r->dprt_id . '","' . $value . '","' . $data['comments'] . '","' . $fid . '","' . $wardd->title . '","' . $priority . '")';                    mysqli_query($con, $query);
                }
            }
        }
