...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of appropriate handovers (accept decimal values)
    ...
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter Number of appropriate handovers ");
        return;
    }
    ...
    // Format output (whole number or two decimals)
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		if ($scope.feedback.initial_assessment_hr > $scope.feedback.total_admission) {
			alert('Number of appropriate handovers  less than Number of handover opportunities');
			return false;
		}

		// First check for duplicates
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3j20')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3j20.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
