...
$scope.calculateErrorRate = function () {
    ...
    // Allow 0 or both 0, and numerator > denominator
    var errorsPerThousand = 0;
    if (testsPerformed === 0) {
        // if denominator = 0 (or both zero), define result as 0
        errorsPerThousand = 0;
    } else {
        errorsPerThousand = (reportingErrors / testsPerformed) * 1000;
    }

    // Format result: integer → no decimals, else → 2 decimals
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CLOTCM21')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CLOTCM21.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
