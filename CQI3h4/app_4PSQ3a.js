...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      // Prevent invalid division (denominator = 0 but numerator > 0)
      if (opportunitiesForErrors === 0 && medicationErrors > 0) {
        alert(
          "Number of discharges / transfers in ICU cannot be zero when there are returns to ICU."
        );
        return;
      }
