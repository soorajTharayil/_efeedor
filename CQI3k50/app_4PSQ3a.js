...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of site markings done as per hospital policy
    var medicationErrors = parseFloat(document.getElementById('formula_para1').value || 0);

    // Get the number of site markings audited in the month
    ...
    if (medicationErrors < 0) {
        alert("Please enter a non-negative value for number of site marking done as per hospital policy");
        return;
    }

    if (opportunitiesForErrors < 0) {
        alert("Please enter a non-negative value for number of site marking audited in the month");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k50'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k50.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
