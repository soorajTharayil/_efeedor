...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
        ...
        .then(
          function (responsedata) {
            $scope.feedback.initial_assessment_hr =
              responsedata.data.hygieneCount;
            $scope.feedback.total_admission =
              responsedata.data.opportunitiesCount;
    ...
    //Calculate  function
    $scope.calculateHandHygieneComplianceRate = function () {
      // Get the total number of hand hygiene actions performed
      var handHygieneActions = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the total number of hand hygiene opportunities
      var handHygieneOpportunities = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for handHygieneActions and handHygieneOpportunities
      if (isNaN(handHygieneActions) || handHygieneActions < 0) {
        alert("Enter the total number of hand hygiene actions performed.");
        return;
      }

      if (isNaN(handHygieneOpportunities) || handHygieneOpportunities <= 0) {
        alert("Enter the total number of hand hygiene opportunities.");
        return;
      }

      if (handHygieneActions > handHygieneOpportunities) {
        alert(
          "Number of hand hygiene actions must be less than the number of hand hygiene opportunities."
        );
        return;
      }

      if (handHygieneActions === 0 && handHygieneOpportunities === 0) {
        $scope.calculatedResult = 0;
      } else {
        // Calculate the hand hygiene compliance rate
        $scope.calculatedResult = (
          (handHygieneActions / handHygieneOpportunities) *
          100
        ).toFixed(2);
      }
      ...
      console.log(
        "Calculated Hand Hygiene Compliance Rate",
        $scope.calculatedResult
      );
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi17.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
