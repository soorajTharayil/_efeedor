  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert("Enter no. of dialysis patients achieving hemoglobin levels.");
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert("Enter total no. of dialysis patients.");
        return;
      }

      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "No. of dialysis patients achieving hemoglobin levels must be less than the total no. of dialysis patients."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi51.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
