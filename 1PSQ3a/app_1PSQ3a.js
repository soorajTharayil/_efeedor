...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
      ...
      $http
        ...
        .then(
          function (responsedata) {
            ...
            $scope.feedback.benchmark = $scope.benchmarkTimes[0];
    ...
    $scope.getBenchmarkInSeconds = function () {
      const benchmarkStr = $scope.benchmarkTimes[0]; // pick the relevant one
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_quality.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
