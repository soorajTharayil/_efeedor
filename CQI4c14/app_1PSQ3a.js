...
$scope.calculateTimeFormat = function () {
	// Combine hour, minute, second fields into total string
	...
	if (assessmentSeconds === 0 && totalAdmissions === 0) {
		// Both zero → 0:00:00
		averageSeconds = 0;
	} else if (totalAdmissions === 0) {
		// Denominator zero → keep numerator
		averageSeconds = assessmentSeconds;
	} else {
		// Normal division (even if numerator > denominator)
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4c14')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4c14.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
