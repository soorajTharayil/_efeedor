...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
        ...
        .then(
          function (responsedata) {
            $scope.feedback.initial_assessment_hr =
              responsedata.data.total_nurses;
    ...
    $scope.calculateNursePatientRatio = function () {
      var nursingStaff = parseInt(
        document.getElementById("formula_para1").value
      );
      var occupiedBeds = parseInt(
      ...
      if (nursingStaff < occupiedBeds) {
        ...
        explanation =
          " means 1 nurse is assigned to " + roundedRatio + " patients.";
      } else {
        ...
        explanation =
          "means " + roundedRatio + " nurses are assigned to 1 patient.";
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi21a.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
