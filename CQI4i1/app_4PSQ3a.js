...
$scope.user_id = ehandor.userid;
    ...
    var servicesProvided = parseFloat(document.getElementById('formula_para1').value || 0);
    var servicesRequested = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (servicesProvided < 0) {
        alert("Total number of medical services accepted or provided cannot be negative.");
        return;
    }
    if (servicesRequested < 0) {
        alert("Total number of medical services requested cannot be negative.");
        return;
    }
    ...
    // Handle zero cases
    if (servicesProvided === 0 || servicesRequested === 0) {
        $scope.calculatedResult = "0";
    } else {
        // Calculate percentage
        var percentage = (servicesProvided / servicesRequested) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4i1'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4i1.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
