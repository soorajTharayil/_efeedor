...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
	...
	$scope.savefeedback = function () {
		...
		if ((!$scope.feedback.initial_assessment_hr && $scope.feedback.initial_assessment_hr !== 0) &&
			(!$scope.feedback.initial_assessment_min && $scope.feedback.initial_assessment_min !== 0) &&
			(!$scope.feedback.initial_assessment_sec && $scope.feedback.initial_assessment_sec !== 0)) {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4h39')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4h39.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
