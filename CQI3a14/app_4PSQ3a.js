...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      // ✅ Allow zero but block negative or invalid values
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter Number of sepsis patients who receive care as per the hour-1 sepsis bundle"
        );
        return;
      }
      ...
      // ✅ Remove restriction for numerator > denominator
      // (You asked to calculate even if numerator > denominator)
      // So this validation is removed.

      // ✅ Safe calculation to handle zero denominator
