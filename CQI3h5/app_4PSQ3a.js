  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter number of return to emergency department after 72 hours with similar presenting complaints"
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "number of return to emergency department after 72 hours with similar presenting complaints less than Number patients who have come to the emergency "
        );
        return;
      }
