...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.calculateTimeFormat = function () {
		...
		if (!assessmentDate || isNaN(assessmentDate.getTime())) {
			alert("Please select procedure entry time.");
			return;
		}

		if (assessmentDate <= admissionDate) {
			alert("Procedure entry time must be greater than bill prepared time.");
			return;
		}
	...
	$scope.savefeedback = function () {
		...
		if (!$scope.feedback.initial_assessment_hr2 || isNaN(assessmentDate.getTime())) {
			alert("Please enter procedure entry time.");
			return;
		}
		...
		$http.post($rootScope.baseurl_main + '/savepatientfeedback_xray_time.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
