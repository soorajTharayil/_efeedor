...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $window, $timeout) {
	...
	$scope.next0 = function () {
		...
		grecaptcha.ready(function () {
			grecaptcha.execute('6Lc8CkcqAAAAACv2WebgYioIVJhljcDhqJk5AbAz', { action: 'submit_mobile_number' }).then(function (token) {
				$http({
					method: 'GET',
					url: $rootScope.baseurl_main + '/mobile_inpatient_captcha.php?mobile=' + $scope.feedback.contactnumber + '&recaptcha_token=' + token,
					timeout: 20000
	...
	$scope.setupapplication = function () {
		...
		$http.get($rootScope.baseurl_main + '/ward.php?patientid=' + id, { timeout: 20000 })
			.then(function (responsedata) {
				...
				if (locationValue && $scope.wardlist.ward) {
					...
					if (matchingWard) {
						...
						$scope.bed_no = matchingWard.bedno || []; // Ensure bed list is properly assigned

						console.log("Available Beds in Ward:", $scope.bed_no);

						// Disable selection fields for ward and site
						$scope.isWardPreselected_location = true;
						$scope.isWardPreselected_site = false;

						// If siteValue exists and matches a bed number, auto-select it
						if (siteValue && Array.isArray($scope.bed_no)) {
							var matchingBed = $scope.bed_no.includes(siteValue.trim());

							if (matchingBed) {
								$scope.feedback.bedno = siteValue.trim(); // Assign matching bed
								console.log("Auto-selected Bed:", $scope.feedback.bedno);

								// Disable bed selection once auto-selected
