...
$scope.user_id = ehandor.userid;
    ...
    if (medicationErrors < 0) {
        alert("Please enter Total quantity rejected (cannot be negative)");
        return;
    }
    if (opportunitiesForErrors < 0) {
        alert("Please enter Total quantity received before Goods Receipt Note (cannot be negative)");
        return;
    }

    

    // Calculate the error rate (%), allow 0/0 → returns 0
