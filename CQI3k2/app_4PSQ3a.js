...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateCesareanRate = function () {
    // Get inputs
    var cesareanCount = parseFloat(document.getElementById('formula_para1').value || 0);
    var liveBirthsOtherThanCesarean = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative values
    if (cesareanCount < 0) {
        alert("Please enter number of women having cesarean section for first time (cannot be negative)");
        return;
    }
    if (liveBirthsOtherThanCesarean < 0) {
        alert("Please enter total number of live birth other than cesarean (cannot be negative)");
        return;
    }
    ...
    var ratePercentage = (liveBirthsOtherThanCesarean === 0) ? 0 : (cesareanCount / liveBirthsOtherThanCesarean) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k2')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k2.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
