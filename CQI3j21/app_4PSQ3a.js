...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of correct site/procedure/surgery adherences
    var adherenceCount = parseFloat(document.getElementById('formula_para1').value || 0);

    // Get the number of samples audited
    var totalSamples = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (adherenceCount < 0) {
        alert("Please enter Adherence to correct site, correct procedure, correct surgery (cannot be negative)");
        return;
    }

    if (totalSamples < 0) {
        alert("Please enter Number of samples audited (cannot be negative)");
        return;
    }
    ...
    // Zero numerator and denominator allowed → result is 0
    var adherenceRate = (adherenceCount === 0 && totalSamples === 0) ? 0 : (adherenceCount / totalSamples) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3j21')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3j21.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
