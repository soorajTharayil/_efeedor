...
$scope.user_id = ehandor.userid;
    ...
    var numerator = parseFloat(document.getElementById('formula_para1').value);
    var denominator = parseFloat(document.getElementById('formula_para2').value);

    // Block negative numbers only
    if (isNaN(numerator) || numerator < 0) {
        alert("Please enter a valid non-negative numerator value");
        return;
    }

    if (isNaN(denominator) || denominator < 0) {
        alert("Please enter a valid non-negative denominator value");
        return;
    }
    ...
    if (numerator === 0 && denominator === 0) {
        result = 0; // both zero → 0
    } else if (denominator === 0) {
        result = 100; // denominator zero, numerator > 0 → 100%
    } else {
        result = (numerator / denominator) * 100; // normal calculation
    }

    // Auto-format: whole number or 2 decimals
    $scope.calculatedResult = result % 1 === 0 ? result.toString() : result.toFixed(2);
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4g1'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4g1.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
