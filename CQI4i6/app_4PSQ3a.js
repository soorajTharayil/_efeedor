...
$scope.user_id = ehandor.userid;
    ...
    var absenteeism = parseFloat(document.getElementById('formula_para1').value || 0);
    var totalManpower = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (absenteeism < 0) {
        alert("Please enter number of Absenteeism in a month (cannot be negative)");
        return;
    }
    ...
    var percentage = (absenteeism === 0 && totalManpower === 0)
        ? 0
        : (absenteeism / totalManpower) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4i6'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4i6.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
