    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (opportunitiesForErrors === 0 && medicationErrors > 0) {
        $scope.calculatedResult =
          medicationErrors + ":0 (No opportunities for errors)";
        $scope.feedback.calculatedResult = $scope.calculatedResult;
        return;
      }

      // ✅ Perform ratio calculation
      var ratio, ratioString, explanation;

      if (medicationErrors < opportunitiesForErrors) {
        ratio = opportunitiesForErrors / medicationErrors;
        var formatted = Number.isInteger(ratio) ? ratio : ratio.toFixed(2);
        ratioString = "1:" + formatted;
        explanation =
          " means 1 nurse is assigned to " + formatted + " patients.";
      } else {
        ratio = medicationErrors / opportunitiesForErrors;
        var formatted = Number.isInteger(ratio) ? ratio : ratio.toFixed(2);
        ratioString = formatted + ":1";
        explanation =
          " means " + formatted + " nurses are assigned to 1 patient.";
      }
