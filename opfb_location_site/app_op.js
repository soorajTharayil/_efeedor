var app = angular.module('ehandorApp', ['ui.bootstrap']);
...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $timeout) {
	...
	$scope.reloadQuestionsForDepartment = function (selectedDepartment) {
		...
		var departmentQuestionMapping = {
			"Department of Laboratory Medicine": ["Appointment Booking", "Registration & Reception", "Billing", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"Emergency": ["Appointment Booking", "Registration & Reception", "Billing", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"Health Check Services": ["Appointment Booking", "Registration & Reception", "Billing", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"Inpatient Services": ["Appointment Booking", "Registration & Reception", "Billing", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"Intensive Care Unit (ICU)": ["Appointment Booking", "Registration & Reception", "Billing", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"Laboratory": ["Appointment Booking", "Registration & Reception", "Billing", "Laboratory", "Other Facilities"],
			"Outpatient Services": ["Appointment Booking", "Registration & Reception", "Billing", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"Pharmacy": ["Billing", "Pharmacy", "Other Facilities"],
			"Radiology": ["Appointment Booking", "Registration & Reception", "Billing", "Radiology", "Other Facilities"]
		};
	...
	$scope.onWardChange = function () {
		if (!$scope.feedback.ward || !$scope.wardlist.ward) return;
	
		// Find the selected ward
		var selectedWard = $scope.wardlist.ward.find(function (ward) {
			return ward.title === $scope.feedback.ward;
		});
	
		if (selectedWard) {
			$scope.bed_no = selectedWard.bedno || [];
			$scope.filteredDoctors = selectedWard.doctorname || [];
	
			console.log("User selected Ward:", selectedWard.title);
			console.log("Beds loaded:", $scope.bed_no);
			console.log("Doctors loaded:", $scope.filteredDoctors);
		}
	};

	$scope.setupapplication = function () {
		var urlParams = new URLSearchParams(window.location.search);
		var locationValue = urlParams.get("location"); // Extract location
		var siteValue = urlParams.get("site"); // Extract site
		...
		$http.get($rootScope.baseurl_main + '/department.php?patientid=' + id, { timeout: 20000 })
			.then(function (responsedata) {
				...
				console.log("Fetched Ward List:", $scope.wardlist);
	
				// Set ward dropdown list
				$scope.wardDropdown = $scope.wardlist.ward || [];
				$scope.isWardPreselected_location = false;
				$scope.isWardPreselected_site = false;
	
				// If location exists in URL, try to match
				if ($scope.wardlist.ward) {
					var matchingWard = $scope.wardlist.ward.find(function (ward) {
						return ward.title.trim().toLowerCase() === locationValue.trim().toLowerCase();
					});
	
					if (matchingWard) {
						console.log("Matching Ward Found:", matchingWard);
						$scope.feedback.ward = matchingWard.title;
						$scope.bed_no = matchingWard.bedno || [];
						$scope.filteredDoctors = matchingWard.doctorname || [];
						$scope.isWardPreselected_location = true;
	
						if (siteValue && Array.isArray($scope.bed_no)) {
							var matchingBed = $scope.bed_no.includes(siteValue.trim());
							if (matchingBed) {
								$scope.feedback.bedno = siteValue.trim();
								$scope.isWardPreselected_site = true;
								console.log("Auto-selected Bed:", $scope.feedback.bedno);
							}
						}
					} else {
						console.warn("No matching ward found for location param");
					}
				} else {
					// No location in URL: allow dropdowns to be used normally
					$scope.feedback.ward = '';
					$scope.bed_no = [];
					$scope.filteredDoctors = [];
				}
	
				console.log("Final Feedback Data:", $scope.feedback);
				$scope.$applyAsync();
			}, function (myError) {
				$rootScope.loader = false;
				console.error("Error fetching ward data:", myError);
			});
	};
