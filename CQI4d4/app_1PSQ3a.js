...
$scope.calculateTimeFormat = function () {
    ...
    // Get numeric values (support decimals)
    ...
    // ✅ Safe handling of divide-by-zero
    let averageSeconds = 0;
    if (assessmentSeconds === 0 && totalAdmissions === 0) {
        averageSeconds = 0; // both zero → show 0:00:00
    } else if (totalAdmissions === 0) {
        averageSeconds = assessmentSeconds; // avoid NaN
    } else {
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4d4')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4d4.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
