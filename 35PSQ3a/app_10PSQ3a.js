  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert(
          "Enter no. of patients with hypoglycemic events where the glucose level was achieved"
        );
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert("Enter no. of patients with Hypoglycemic events");
        return;
      }

      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "Enter no. of patients achieving glucose level is must be less than total hypoglycemic events."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi36.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
