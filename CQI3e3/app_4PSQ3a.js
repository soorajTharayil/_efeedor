...
$scope.user_id = ehandor.userid;
    ...
    var surgeriesFollowed = parseFloat(document.getElementById('formula_para1').value || 0);
    var surgeriesPerformed = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (surgeriesFollowed < 0) {
        alert("Please enter number of surgeries where the procedure was followed (cannot be negative).");
        return;
    }
    if (surgeriesPerformed < 0) {
        alert("Please enter number of surgeries performed in that month (cannot be negative).");
        return;
    }
    ...
    // Handle zero cases
    if (surgeriesFollowed === 0 || surgeriesPerformed === 0) {
        $scope.calculatedResult = "0";
    } else {
        // Calculate percentage
        var percentage = (surgeriesFollowed / surgeriesPerformed) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3e3')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3e3.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
