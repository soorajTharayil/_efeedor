  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter number of surgeries where the procedure was followed"
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "number of surgeries where the procedure was followed less than number of surgeries performed in that month"
        );
        return;
      }
