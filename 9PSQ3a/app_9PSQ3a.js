...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate transfusion reaction function
    $scope.calculateTransfusionReactionRate = function () {
      // Get the number of transfusion reactions
      var transfusionReactions = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the total number of units transfused
      var totalUnitsTransfused = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for transfusion reactions and total units transfused
      if (isNaN(transfusionReactions) || transfusionReactions < 0) {
        alert("Enter the number of transfusion reactions.");
        return;
      }

      if (isNaN(totalUnitsTransfused) || totalUnitsTransfused <= 0) {
        alert("Enter the total number of units transfused.");
        return;
      }

      if (transfusionReactions > totalUnitsTransfused) {
        alert(
          "Number of transfusion reactions must be less than the number of units transfused."
        );
        return;
      }

      // Calculate the percentage of transfusion reactions
      var transfusionReactionRate =
        (transfusionReactions / totalUnitsTransfused) * 100;

      // Format the result: if it's a whole number, show it without decimal places; otherwise, use two decimal places
      if (transfusionReactionRate % 1 === 0) {
        $scope.calculatedResult = transfusionReactionRate.toString();
      } else {
        $scope.calculatedResult = transfusionReactionRate.toFixed(2);
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi9.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
