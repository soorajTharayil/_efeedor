...
$scope.user_id = ehandor.userid;
    ...
    // Get values for hours, minutes, and seconds from user input
    ...
    if (hoursInput < 0 || minutesInput < 0 || secondsInput < 0) {
        alert("Time inputs cannot be negative");
        return;
    }
    ...
    // Block negative total admissions
    if (totalAdmissions < 0) {
        alert("Total admissions cannot be negative");
        return;
    }

    // Calculate average seconds safely
    var averageSeconds = 0;
    if (totalAdmissions !== 0) {
        averageSeconds = Math.floor(assessmentSeconds / totalAdmissions);
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3f5')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3f5.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
