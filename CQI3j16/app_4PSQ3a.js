...
$scope.user_id = ehandor.userid;
    ...
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter Number of appropriate handovers by Doctors");
        return;
    }
    ...
    // Handle case: both values zero → result should be 0%
    ...
    // Prevent division by zero (if denominator is 0 but numerator > 0)
    if (opportunitiesForErrors === 0) {
        alert("Number of handover opportunities Doctors cannot be zero unless both values are zero");
        return;
    }

    // Calculate even if numerator > denominator
    var errorRatePercentage = (medicationErrors / opportunitiesForErrors) * 100;

    // Format result (integer or 2 decimal places)
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3j16')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3j16.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
