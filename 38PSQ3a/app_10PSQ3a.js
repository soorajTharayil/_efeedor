  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert("Enter no. of patients sedated for the colonoscopy procedure");
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert("Enter total no. of patients undergoing Colonoscopy");
        return;
      }

      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "No. of patients sedated for colonoscopy procedure must be less than the total no. of patients undergoing Colonoscopy."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi39.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
