  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateErrorRate = function () {
      ...
      if (isNaN(reportingErrors) || reportingErrors < 0) {
        alert(
          "Please enter number of IVD associated blood stream infections in a month"
        );
        return;
      }
      ...
      if (reportingErrors > testsPerformed) {
        alert(
          "number of IVD associated blood stream infections in a  month less than Number of IVD inserted that month."
        );
        return;
      }
