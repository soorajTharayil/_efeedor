...
$sql = 'SELECT * FROM `bf_nursing_notes_two` WHERE patientid="'.$_REQUEST['patient_id'].'" GROUP BY datet,shift ORDER BY datet asc' ;			
$result = mysqli_query($con,$sql);	
$i=1;
while($row = mysqli_fetch_object($result)){
	...
	$sql = 'SELECT * FROM `bf_nursing_notes_two` Where datet="'.$row->datet.'" AND shift="'.$row->shift.'" AND patientid="'.$_REQUEST['patient_id'].'"' ;			
	$resshift = mysqli_query($con,$sql);	
	while($rowshift = mysqli_fetch_object($resshift)){
		$datashift = json_decode($rowshift->notes);
		 $datashift->evaluationList = array_unique($datashift->evaluationList);
		$sql = 'SELECT * FROM `bf_brand_two` WHERE guid="'.$datashift->problemNameKey.'"' ;			
        ...
        foreach ($datashift->evaluationList as $d) {
			$sql   = 'SELECT * FROM `bf_related_two` WHERE guid="' . $d . '"';
