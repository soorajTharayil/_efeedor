...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter Number of patients with hypoglycemic events where the target glucose level was achieved post-treatment");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter Number of patients with Hypoglycemic events");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Number of patients with hypoglycemic events where the target glucose level was achieved post-treatment be less than Number of patients with Hypoglycemic events");
			return;
		}
