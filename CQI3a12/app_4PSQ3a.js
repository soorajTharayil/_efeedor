...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of patients with hypoglycemic events where target glucose achieved
    var achieved = parseFloat(document.getElementById('formula_para1').value || 0);

    // Get the total number of patients with hypoglycemic events
    var total = parseFloat(document.getElementById('formula_para2').value || 0);

    // Validate inputs: negative values blocked
    if (achieved < 0) {
        alert("Number of patients with hypoglycemic events where target glucose level was achieved cannot be negative");
        return;
    }
    if (total < 0) {
        alert("Total number of patients with hypoglycemic events cannot be negative");
        return;
    }

    
    // Calculate percentage
    var percentage = (achieved === 0 && total === 0) ? 0 : (achieved / total) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3a12')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3a12.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
