...
$scope.user_id = ehandor.userid;
    ...
    var handoversDone = parseFloat(document.getElementById('formula_para1').value || 0);
    var handoverOpportunities = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (handoversDone < 0) {
        alert("Number of handovers done by doctors cannot be negative.");
        return;
    }
    if (handoverOpportunities < 0) {
        alert("Number of handover opportunities for doctors cannot be negative.");
        return;
    }
    ...
    if (handoversDone === 0 || handoverOpportunities === 0) {
        ...
        var percentage = (handoversDone / handoverOpportunities) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3j29')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3j29.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
