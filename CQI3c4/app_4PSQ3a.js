...
$scope.user_id = ehandor.userid;
    ...
    if (medicationErrors < 0) {
        alert("Please enter number of adverse drug reactions");
        return;
    }
    ...
    if (opportunitiesForErrors === 0) {
        errorRatePercentage = (medicationErrors === 0 ? 0 : 100); // 0/0 = 0%, numerator>0 with denom=0 = 100%
    } else {
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		if ($scope.feedback.initial_assessment_hr > $scope.feedback.total_admission) {
			alert('The no.of adverse drug reactions less than the no.of discharges and deaths in that month');
			return false;
		}

		// First check for duplicates
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3c4')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3c4.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
