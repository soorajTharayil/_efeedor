...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter total ischemic stroke patients receiving the intravenous thrombolysis within appropriate time");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
			alert("Please enter total eligible ischemic stroke patients receiving the intravenous thrombolysis within appropriate time");
			return;
		}
