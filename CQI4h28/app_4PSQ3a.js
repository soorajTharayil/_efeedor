  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter total critical values reported in POCT as per policy "
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter total critical values reported in POCT as per policy be less than total number of critical values"
        );
        return;
      }
