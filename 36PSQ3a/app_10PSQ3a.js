  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "Enter no. of cases where spontaneous perineal tear occurs must be less than the total no. of vaginal deliveries."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi37.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
