...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.setupapplication = function () {
        ...
        $http.get($rootScope.baseurl_main + '/assetdepartmentlist.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
            ...
            $scope.gradelist = responsedata.data;
    ...
    // Load depreciation and short code based on asset group & generate asset code
    $scope.change_asset = function () {
        ...
        if (foundWard) {
            ...
            // Generate asset code using short_code from selected asset group
            if ($scope.setting_data && $scope.setting_data.description && foundWard.short_code) {
                let hid = $scope.setting_data.description.trim();
                let shortCode = foundWard.short_code.trim();
                let randomNumber = Math.floor(100000 + Math.random() * 900000);

                let generatedAssetCode = `${hid}/${shortCode}/${randomNumber}`;
                $scope.feedback.patientid = generatedAssetCode;

                console.log("Generated New Asset Code:", generatedAssetCode);
            }
        } else {
            $scope.feedback.depreciation = null;
            $scope.feedback.patientid = null;
        }
    ...
    $scope.setupapplication1 = function () {
        ...
        var id = url.substring(url.lastIndexOf('=') + 1);
        //alert(id);
        $http.get($rootScope.baseurl_main + '/esr_wards.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
            $scope.locationlist = responsedata.data;
            $scope.setting_data = responsedata.data.setting_data;
            $scope.assetdept = responsedata.data.assetdept;
            ...
            // // Generate asset code 
            // if ($scope.setting_data && $scope.setting_data.description && $scope.assetdept && $scope.assetdept.short_code) {
            //     let hid = $scope.setting_data.description.trim();
            //     let shortCode = $scope.assetdept.short_code.trim();
            //     let timestamp = Date.now(); // Unique part

            //     let generatedAssetCode = `${hid}/${shortCode}/${timestamp}`;
    ...
    $scope.calculateDepreciation = function () {
        ...
        if (!installDate || isNaN(unitPrice) || isNaN(depreciationRate) || !assetGroup) {
            ...
            $scope.fyAssetValues = {};
            $scope.feedback.fyAssetValues = {};
            return;
        }

        // Retrieve method from wardlist.ward
        let method = null;
        if ($scope.wardlist && Array.isArray($scope.wardlist.ward)) {
            ...
            if (matchedWard && matchedWard.method) {
                method = matchedWard.method.toUpperCase();
            }
        }

        if (!method || (method !== 'SLM' && method !== 'WDV')) {
            $scope.assetCurrentValue = unitPrice.toFixed(2);
            $scope.feedback.assetCurrentValue = unitPrice.toFixed(2);
            $scope.fyAssetValues = {};
            $scope.feedback.fyAssetValues = {};
            return;
        }

        // Start FY-based calculation
        let fyAssetValues = {};
        ...
        while (fyStart <= currentDate) {
            ...
            if (fyYear >= currentFYStart - 2) {
                let periodStart = fyStart > installDate ? new Date(fyStart) : new Date(installDate);
                let periodEnd;

                if (fyYear === currentFYStart) {
                    periodEnd = new Date(currentDate);
                } else {
                    periodEnd = new Date(fyYear + 1, 2, 31); // March 31 of next year
                }
        ...
        // Store FY-wise values
        $scope.fyAssetValues = fyAssetValues;
        $scope.feedback.fyAssetValues = fyAssetValues;


        const fyKeys = Object.keys(fyAssetValues);
        if (fyKeys.length > 0) {
            const latestFyLabel = fyKeys[fyKeys.length - 1];
            const latestValue = parseFloat(fyAssetValues[latestFyLabel]);

            $scope.assetCurrentValue = latestValue.toFixed(2);
            $scope.feedback.assetCurrentValue = latestValue.toFixed(2);
        } else {
            $scope.assetCurrentValue = unitPrice.toFixed(2);
            $scope.feedback.assetCurrentValue = unitPrice.toFixed(2);
        }
    ...
    $scope.savefeedback = function () {
        ...
        if (!$scope.isDepreciationCalculated) {
            alert("Please calculate depreciation before saving.");
            return;
        }
        ...
        $scope.feedback.lastMaintenance = formatDateToLocalString($scope.feedback.lastMaintenance);
        $scope.feedback.upcomingMaintenance = formatDateToLocalString($scope.feedback.upcomingMaintenance);
        $scope.feedback.lastCalibration = formatDateToLocalString($scope.feedback.lastCalibration);
        $scope.feedback.upcomingCalibration = formatDateToLocalString($scope.feedback.upcomingCalibration);
        ...
        QRCode.toDataURL($scope.assetUrl, {
