  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateComplianceRate = function () {
      ...
      if (isNaN(prescriptionsInCapitals) || prescriptionsInCapitals < 0) {
        alert("Enter the total number of prescriptions in capital letters.");
        return;
      }

      if (isNaN(totalPrescriptions) || totalPrescriptions <= 0) {
        alert("Enter the total number of prescriptions.");
        return;
      }

      if (prescriptionsInCapitals > totalPrescriptions) {
        alert(
          "The number of prescriptions in capital letters must be less than the total number of prescriptions."
        );
        return;
      }
      ...
      $scope.calculatedResult = (
        (prescriptionsInCapitals / totalPrescriptions) *
        100
      ).toFixed(2);
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi32.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
