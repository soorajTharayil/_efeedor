...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculateCLABSIRate = function () {
      // Get the number of central line-associated bloodstream infection
      var centralLineInfections = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of central line days in that month
      var centralLineDays = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for centralLineInfections and centralLineDays
      if (isNaN(centralLineInfections) || centralLineInfections < 0) {
        alert(
          "Enter the number of central line-associated bloodstream infections."
        );
        return;
      }

      // if (isNaN(centralLineDays) || centralLineDays <= 0) {
      // 	alert("Enter the number of central line days.");
      // 	return;
      // }

      if (centralLineInfections > centralLineDays) {
        alert(
          "Number of central line-associated bloodstream infections must be less than the number of central line days."
        );
        return;
      }

      // Calculate the CLABSI rate
      var clabsiRate = (centralLineInfections / centralLineDays) * 1000;

      // Format the result to two decimal places
      if (clabsiRate % 1 === 0) {
        $scope.calculatedResult = clabsiRate.toString();
      } else {
        $scope.calculatedResult = clabsiRate.toFixed(2);
      }

      // Store the result in the feedback object for further use
      $scope.feedback.calculatedResult = $scope.calculatedResult;

      console.log("Calculated CLABSI rate", $scope.calculatedResult);
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi15.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
