...
<body ng-app="ehandorApp" ng-controller="PatientFeedbackCtrl" style="display:none;" id="body">
	...
	<nav class="navbar navbar-expand-sm navbar-dark bg-dark" style="height: 60px;">
		...
		<div class="ml-auto d-flex justify-content-between align-items-center w-100">
			<div class="left-buttons d-flex" ng-show="step2 || step3 || step4 || step5 || step6">
			...
			<div class="right-buttons d-flex align-items-center">
				<button type="button" class="btn btn-dark language-btn" data-toggle="modal" data-target="#languageModal" style="margin: 4px;" ng-show="step2 || step3 || step4 || step5 || step6">
					{{type2}}
					<i class="fa fa-language" aria-hidden="true"></i>
				</button>
				<button class="btn btn-dark menu-toggle" ng-click="menuVisible = !menuVisible" style="margin: 4px; float:right;" ng-show="step2 || step3 || step4 || step5 || step6">
	...
	<div class="menu-dropdown" ng-show="menuVisible" style="margin-top: 32px; margin-right: 10px;">
		<div class="user-info" style="display: flex; align-items: center; padding: 10px; border-bottom: 1px solid #ddd; background: #f5f5f5;">
			...
			<div>
				<div style="font-weight: bold; font-size: 14px;">{{ (userid ? loginname : profilen.name) }}</div>
				<div style="font-size: 12px; font-weight: bold;">{{ (userid ? loginemail : profilen.email) }}</div>
			</div>
		</div>
								...
								<fieldset ng-show="step2 == true">
									...
									<div class="text-left" style="width: 94%; margin: 0px auto;">
										<div class="search-container">
											...
											<input type="text" placeholder="{{lang.mainSearch}}"
												ng-model="searchTextmain" ng-change="searchChanged()"
												value="searchTextmain" oninput="restrictToAlphabets(event)">
											<i class="fa fa-search search-icon"></i>
											<div class="dropdown-results"
												ng-if="searchTextmain && searchTextmain.length > 1">



												<div ng-repeat="q in questioset">
													<ul>
														<li ng-repeat="p in q.question | filter:searchTextmain"
															ng-click="selectQuestionCategory1(p, q)"
															...
															<label class="container">
																...
																<input type="checkbox" ng-model="p.valuetext"
																	ng-show="p.showQuestion">
								...
								<fieldset ng-show="step4 == true">
									<div class="form-card" style="margin-top: 10px;">
										<div class="form-group" style="margin-left: 13px; margin-right:13px;" ng-show="submit_as_concern == true">
											...
											<label for="comment" ng-show="typel == 'english'">{{lang.u_select}} <br>
												<b> {{lang.incident_category}}</b>{{selectedParameterObject.title}} <br>
												<b> {{lang.incident_parameter}}</b> {{selectedParameterObject.question}}
											</label>
											<label for="comment" ng-show="typel == 'lang2'">{{lang.u_select}} <br>
												<b> {{lang.incident_category}}</b>{{selectedParameterObject.titlek}} <br>
												<b> {{lang.incident_parameter}}</b> {{selectedParameterObject.questionk}}
											</label>
											<label for="comment" ng-show="typel == 'lang3'">{{lang.u_select}} <br>
												<b> {{lang.incident_category}}</b>{{selectedParameterObject.titlem}} <br>
												<b> {{lang.incident_parameter}}</b> {{selectedParameterObject.questionm}}
											</label>
										</div>
											...
											<table class="details-content" style="border-spacing: 10px; border-collapse: collapse; width: 100%; margin-bottom: 0px; border: 1px solid #dddddd;">
												...
												<tr ng-show="feedback.images && feedback.images.length > 0">
													...
													<td style="border: 1px solid #dddddd; padding: 10px;">
														<div ng-repeat="img in feedback.images track by $index" style="margin-bottom: 5px;">
															<span>attachment_{{ $index + 1 }}.jpg</span>
														</div>
													</td>
												</tr>
