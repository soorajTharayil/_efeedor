  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert("Enter functional gain before discharge (neurorehabilitation).");
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert("Enter total no. of patients undergoing neurorehabilitation");
        return;
      }

      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "Enter functional gain before discharge must be less than total no. of patients undergoing neurorehabilitation."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi42.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
