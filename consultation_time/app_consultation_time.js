...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
        ...
        $http.get($rootScope.baseurl_main + '/audit_load_department.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
			...
			console.log("API Response:", $scope.auditdept);
	...
	$scope.savefeedback = function () {
		...
		if (!$scope.feedback.initial_assessment_hr1 || isNaN(admissionDate.getTime())) {
			alert("Please select valid registration time.");
			return;
		}

		if (!$scope.feedback.initial_assessment_hr2 || isNaN(assessmentDate.getTime())) {
			alert("Please select valid consultation room entry time.");
			return;
		}

		if (assessmentDate <= admissionDate) {
			alert("Consultation room entry time must be after registration time.");
			return;
		}
		...
		$http.post($rootScope.baseurl_main + '/savepatientfeedback_consultation_time.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
