...
$scope.calculateMedicationErrorRate = function () {
    // Get inputs as float (to accept decimal values)
    ...
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter total number of compliance in audited sample size (non-negative).");
        return;
    }

    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter total number of opportunities (non-negative).");
        return;
    }
    ...
    if (opportunitiesForErrors === 0) {
        if (medicationErrors === 0) {
            ...
            alert("Total number of opportunities cannot be 0 when compliance is greater than 0.");
            return;
        }
    } else {
    ...
    // Auto-format: if whole, show integer; otherwise, 2 decimal places
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k8'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k8.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
