  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateNeedleStickInjuryRate = function () {
      ...
      if (parenteralExposures > inpatientDays) {
        alert(
          "The no. of parenteral exposures must be less than the number of in-patient days."
        );
        return;
      }
      ...
      if (parenteralExposures === 0 && inpatientDays === 0) {
        ...
        $scope.calculatedResult = (
          (parenteralExposures / inpatientDays) *
          1000
        ).toFixed(2);
      }
      ...
      console.log(
        "Calculated Needle Stick Injury Rate",
        $scope.calculatedResult
      );
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi30.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
