...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of actual deaths in NICU
    var actualDeaths = parseFloat(document.getElementById('formula_para1').value);

    // Get the number of predicted deaths in NICU
    ...
    if (isNaN(actualDeaths) || actualDeaths < 0) {
        alert("Please enter actual deaths in NICU (cannot be negative)");
        return;
    }

    if (isNaN(predictedDeaths) || predictedDeaths < 0) {
        alert("Please enter predicted deaths in NICU (cannot be negative)");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4h56'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4h56.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
