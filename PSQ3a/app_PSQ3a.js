...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
        ...
        .then(
          function (responsedata) {
            $scope.feedback.initial_assessment_hr =
              responsedata.data.capitalCount;
            $scope.feedback.total_admission =
              responsedata.data.prescriptionsCount;
    ...
    //Calculate  function
    $scope.calculateAdmissionRate = function () {
      // Get the total number of prescriptions in capital letters
      var noOfReadmission = parseInt(
      ...
      // Validate inputs for noOfReadmission and totalAdmissions
      if (isNaN(noOfReadmission) || noOfReadmission < 0) {
        alert("Enter the total number of readmission to ICU within 48 hours.");
        return;
      }

      if (isNaN(totalAdmissions) || totalAdmissions <= 0) {
        alert("Enter the total number of admissions.");
        return;
      }

      if (noOfReadmission > totalAdmissions) {
        alert(
          "The number of readmission to ICU within 48 hours must be less than the total number of admissions."
        );
        return;
      }
      ...
      $scope.calculatedResult = (
        (noOfReadmission / totalAdmissions) *
        100
      ).toFixed(2);
      ...
      console.log("Calculated Compliance Rate", $scope.calculatedResult);
    ...
    $scope.savefeedback = function () {
      ...
      if (
        ...
        alert(
          "No. of readmission to ICU within 48 hours must be less than the total number of admissions."
        );
        return;
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi33.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
