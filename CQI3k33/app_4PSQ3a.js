...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter number of patients surviving in this month last year ");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total number of renal transplant surgery in this month last year ");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter number of patients surviving in this month last year be less than total number of renal transplant surgery in this month last year");
			return;
		}
