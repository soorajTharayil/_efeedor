...
$scope.user_id = ehandor.userid;
    ...
    var patientsSurviving = parseFloat(document.getElementById('formula_para1').value || 0);
    var totalTransplants = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (patientsSurviving < 0) {
        alert("Number of patients surviving in this month last year cannot be negative.");
        return;
    }
    if (totalTransplants < 0) {
        alert("Total number of renal transplant surgeries in this month last year cannot be negative.");
        return;
    }
    ...
    // Handle zero cases
    if (patientsSurviving === 0 || totalTransplants === 0) {
        $scope.calculatedResult = "0";
    } else {
        // Calculate percentage
        var percentage = (patientsSurviving / totalTransplants) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k33'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k33.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
