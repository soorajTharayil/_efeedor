...
$scope.user_id = ehandor.userid;
    ...
    if (medicationErrors < 0) {
        alert("Number of elements cannot be negative");
        return;
    }
    ...
    if (medicationErrors === 0 && opportunitiesForErrors === 0) {
        ...
        errorRatePercentage = opportunitiesForErrors === 0 ? 0 : (medicationErrors / opportunitiesForErrors) * 100;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k22'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k22.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
