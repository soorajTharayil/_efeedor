  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert("Enter no. of POCT tests which led in clinical intervention. ");
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert(
          "Enter no. of POCT tests where clinical intervention was deemed necessary."
        );
        return;
      }
      ...
      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "No. of POCT tests with clinical intervention must be less than tests requiring intervention."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi41.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
