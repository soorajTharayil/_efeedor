...
$scope.user_id = ehandor.userid;
    ...
    // Validate inputs for medication errors
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter total no. of restraint related injuries (cannot be negative)");
        return;
    }

    // Validate inputs for opportunities for errors
    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter total no. of admitted patients under restraint (cannot be negative)");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k40'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k40.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
