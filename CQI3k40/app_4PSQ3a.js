...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter total no. of restraint related injuries");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total no. of admitted patients under restraint");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter total no. of restraint related injuries be less than total no. of admitted patients under restraint");
			return;
		}
