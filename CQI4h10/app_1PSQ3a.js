...
$scope.calculateTimeFormat = function () {
    // Combine entered hours, minutes, and seconds into a readable total (for display)
    ...
    console.log("Entered time:", $scope.feedback.initial_assessment_total);

    // Get values from input fields (allow decimals)
    ...
    if (hoursInput < 0 || minutesInput < 0 || secondsInput < 0) {
        alert("Please enter non-negative time values (hours, minutes, seconds).");
        return;
    }

    if (isNaN(totalAdmissions) || totalAdmissions < 0) {
        alert("Please enter a valid number of admissions (non-negative).");
        return;
    }
    ...
    if (totalAdmissions === 0 && assessmentSeconds > 0) {
        alert("Total admissions cannot be 0 when assessment time is greater than 0.");
        return;
    }

    // ✅ Normal calculation (numerator can be greater)
    var averageSeconds = assessmentSeconds / totalAdmissions;

    // Convert average seconds back into hr:min:sec format
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4h10')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4h10.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
