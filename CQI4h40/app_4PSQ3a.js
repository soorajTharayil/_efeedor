...
$scope.user_id = ehandor.userid;
    ...
    if (medicationErrors < 0) {
        alert("Please enter total number of manpower days in the month (cannot be negative)");
        return;
    }

    if (opportunitiesForErrors < 0) {
        alert("Please enter total number of staff required (cannot be negative)");
        return;
    }

    

    // Handle zero denominator (if both zero, result is 0%)
