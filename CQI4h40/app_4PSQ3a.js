...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter total number of manpower days in the month");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total number of staff required");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter total number of manpower days in the month be less than Total number of staff required ");
			return;
		}
