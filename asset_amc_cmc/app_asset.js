...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.$watch('feedback.amcEndDate', function (newVal) {
		if (newVal instanceof Date && !isNaN(newVal.getTime())) {
			...
			$scope.feedback.setAmcReminderOne = r1;
			$scope.feedback.setAmcReminderTwo = r2;
		}
	});

	$scope.$watch('feedback.cmcEndDate', function (newVal) {
		if (newVal instanceof Date && !isNaN(newVal.getTime())) {
			console.log('Captured Date:', newVal);
			...
			$scope.feedback.setCmcReminderOne = r1;
			$scope.feedback.setCmcReminderTwo = r2;
		}
	});
	...
	$scope.savefeedback = function () {
		...
		$scope.feedback.setAmcReminderOne = formatDateToLocalString($scope.feedback.setAmcReminderOne);
		$scope.feedback.setAmcReminderTwo = formatDateToLocalString($scope.feedback.setAmcReminderTwo);


		$scope.feedback.setCmcReminderOne = formatDateToLocalString($scope.feedback.setCmcReminderOne);
		$scope.feedback.setCmcReminderTwo = formatDateToLocalString($scope.feedback.setCmcReminderTwo);
		...
		$http.post($rootScope.baseurl_main + '/savefeedback_asset_amc_cmc.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
