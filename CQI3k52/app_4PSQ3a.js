...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter total number of sign out completed as per hospital policy ");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total number of sign out parameters audited");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter total number of sign out completed as per hospital policy be less than total number of sign out parameters audited");
			return;
		}
