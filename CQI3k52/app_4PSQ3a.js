...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of sign outs completed
    var medicationErrors = parseFloat(document.getElementById('formula_para1').value || 0);

    // Get the number of sign out parameters audited
    ...
    if (medicationErrors < 0) {
        alert("Please enter a non-negative value for total number of sign out completed as per hospital policy");
        return;
    }

    if (opportunitiesForErrors < 0) {
        alert("Please enter a non-negative value for total number of sign out parameters audited");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k52'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQIk52.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
