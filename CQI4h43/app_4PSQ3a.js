...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of elemented (can be decimal)
    var elemented = parseFloat(document.getElementById('formula_para1').value || 0);

    // Get the number of elements audited (can be decimal)
    var elementsAudited = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (elemented < 0) {
        alert("Number of elemented cannot be negative.");
        return;
    }

    if (elementsAudited < 0) {
        alert("Number of elements audited cannot be negative.");
        return;
    }
    ...
    var errorRatePercentage = elementsAudited === 0 ? 0 : (elemented / elementsAudited) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4h43'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4h43.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
