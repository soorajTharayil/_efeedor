...
$scope.user_id = ehandor.userid;
    ...
    // Get inputs, default to 0 if empty
    var analyzedClosed = parseFloat(document.getElementById('formula_para1').value || 0);
    var reportedErrors = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (analyzedClosed < 0) {
        alert("Please enter No of clinical errors analyzed and closed within the same month (cannot be negative)");
        return;
    }
    if (reportedErrors < 0) {
        alert("Please enter No of clinical errors reporting in the month (cannot be negative)");
        return;
    }

    

    // Handle zero denominator → allowed
    var errorRatePercentage = (analyzedClosed === 0 && reportedErrors === 0) ? 0 : (analyzedClosed / reportedErrors) * 100;

    // Format result: whole number if exact, else 2 decimals
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4b8')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4b8.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
