...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter No of clinical errors analyzed and closed within the same month ");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter No of clinical errors reporting in the month");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("No of clinical errors analyzed and closed within the same month less than No of clinical errors reporting in the month ");
			return;
		}
