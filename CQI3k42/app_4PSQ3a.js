...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of accidental removals of line
    var accidentalRemovals = parseFloat(document.getElementById('formula_para1').value);
    ...
    if (isNaN(accidentalRemovals) || accidentalRemovals < 0) {
        alert("Please enter total number of accidental removal of line (cannot be negative)");
        return;
    }
    ...
    var errorRate;
    if (accidentalRemovals === 0 && dialysisProcedures === 0) {
        errorRate = 0; // both zero → 0%
    } else {
        errorRate = (accidentalRemovals / dialysisProcedures) * 100;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k42'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k42.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
