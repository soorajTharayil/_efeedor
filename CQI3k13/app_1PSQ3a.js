...
$scope.user_id = ehandor.userid;
    ...
    // Get hours, minutes, and seconds from input (default to 0)
    ...
    // Convert total assessment time into seconds
    ...
    if (assessmentSeconds === 0 && totalAdmissions === 0) {
        averageSeconds = 0; // both zero → 0
    } else {
        averageSeconds = totalAdmissions === 0 ? assessmentSeconds : Math.floor(assessmentSeconds / totalAdmissions);
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k13')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k13.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
