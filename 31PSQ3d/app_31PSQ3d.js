...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
        ...
        .then(
          function (responsedata) {
            ...
            $scope.feedback.initial_assessment_hr =
              responsedata.data.handoverCount;
    ...
    //Calculate  function
    $scope.calculateHandoversRate = function () {
      // Get the total number of handovers done appropriately
      var appropriateHandovers = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the total number of handover opportunities
      var handoverOpportunities = parseInt(
      ...
      if (appropriateHandovers > handoverOpportunities) {
        alert(
          "The no. of handovers done appropriately must be less than the total no. of handover opportunities."
        );
        return;
      }
      ...
      if (appropriateHandovers === 0 && handoverOpportunities === 0) {
        ...
        $scope.calculatedResult = (
          (appropriateHandovers / handoverOpportunities) *
          100
        ).toFixed(2);
      }
      ...
      console.log("Calculated Handovers Rate", $scope.calculatedResult);
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi31.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
