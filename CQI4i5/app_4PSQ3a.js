...
$scope.user_id = ehandor.userid;
    ...
    var executedSolidWaste = parseFloat(document.getElementById('formula_para1').value || 0);
    var scheduledSolidWaste = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (executedSolidWaste < 0) {
        alert("Please enter number of executed solid waste collection (cannot be negative)");
        return;
    }

    if (scheduledSolidWaste < 0) {
        alert("Please enter total No. of Scheduled solid waste collection (cannot be negative)");
        return;
    }

    

    // Calculate percentage (if both are zero, result is 0)
    var percentage = (executedSolidWaste === 0 && scheduledSolidWaste === 0)
        ? 0
        : (executedSolidWaste / scheduledSolidWaste) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4i5'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4i5.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
