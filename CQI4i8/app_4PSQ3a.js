...
$scope.user_id = ehandor.userid;
    ...
    var manpowerSupplied = parseFloat(document.getElementById('formula_para1').value || 0);
    var totalManpower = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (manpowerSupplied < 0) {
        alert("Please enter number of manpower supplied (cannot be negative)");
        return;
    }

    if (totalManpower < 0) {
        alert("Please enter total number of manpower contracted (cannot be negative)");
        return;
    }
    ...
    var percentage = (manpowerSupplied === 0 && totalManpower === 0)
        ? 0
        : (manpowerSupplied / totalManpower) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4i8'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4i8.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
