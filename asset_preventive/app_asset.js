...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.$watch('feedback.upcomingMaintenance', function (newVal) {
		if (newVal instanceof Date && !isNaN(newVal.getTime())) {
			console.log('Captured Date:', newVal);

			var r1 = new Date(newVal.getTime());
			r1.setDate(r1.getDate() - 10);

			var r2 = new Date(newVal.getTime());
			r2.setDate(r2.getDate() - 2);

			$scope.feedback.setReminderOne = r1;
			$scope.feedback.setReminderTwo = r2;
		}
	});
	...
	$scope.setupapplication1 = function () {
		...
		$http.get($rootScope.baseurl_main + '/esr_wards.php?patientid=' + userId + '&assetcode=' + encodeURIComponent(assetCode), { timeout: 20000 }).then(function (responsedata) {
			...
			if ($scope.asset_details) {
				console.log("Asset", $scope.asset_details);
				...
				if ($scope.asset_details && $scope.asset_details.preventive_maintenance_date) {
					const rawDate = new Date($scope.asset_details.preventive_maintenance_date);
					$scope.feedback.lastMaintenance = formatDate(rawDate);
					console.log("Prefilled datetime-local value:", $scope.feedback.lastMaintenance);
				}

				$scope.feedback.upcoming_preventive_maintenance_date = formatDate($scope.asset_details.upcoming_preventive_maintenance_date);
	...
	$scope.savefeedback = function () {
		...
		$scope.feedback.setReminderOne = formatDateToLocalString($scope.feedback.setReminderOne);
		$scope.feedback.setReminderTwo = formatDateToLocalString($scope.feedback.setReminderTwo);
		...
		$http.post($rootScope.baseurl_main + '/savefeedback_preventive.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
			$rootScope.loader = false;
