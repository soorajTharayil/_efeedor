...
$scope.user_id = ehandor.userid;
    ...
    // Validate medication errors (allow 0)
    if (medicationErrors < 0 || isNaN(medicationErrors)) {
        alert("Please enter Total number of Contaminated blood culture.");
        return;
    }

    // Validate opportunities for errors (allow 0 if numerator is 0)
    if (opportunitiesForErrors <= 0 && medicationErrors > 0) {
        alert("Please enter Total number of blood culture test Performed");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3a10')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3a10.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
