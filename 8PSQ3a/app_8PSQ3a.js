...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.setupapplication = function () {
      ...
      //alert(id);
      $http
        .get($rootScope.baseurl_main + "/api_8PSQ3a.php?patientid=" + id, {
          timeout: 20000,
        })
        .then(
          function (responsedata) {
            $scope.feedback.initial_assessment_hr =
              responsedata.data.error_prone_abbreviation_count;
            $scope.feedback.total_admission =
              responsedata.data.charts_reviewed_count;
    ...
    //Calculate surgery adherence to prevent function
    $scope.calculateAdherenceToProcedureRate = function () {
      // Get the number of surgeries where the procedure was followed
      var surgeriesWithProcedureFollowed = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the total number of surgeries performed
      var totalSurgeriesPerformed = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for surgeries with procedure followed and total surgeries performed
      if (
        isNaN(surgeriesWithProcedureFollowed) ||
        surgeriesWithProcedureFollowed < 0
      ) {
        alert(
          "Enter the number of surgeries where the procedure was followed."
        );
        return;
      }

      if (isNaN(totalSurgeriesPerformed) || totalSurgeriesPerformed <= 0) {
        alert("Enter the total number of surgeries performed.");
        return;
      }

      if (surgeriesWithProcedureFollowed > totalSurgeriesPerformed) {
        alert(
          "Number of surgeries where the procedure was followed must be less than the number of surgeries performed."
        );
        return;
      }

      // Calculate the percentage of surgeries where the procedure was followed
      var adherenceToProcedureRate =
        (surgeriesWithProcedureFollowed / totalSurgeriesPerformed) * 100;

      // Format the result: if it's a whole number, show it without decimal places; otherwise, use two decimal places
      if (adherenceToProcedureRate % 1 === 0) {
        $scope.calculatedResult = adherenceToProcedureRate.toString();
      } else {
        $scope.calculatedResult = adherenceToProcedureRate.toFixed(2);
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi8.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
