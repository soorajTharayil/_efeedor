...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.$watch('feedback.warrantyEDate', function (newVal) {
	...
	$scope.savefeedback = function () {
		...
		$scope.feedback.warrantySDate = formatDateToLocalString($scope.feedback.warrantySDate);
		$scope.feedback.warrantyEDate = formatDateToLocalString($scope.feedback.warrantyEDate);
		...
		$http.post($rootScope.baseurl_main + '/savefeedback_asset_warranty.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
			$rootScope.loader = false;
