...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTimeFormat = function () {
      // Get hours, minutes, seconds inputs (default to 0 if empty)
      ...
      // Display initial assessment total
      ...
      // Get total admissions (zero allowed)
      ...
      // Calculate average seconds safely (handle zero denominator)
      var averageSeconds =
        totalAdmissions === 0
          ? 0
          : Math.floor(assessmentSeconds / totalAdmissions);

      // Convert average seconds to hours, minutes, seconds
