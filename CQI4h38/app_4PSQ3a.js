  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter total number of variations observed in finished linen in a month"
        );
        return;
      }
      ...
      if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
        alert(
          "Please enter total number of finished linen received for the same month"
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter total number of variations observed in finished linen in a month be less than total number of finished linen received for the same month "
        );
        return;
      }
