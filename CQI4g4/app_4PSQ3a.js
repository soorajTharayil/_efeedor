  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter number of missing records (not delivered 72 hours after request)"
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter number of missing records (not delivered 72 hours after request be less than Total number of medical records"
        );
        return;
      }
