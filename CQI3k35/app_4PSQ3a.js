...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter total number of cases with difference in pre operative vs post operative diagnosis");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total number of surgeries");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter total number of cases with difference in pre operative vs post operative diagnosis be less than total number of surgeries");
			return;
		}
