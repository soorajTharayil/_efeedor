...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of cases with difference in pre vs post operative diagnosis
    var medicationErrors = parseFloat(document.getElementById('formula_para1').value || 0);

    // Get the total number of surgeries
    ...
    if (medicationErrors < 0) {
        alert("Please enter a non-negative value for total number of cases with difference in pre operative vs post operative diagnosis");
        return;
    }

    if (opportunitiesForErrors < 0) {
        alert("Please enter a non-negative value for total number of surgeries");
        return;
    }

    
    // Calculate the rate as percentage
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k35'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k35.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
