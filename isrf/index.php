...
<head>
	...
	<script src="app_esr.js?<?php echo time(); ?>"></script>
</head>
<!-- head part end -->
...
<body ng-app="ehandorApp" ng-controller="PatientFeedbackCtrl" style="display:none;" id="body">
	...
	<nav class="navbar navbar-expand-sm navbar-dark bg-dark" style="height: 60px;">
		...
		<div class="ml-auto d-flex justify-content-between align-items-center w-100">
			<div class="left-buttons d-flex" ng-show="step2 || step3 || step4 || step5">
				<a ng-href="/login/?userid={{ adminId }}" class="btn btn-light mr-3 dashboard-btn" style="display:none; width: 100px; height: 32px; font-size: 14px; font-weight: bold; text-align: left; display: flex; align-items: center; padding-left: 10px;">
					Dashboard
				</a>
			</div>
			...
			<div class="right-buttons d-flex align-items-center">
				<button type="button" class="btn btn-dark language-btn" data-toggle="modal" data-target="#languageModal" style="margin: 4px;" ng-show="step2 || step3 || step4 || step5">
					{{type2}}
					<i class="fa fa-language" aria-hidden="true"></i>
				</button>
				<button class="btn btn-dark menu-toggle" ng-click="menuVisible = !menuVisible" style="margin: 4px; float:right;" ng-show="step2 || step3 || step4 || step5">
						...
						<div class="col-md-12 mx-0">
							...
							<form id="msform">
								...
								<fieldset ng-show="step2 == true">
									<div class="form-card">
										<h3 class="sectiondivision" style="font-weight:bold;" ng-show="feedback.section == 'ISR'">{{lang.pagetitle}}</h3>
										<p style="margin:0px; margin-top:10px;font-size: 16px; margin-left: 6px;">
											<?php
									...
									<div class="col-xs-12 col-sm-12 col-md-12" style="margin-top: -17px;">
										<div class="form-group" style="text-align: left; margin-left: 4px;">
											<span class="addon" style="font-size: 16px;">{{lang.patientid}}<sup style="color:red">*</sup></span>
											<span class="has-float-label">
												<input class="form-control" placeholder="{{lang.enter_patientid}}" maxlength="20" ng-change="fetchAssetDetails()" type="text" id="contactnumber" ng-required="true" ng-model="feedback.assetcode" autocomplete="off" style="padding-top:0px; margin-top: -4px;" />
												<label for="contactnumber"></label>
											</span>
										</div>
									</div>
									<div class="row" ng-if="assetDetails.length > 0" style="margin-top: -18px; margin-left:-22px; width: 100%;">
											...
											<div class="form-group">
												<span class="has-float-label">
													<select class="form-control" ng-model="feedback.ward" ng-change="change_ward()" style=" width: 94%; margin: 0px auto;">
														<option value="" disabled>{{lang.floor}}<sup style="color:red;font-size:15px;">*</sup></option>
														<option ng-repeat="x in wardlist.ward" ng-show="x.title != 'ALL' && x.title != 'Others' " value="{{x.title}}" required>{{x.title}}</option>
													</select>
												</span>
											</div>
										</div>
											...
											<div class="form-group">
												<span class="has-float-label">
													<select class="form-control" ng-model="feedback.bedno" style=" width: 94%; margin: 0px auto;">
														<option value="" disabled>{{lang.location}}<sup style="color:red;font-size:15px;">*</sup></option>
									...
									<br>
									<p style="margin:0px;font-size: 17px; margin-bottom: 22px;">{{lang.gustmessageint}}</p>
									<!-- <h4 style="font-size: 18px; margin-bottom: 22px;"><b>{{lang.chooseCategory}}</b></h4> -->
								...
								<fieldset ng-show="step3 == true">
									<div class="text-left">
										...
										<!-- for radio button ........if u want old flow uncomment above code and in js file also uncomment the code -->
										...
										<div class="text-left" style="width: 94%; margin: 0px auto;" ng-show="selectedQuestionObject.question.length != 0">
											<input type="text" placeholder="{{lang.search_placholder}}" ng-click="filterFunction()" ng-model="searchText" oninput="restrictToAlphabets(event)">
											<i class="fa fa-search search-icon"></i>
								...
								<fieldset ng-show="step4 == true">
									<div class="text-left details-section" style="background: white;">
										...
										<table class="details-content" style="border-spacing: 10px; border-collapse: collapse; width: 100%; margin-bottom: 0px; border: 1px solid #dddddd;">
											...
											<tr>
												<td colspan="2" class="details-label" style="padding: 10px;"><b style="font-size: 15px;">{{lang.recieved}}</b></td>
											</tr>
											<tr>
											...
											<tr>
												<td colspan="2" class="details-label" style="padding: 10px;"><b style="font-size: 15px;">{{lang.raised}}</b></td>
											</tr>
											<tr>
												<td class="details-label" style="border: 1px solid #dddddd; padding: 10px;">{{lang.employeename}}</td>
												<td style="border: 1px solid #dddddd; padding: 10px;">{{loginname}}</td>
											</tr>
											<tr>
											...
											<tr>
												<td colspan="2" class="details-label" style="padding: 10px;"><b style="font-size: 15px;">Asset details</b></td>
											</tr>

											<tr ng-repeat="asset in assetDetails">
												<td class="details-label" style="border: 1px solid #dddddd; padding: 10px;">Asset Code</td>
												<td style="border: 1px solid #dddddd; padding: 10px;">
													<input type="text" ng-model="feedback.assetcode" ng-init="feedback.assetcode = asset.patientid" style="border: none; width: 100%;" readonly />
												</td>
											</tr>

											<tr ng-repeat="asset in assetDetails">
												<td class="details-label" style="border: 1px solid #dddddd; padding: 10px;">Asset Name</td>
												<td style="border: 1px solid #dddddd; padding: 10px;">
													<input type="text" ng-model="feedback.assetname" ng-init="feedback.assetname = asset.assetname" style="border: none; width: 100%;" readonly />
												</td>
											</tr>

											<tr ng-repeat="asset in assetDetails">
												<td class="details-label" style="border: 1px solid #dddddd; padding: 10px;">Allocated Department</td>
												<td style="border: 1px solid #dddddd; padding: 10px;">
													<input type="text" ng-model="feedback.depart" ng-init="feedback.depart = asset.depart" style="border: none; width: 100%;" readonly />
												</td>
											</tr>

											<tr ng-repeat="asset in assetDetails">
												<td class="details-label" style="border: 1px solid #dddddd; padding: 10px;">Allocated User</td>
												<td style="border: 1px solid #dddddd; padding: 10px;">
													<input type="text" ng-model="feedback.assignee" ng-init="feedback.assignee = asset.assignee" style="border: none; width: 100%;" readonly />
												</td>
											</tr>

											<tr ng-repeat="asset in assetDetails">
												<td class="details-label" style="border: 1px solid #dddddd; padding: 10px;" rowspan="1">Location</td>
												<td colspan="2" style="border: 1px solid #dddddd; padding: 10px;">
													<div>
														Area:
														<input type="text" ng-model="feedback.locationsite" ng-init="feedback.locationsite = asset.locationsite" style="border: none; width: auto;" readonly />
													</div>
													<div>
														Site:
														<input type="text" ng-model="feedback.bedsite" ng-init="feedback.bedsite = asset.bedno" style="border: none; width: auto;" readonly />
													</div>
												</td>
											</tr>
										...
										<!-- <p class="details-content">
												<span class="details-label">{{lang.employeename}}:</span> {{feedback.name}}<br>
												<span class="details-label">{{lang.employeeid}}:</span> {{feedback.patientid}}<br>
												<span class="details-label">{{lang.showrole}}:</span> {{feedback.role}}<br>
												<span class="details-label">{{lang.showfloor}}:</span> {{feedback.ward}}<br>
												<span class="details-label">{{lang.showlocation}}:</span> {{feedback.bedno}}<br>
												<span class="details-label">{{lang.showpriority}}:</span> {{feedback.priority}}<br>
										...
										<div class="form-group">
											<label for="comment"><b style="font-size: 15px;margin-left:6px;">{{lang.issue_optional_title}}:</b></label>
										...
										<div class="form-group" style="margin-left: 13px; margin-right:13px;margin-top:30px;">
											...
											<div id="imageEditorModal" style="display:none; position: fixed; top: 10%; left: 10%; background: white; border: 1px solid #ccc; padding: 10px; z-index: 9999;">
												<canvas id="fabricCanvas"></canvas>
												<div style="position: absolute; top: 10px; right: 10px; z-index: 10000;">
													<button class="btn btn-sm btn-warning" ng-click="undoLast()">⟲ Undo</button>
													<button class="btn btn-sm btn-danger" ng-click="closeEditor()">✖ Close</button>
												</div>
											</div>
...
<style>
	...
	@media (min-width: 500px) {
		#imageEditorModal {
			margin-left: 200px;
			/* Add left margin on tablets and desktops */
		}
	}
</style>
...
<div class="modal fade" id="pinModel" tabindex="-1" role="dialog" aria-labelledby="pinModelLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="pinModelLabel">Regenerate PIN</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
<div class="modal fade" id="resendPin" tabindex="-1" role="dialog" aria-labelledby="resentPinLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="resentPinLabel">Resend my PIN</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			...
			<div class="col-xs-12 col-sm-12 col-md-12">
				<div class="form-group">
					<span class="addon" style=" font-size: 16px;">{{lang.forgot_mainresend}}</span>
					<p></p>
					<span class="has-float-label">
						<input class="form-control" placeholder="{{lang.forgot_mainplaceholder}}" maxlength="25" name="name" type="text" id="emailid" ng-model="feedback.resend_contactnumber" autocomplete="on" style=" padding-top:0px;" />
						<label for="emailid"></label>
					</span>
					<span class="has-float-label" ng-show="no_email == true">
						<p id="errorMessage" style="color: red;">{{lang.nodata}}</p>
			...
			<div class="modal-footer">
				<!-- <div ng-show="tagpatient == true" class="d-flex justify-content-between w-100"> -->
				<!-- <button type="button" ng-click="prev_pop()" class="btn btn-light">Back</button> -->
				<button type="button" class="btn btn-dark" ng-click="resend_pin()">{{lang.sentpin}}</button>
				<!-- </div> -->
				<!-- Add additional buttons if needed -->
			</div>
