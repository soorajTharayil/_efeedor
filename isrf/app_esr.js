...
app.controller("PatientFeedbackCtrl", function ($rootScope, $scope, $http, $window, $timeout) {
  ...
  $scope.fetchAssetDetails = function () {
    if ($scope.feedback.assetcode) {
      // Fetch the asset details from your API
      $scope.feedback.patientid = $scope.feedback.assetcode;
      $http.get($rootScope.baseurl_main + '/asset_isr.php?patientid=' + $scope.feedback.patientid, { timeout: 20000 })
        .then(function (response) {
          if (response.data && response.data.length > 0) {
            ...
            if ($scope.assetDetails[0]) {
              $scope.feedback.ward = $scope.assetDetails[0].locationsite;
              $scope.feedback.bedno = $scope.assetDetails[0].bedno;
  ...
  $scope.setupapplication = function () {
    ...
    $http
      .get($rootScope.baseurl_main + "/esr_wards.php?patientid=" + id + '&location=' + encodeURIComponent(location) + '&site=' + encodeURIComponent(site) + '&assetcode=' + encodeURIComponent(assetcode), {
        timeout: 20000,
      })
      .then(
        function (responsedata) {
          if (responsedata.data) {
            ...
            if (location && site) {
              $timeout(function () {
                const match = $scope.wardlist.ward.find(w => w.title === location);
                if (match) {
                  $scope.feedback.ward = match.title;
                  console.log("Prefilled ward with location:", $scope.feedback.ward);

                  // Trigger the change_ward function to populate bed_no
                  $scope.change_ward();

                  $timeout(function () {
                    if ($scope.bed_no.includes(site)) {
                      $scope.feedback.bedno = site;
                    } else {
                      console.warn("Site not found in bed list:", site);
                    }
                  }, 100);
                } else {
                  console.warn("Location from URL not found in ward list:", location);
                }
              }, 100);
            }
            ...
            if (assetcode) {
              ...
              $scope.feedback.assetcode = assetcode;
  ...
  $scope.savefeedback = function () {
    ...
    const feedbackData = {
      assetcode: $scope.feedback.assetcode,
      assetname: $scope.feedback.assetname,
      depart: $scope.feedback.depart,
      assignee: $scope.feedback.assignee,
      locationsite: $scope.feedback.locationsite,
      bedsite: $scope.feedback.bedsite,
    };
