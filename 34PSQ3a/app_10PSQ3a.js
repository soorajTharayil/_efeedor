  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert(
          "Enter no. of AMI patients for whom door-to-balloon time of 90 minutes is achieved."
        );
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert("Enter no. of AMI patients undergoing primary angioplasty.");
        return;
      }

      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "Enter no. of AMI patients with door-to-balloon time within 90 min must be less than total AMI patients undergoing primary angioplasty"
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi35.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
