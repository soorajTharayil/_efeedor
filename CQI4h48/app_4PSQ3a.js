...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter number of patient complaints received in a month ");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total occupancy per month");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter number of patient complaints received in a month less than total occupancy per month");
			return;
		}
