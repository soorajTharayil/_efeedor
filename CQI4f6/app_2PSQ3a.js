...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
	...
	$scope.calculateErrorRate = function () {
		...
		if (isNaN(reportingErrors) || reportingErrors < 0) {
			alert("Please enter number of blood body fluid exposures.");
			return;
		}

		if (isNaN(testsPerformed) || testsPerformed < 0) {
			alert("Please enter total number of out-patient days for the month.");
			return;
		}
		...
		// Format and append unit
		if (errorsPerThousand % 1 === 0) {
			$scope.calculatedResult = errorsPerThousand.toString() + " per 1,000 out-patient days";
		} else {
			$scope.calculatedResult = errorsPerThousand.toFixed(2) + " per 1,000 out-patient days";
		}
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4f6')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4f6.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
