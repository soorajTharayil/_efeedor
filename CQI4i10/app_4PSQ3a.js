...
$scope.user_id = ehandor.userid;
    ...
    var damagedLinen = parseFloat(document.getElementById('formula_para1').value || 0);
    var totalLaundrySent = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (damagedLinen < 0) {
        alert("Please enter total No. of damaged linen identified in the month (cannot be negative)");
        return;
    }

    if (totalLaundrySent < 0) {
        alert("Please enter total No of laundry sent for washing in the month (cannot be negative)");
        return;
    }
    ...
    var percentage = (damagedLinen === 0 && totalLaundrySent === 0)
        ? 0
        : (damagedLinen / totalLaundrySent) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4i10'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4i10.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
