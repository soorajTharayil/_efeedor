...
$scope.user_id = ehandor.userid;
    ...
    // ✅ Validation: check if inputs are valid numbers (not negative)
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter a valid Number of ICU equipment utilised days (non-negative).");
        return;
    }

    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter a valid ICU equipment days available (non-negative).");
        return;
    }

    // ✅ If both are zero, allow calculation (0%)
    ...
    // ✅ Auto-format output: whole number or 2 decimals
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4c6')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4c6.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
