...
$scope.user_id = ehandor.userid;
    ...
    // Get the number of handovers done by nurses
    var medicationErrors = parseFloat(document.getElementById('formula_para1').value);

    // Get the number of handover opportunities
    ...
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter Number of handovers done by nurses");
        return;
    }

    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter Number of handover opportunities");
        return;
    }
    ...
    // ✅ Calculate handover rate percentage
    var errorRatePercentage = (medicationErrors / opportunitiesForErrors) * 100;

    // ✅ Auto-format (whole number → plain, decimal → 2 digits)
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3j3')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3j3.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
