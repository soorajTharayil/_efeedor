...
$scope.calculateErrorRate = function () {
    ...
    if (isNaN(reportingErrors) || reportingErrors < 0) {
        alert("Please enter a number of Urinary Catheter associated UTIs in a month");
        return;
    }

    if (isNaN(testsPerformed) || testsPerformed < 0) {
        alert("Please enter a Number of urinary catheter days in the month");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CLOTCM19')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CLOTCM19.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
