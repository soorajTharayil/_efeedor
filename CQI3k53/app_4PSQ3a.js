...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter donor complications ");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total number of liver donor hepatectomy performed");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert("Please enter donor complications be less than total number of liver donor hepatectomy performed");
			return;
		}
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k53'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k53.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
