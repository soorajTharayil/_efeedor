  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert(
          "Enter the no. of cases where bile duct injuries occurred during laparoscopic cholecystectomy"
        );
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert("Enter the no. of Laparoscopic cholecystectomies performed.");
        return;
      }

      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "No. of cases where bile duct injuries occurred must be less than the no. of Laparoscopic cholecystectomies performed."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi40.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
