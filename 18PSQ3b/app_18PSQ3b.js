...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
        ...
        .then(
          function (responsedata) {
            $scope.feedback.initial_assessment_hr =
              responsedata.data.antibioticCount;
            $scope.feedback.total_admission = responsedata.data.surgicalCount;

            // Logging one of the variables to console for verification
            console.log($scope.feedback.antibioticCount);
          },
    ...
    //Calculate  function
    $scope.calculateProphylacticAntibioticsRate = function () {
      // Get the number of patients who received appropriate prophylactic antibiotics
      var patientsWithProphylacticAntibiotics = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of patients who underwent surgeries in the OT
      var patientsUnderwentSurgeries = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for patientsWithProphylacticAntibiotics and patientsUnderwentSurgeries
      if (
        isNaN(patientsWithProphylacticAntibiotics) ||
        patientsWithProphylacticAntibiotics < 0
      ) {
        alert(
          "Enter the no. of patients who received appropriate prophylactic antibiotics."
        );
        return;
      }

      if (
        isNaN(patientsUnderwentSurgeries) ||
        patientsUnderwentSurgeries <= 0
      ) {
        alert("Enter the no. of patients who underwent surgeries in the OT.");
        return;
      }

      if (patientsWithProphylacticAntibiotics > patientsUnderwentSurgeries) {
        alert(
          "Number of patients who received prophylactic antibiotics must be less than number of patients who underwent surgeries."
        );
        return;
      }
      ...
      if (
        ...
        // Calculate the prophylactic antibiotics rate
        $scope.calculatedResult = (
          (patientsWithProphylacticAntibiotics / patientsUnderwentSurgeries) *
          100
        ).toFixed(2);
      }
      ...
      console.log(
        "Calculated Prophylactic Antibiotics Rate",
        $scope.calculatedResult
      );
    ...
    $scope.savefeedback = function () {
      ...
      if (
        ...
        alert(
          "Number of patients received prophylactic antibiotics must be less than number of patients who underwent surgeries."
        );
        return;
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi18.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
