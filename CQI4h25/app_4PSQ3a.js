...
$scope.calculateMedicationErrorRate = function () {
    ...
    // Case: both zero → allowed (result = 0%)
    if (medicationErrors === 0 && opportunitiesForErrors === 0) {
        errorRatePercentage = 0;
    } 
    // Case: denominator = 0 but numerator > 0 → blocked
    else if (opportunitiesForErrors === 0 && medicationErrors > 0) {
        alert("Total number of observations cannot be zero when Compliance is greater than zero");
        return;
    } 
    // Normal calculation (including numerator > denominator)
    else {
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4h25'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4h25.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
