...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.calculateTimeFormat = function () {
		...
		if (assessmentDate <= admissionDate) {
			alert("Revisit time must be greater than initial consultation time.");
			return;
		}
	...
	$scope.savefeedback = function () {
		...
		function isFeedbackValid() {
			if ($scope.feedback.patientname == '' || $scope.feedback.patientname == undefined) {
		...
		$http.post($rootScope.baseurl_main + '/savepatientfeedback_return_to_ed.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
