...
$scope.calculateErrorRate = function () {
    // Get the number of reporting errors from the input field (accept decimals)
    ...
    // Allow zero or both zero, allow numerator > denominator
    var errorsPerThousand = 0;

    if (testsPerformed === 0 && reportingErrors === 0) {
        // Both zero → 0
        errorsPerThousand = 0;
    } else if (testsPerformed === 0 && reportingErrors > 0) {
        // Denominator zero, numerator positive → calculate directly
    ...
    // Format result — whole number → no decimals, else → 2 decimals
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CLOTCM29')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CLOTCM29.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
