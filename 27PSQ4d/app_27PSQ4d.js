...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculateMockDrillVariationsRate = function () {
      // Get the total number of variations observed in a mock drill
      var variationsObserved = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the total number of opportunities of variations
      var opportunitiesOfVariations = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for variationsObserved and opportunitiesOfVariations
      if (isNaN(variationsObserved) || variationsObserved < 0) {
        alert("Enter the total no. of variations observed in a mock drill.");
        return;
      }

      if (isNaN(opportunitiesOfVariations) || opportunitiesOfVariations < 0) {
        alert("Enter the total no. of opportunities of variations.");
        return;
      }

      if (variationsObserved > opportunitiesOfVariations) {
        alert(
          "The total no. of variations observed must be less than the total no. of opportunities of variations."
        );
        return;
      }

      if (variationsObserved === 0 && opportunitiesOfVariations === 0) {
        $scope.calculatedResult = 0;
      } else {
        // Calculate the percentage of variations observed in the mock drill
        $scope.calculatedResult = (
          (variationsObserved / opportunitiesOfVariations) *
          100
        ).toFixed(2);
      }
      ...
      console.log(
        "Calculated Mock Drill Variations Rate",
        $scope.calculatedResult
      );
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi27.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
