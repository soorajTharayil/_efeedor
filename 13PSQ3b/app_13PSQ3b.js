  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateCAUTIRate = function () {
      ...
      if (isNaN(urinaryCatheterUTIs) || urinaryCatheterUTIs < 0) {
        alert(
          "Enter the number of urinary catheter associated UTIs in a month."
        );
        return;
      }
      ...
      if (urinaryCatheterUTIs > urinaryCatheterDays) {
        alert(
          "Number of urinary catheter associated UTIs must be less than number of urinary catheter days."
        );
        return;
      }
      ...
      if (urinaryCatheterUTIs === 0 && urinaryCatheterDays === 0) {
        ...
        $scope.calculatedResult = Math.round(
          (urinaryCatheterUTIs / urinaryCatheterDays) * 1000
        );
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi13.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
