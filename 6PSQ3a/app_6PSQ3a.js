...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateAdverseDrugReactionRate = function () {
      // Get the number of patients developing adverse drug reactions
      var patientsWithReactions = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of in-patients
      var totalInPatients = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for patients with reactions and total in-patients
      if (isNaN(patientsWithReactions) || patientsWithReactions < 0) {
        alert(
          "Enter the number of patients developing adverse drug reactions."
        );
        return;
      }

      if (isNaN(totalInPatients) || totalInPatients <= 0) {
        alert("Enter the number of in-patients.");
        return;
      }

      if (patientsWithReactions > totalInPatients) {
        alert(
          "Number of patients developing adverse drug reactions must be less than the number of in-patients."
        );
        return;
      }

      // Calculate the percentage of in-patients developing adverse drug reactions
      var adverseDrugReactionRate =
        (patientsWithReactions / totalInPatients) * 100;

      // Format the result: if it's a whole number, show it without decimal places; otherwise, use two decimal places
      if (adverseDrugReactionRate % 1 === 0) {
        $scope.calculatedResult = adverseDrugReactionRate.toString();
      } else {
        $scope.calculatedResult = adverseDrugReactionRate.toFixed(2);
      }
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi6.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
