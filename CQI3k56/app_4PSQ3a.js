  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter number of patients who have withdrawn from all on-going studies"
        );
        return;
      }
      ...
      if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
        alert(
          "Please enter number of patients enrolled in all On-going studies"
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Please enter number of patients who have withdrawn from all on-going studies be less than number of patients enrolled in all On-going studies"
        );
        return;
      }
