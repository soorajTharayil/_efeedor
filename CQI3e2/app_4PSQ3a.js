...
$scope.user_id = ehandor.userid;
    ...
    // Get input values (allow decimals, default to 0)
    var surgeriesRescheduled = parseFloat(document.getElementById('formula_para1').value || 0);
    var surgeriesPlanned = parseFloat(document.getElementById('formula_para2').value || 0);

    // Block negative inputs
    if (surgeriesRescheduled < 0) {
        alert("Please enter number of surgeries rescheduled (cannot be negative).");
        return;
    }
    if (surgeriesPlanned < 0) {
        alert("Please enter number of surgeries planned (cannot be negative).");
        return;
    }
    ...
    // Handle zero cases
    if (surgeriesRescheduled === 0 || surgeriesPlanned === 0) {
        $scope.calculatedResult = "0";
    } else {
        // Calculate percentage
        var percentage = (surgeriesRescheduled / surgeriesPlanned) * 100;
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3e2')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3e2.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
