...
$scope.user_id = ehandor.userid;
    ...
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter Number of reintubation within 48 hours of extubating");
        return;
    }

    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter Number of intubations");
        return;
    }
    ...
    // If denominator is zero but numerator > 0, invalid
    if (opportunitiesForErrors === 0 && medicationErrors > 0) {
        alert("Number of intubations cannot be zero when reintubations exist.");
        return;
    }
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3h7')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3h7.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
