...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    var dischargesDeaths = parseFloat(document.getElementById('formula_para2').value);
    ...
    if (isNaN(dischargesDeaths) || dischargesDeaths < 0) {
        alert("Please enter number of discharges and deaths in that month");
        return;
    }

    // Calculate average safely (ALOS)
    var averageValue = 0;
    if (inPatientDays === 0 && dischargesDeaths === 0) {
        averageValue = 0; // both zero allowed
    } else if (dischargesDeaths === 0) {
        averageValue = inPatientDays; // avoid division by zero
    } else {
        averageValue = inPatientDays / dischargesDeaths;
    }

    // Format result: whole number or two decimals
    $scope.calculatedResult = averageValue % 1 === 0 ?
        averageValue.toString() + " days" :
        averageValue.toFixed(2) + " days";
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CLOTCM3'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CLOTCM3.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
