...
$scope.user_id = ehandor.userid;
	...
	$scope.calculateMedicationErrorRate = function () {
		...
		if (isNaN(medicationErrors) || medicationErrors < 0) {
			alert("Please enter total number of safe and rational prescriptions");
			return;
		}

		if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
			alert("Please enter total number of prescription audited");
			return;
		}

		if (medicationErrors > opportunitiesForErrors) {
			alert( "Total number of safe and rational prescriptions than total number of prescription audited ");
			return;
		}
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		if ($scope.feedback.initial_assessment_hr > $scope.feedback.total_admission) {
			alert('Total number of safe and rational prescriptions less than  total number of prescription audited');
			return false;
		}

		// First check for duplicates
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3d4')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3d4.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
