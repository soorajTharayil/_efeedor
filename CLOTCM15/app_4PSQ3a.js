...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    // Get input values (allow decimal)
    ...
    // Validation: must be a number (including zero)
    ...
    // Calculation: handle division by zero safely
    var result = 0;
    if (opportunitiesForErrors !== 0) {
        result = medicationErrors / opportunitiesForErrors;
    } else {
        // if denominator is zero but numerator > 0 → result = numerator
        result = medicationErrors;
    }

    // Format result
    if (result % 1 === 0) {
        $scope.calculatedResult = result.toString();
    } else {
        $scope.calculatedResult = result.toFixed(2);
    }
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CLOTCM15'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CLOTCM15.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
