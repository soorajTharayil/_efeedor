...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_7PSQ3a.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
	...
	//Calculate unplanned return to OT function
	$scope.calculateUnplannedReturnToOTRate = function () {
		// Get the number of unplanned returns to OT
		var unplannedReturnsToOT = parseInt(document.getElementById('formula_para1').value);

		// Get the number of patients who underwent surgeries in the OT
		var totalSurgeriesInOT = parseInt(document.getElementById('formula_para2').value);

		// Validate inputs for unplanned returns to OT and total surgeries in the OT
		if (isNaN(unplannedReturnsToOT) || unplannedReturnsToOT < 0) {
			alert("Enter the number of unplanned returns to the OT.");
			return;
		}

		if (isNaN(totalSurgeriesInOT) || totalSurgeriesInOT <= 0) {
			alert("Enter the number of patients who underwent surgeries in the OT.");
			return;
		}

		if (unplannedReturnsToOT > totalSurgeriesInOT) {
			alert("Number of unplanned returns to the OT must be less than the number of surgeries performed.");
			return;
		}

		// Calculate the percentage of unplanned returns to OT
		var unplannedReturnToOTRate = (unplannedReturnsToOT / totalSurgeriesInOT) * 100;

		// Format the result: if it's a whole number, show it without decimal places; otherwise, use two decimal places
		if (unplannedReturnToOTRate % 1 === 0) {
			$scope.calculatedResult = unplannedReturnToOTRate.toString();
		} else {
			$scope.calculatedResult = unplannedReturnToOTRate.toFixed(2);
		}
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_7PSQ3a')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi7.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
