  ...
  function ($rootScope, $scope, $http, $location, $window) {
        ...
        .then(
          function (responsedata) {
            $scope.feedback.initial_assessment_hr =
              responsedata.data.errors_count;
    ...
    $scope.calculateUnplannedReturnToOTRate = function () {
      ...
      if (isNaN(totalSurgeriesInOT) || totalSurgeriesInOT <= 0) {
        alert(
          "Enter the number of patients who underwent surgeries in the OT."
        );
        return;
      }
      ...
      if (unplannedReturnsToOT > totalSurgeriesInOT) {
        alert(
          "Number of unplanned returns to the OT must be less than the number of surgeries performed."
        );
        return;
      }

      // Calculate the percentage of unplanned returns to OT
      var unplannedReturnToOTRate =
        (unplannedReturnsToOT / totalSurgeriesInOT) * 100;
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi7.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
