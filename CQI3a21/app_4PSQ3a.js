  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert(
          "Please enter Total no of critical results the physician acted/ responded with the specified timeframe as per policy "
        );
        return;
      }
      ...
      if (isNaN(opportunitiesForErrors) || opportunitiesForErrors <= 0) {
        alert(
          "Please enter Total No of critical results reported in the month."
        );
        return;
      }
      ...
      if (medicationErrors > opportunitiesForErrors) {
        alert(
          "Total no of critical results the physician acted/ responded with the specified timeframe as per policy less than Total No of critical results reported in the month."
        );
        return;
      }
