  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateICUSMR = function () {
      ...
      if (isNaN(actualDeathsInICU) || actualDeathsInICU < 0) {
        alert(
          "Enter no. of stroke patients in whom the DTN of 60 minutes is achieved."
        );
        return;
      }

      if (isNaN(predictedDeathsInICU) || predictedDeathsInICU <= 0) {
        alert("Enter no. of stroke patients who receive thrombolytic therapy.");
        return;
      }

      if (actualDeathsInICU > predictedDeathsInICU) {
        alert(
          "Enter no. of stroke patients with DTN of 60 minutes must be less than the total stroke patients who receive thrombolytic therapy."
        );
        return;
      }
    ...
    $scope.savefeedback = function () {
      ...
      $http
        .post(
          $rootScope.baseurl_main +
            "/savepatientfeedback_kpi45.php?patient_id=" +
            $rootScope.patientid +
            "&administratorId=" +
            $rootScope.adminId +
            "&month=" +
