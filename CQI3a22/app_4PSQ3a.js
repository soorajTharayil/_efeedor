  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      if (medicationErrors < 0) {
        alert(
          "Negative values are not allowed for Number of Compliances in Parameters."
        );
        return;
      }
      ...
      if (opportunitiesForErrors < 0) {
        alert(
          "Negative values are not allowed for Total number of Patients admitted in the Holding area in a month."
        );
        return;
      }

      // Zero or both zero → allowed
      if (opportunitiesForErrors === 0) {
        ...
        if (medicationErrors === 0) {
          ...
          alert(
            "Total number of Patients admitted cannot be zero unless both values are zero."
          );
          return;
        }
      }
      ...
      // Format result: whole → as-is, decimal → up to 2 places
