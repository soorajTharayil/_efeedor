...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      // Get the number of babies re-admitted (allow decimals)
      ...
      // Prevent division by zero when denominator is zero but numerator > 0
      if (opportunitiesForErrors === 0 && medicationErrors > 0) {
        alert(
          "Total number of babies admitted in NICU cannot be zero when re-admissions exist."
        );
        return;
      }
