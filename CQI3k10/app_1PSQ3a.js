...
$scope.user_id = ehandor.userid;
    ...
    // Convert total assessment time to seconds
    var assessmentSeconds = Math.round(hoursInput * 3600 + minutesInput * 60 + secondsInput);

    // Get total admissions
    var totalAdmissions = parseInt(document.getElementById('formula_para2').value || 0);

    // Validate total admissions (must be > 0)
    if (isNaN(totalAdmissions) || totalAdmissions <= 0) {
        alert("Please enter total admissions to calculate average time");
        return;
    }

    // Calculate average time per admission
    var averageSeconds = Math.floor(assessmentSeconds / totalAdmissions);

    // Convert average seconds back to hours, minutes, seconds
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3k10')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3k10.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
