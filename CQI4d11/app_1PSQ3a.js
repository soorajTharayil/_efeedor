...
$scope.user_id = ehandor.userid;
    ...
    // Combine HR:MIN:SEC for display
    ...
    // Get input values (accept decimals too)
    ...
    // ✅ Both zero → allowed, output 0:00:00
    if ((hoursInput === 0 && minutesInput === 0 && secondsInput === 0 && totalAdmissions === 0)) {
    ...
    // ✅ Format neatly with leading zeros
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4d11')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4d11.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears, $scope.feedback)
