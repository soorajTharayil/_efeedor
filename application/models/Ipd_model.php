...
// asort($data); 
	...
	public function get_feedback_rows() {
		// Retrieve the login email from the session
		$login_email = $this->session->userdata('email');
		
		// Query to get all rows with matching login email
		$this->db->select('*');
		...
		return []; // Return an empty array if no matching rows are found
	}
	...
	public function psat_analytics($table_patients, $table_feedback, $table_tickets, $sorttime)
	{
		...
		$unsatisfied_count = 0;
		$neutral_count = 0;
        $satisfied_feedback = array();
        $unsatisfied_feedback = array();
