...
class Ticketsgrievance_model extends CI_Model
{

	private $table = 'tickets_grievance';
	...
	public function addressedtickets()
	{
		...
		foreach ($result as $row) {
			...
			if (isset($_SESSION['ward']) && $_SESSION['ward'] != 'ALL') {
				$this->db->where('bf_feedback_grievance.ward', $_SESSION['ward']);
			}elseif (count($this->session->userdata['floor_ward_esr']) > 0) {
				$floorwiseArray = $this->session->userdata['floor_ward_esr'];
				$this->db->where_in('bf_feedback_grievance.ward', $floorwiseArray);
			}
