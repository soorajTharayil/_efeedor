...
class Asset_model extends CI_Model
{
	...
	public function update_bf_feedback_asset_creation($id, $data)
	{
		$this->db->where('id', $id);
		$this->db->update('bf_feedback_asset_creation', $data);
	}


	public function get_asset_by_code($assetcode)
	{
		return $this->db
			->select('id')
			->from('bf_feedback_asset_creation')
			->where('patientid', $assetcode)
			->get()
			->row();
	}
	...
	public function key_highlights2($table_patients, $table_feedback, $sorttime, $setup)
	{
		...
		$feedback_data = $this->patient_and_feedback($table_feedback, $sorttime, $setup);
		$i = 0;
		$z = 0;
		$set = array();
		$set1 = array();
	...
	//keep
	public function patient_and_feedback($table_feedback, $sorttime)
	{
		$fdate = date('Y-m-d', strtotime($_SESSION['from_date']));
		$tdate = date('Y-m-d', strtotime($_SESSION['to_date']));
		$this->db->select($table_feedback . '.*');
	...
	public function psat_analytics($table_patients, $table_feedback, $table_tickets, $sorttime)
	{
		...
		$feedback_data = $this->patient_and_feedback($table_feedback, $sorttime);
		$satisfied_count = 0;
		$unsatisfied_count = 0;
		$neutral_count = 0;
