<?php defined('BASEPATH') or exit('No direct script access allowed');
	...
	class Ticketspdf_model extends CI_Model
	{

		private $table = 'tickets_pdf';
		...
		public function editcapa($data = [])
		{
			return $this->db->where('id', $data['id'])
				->update('ticket_pdf_message', $data);
		}
		...
		public function addressedtickets()
		{
			...
			foreach ($result as $row) {
				...
				if (isset($_SESSION['ward']) && $_SESSION['ward'] != 'ALL') {
					...
					$this->db->where('bf_feedback_pdf.ward', $_SESSION['ward']);
					...
					$this->db->where_in('bf_feedback_pdf.ward', $floorwiseArray);
				}
