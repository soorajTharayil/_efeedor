<?php defined('BASEPATH') or exit('No direct script access allowed');
	...
	class Ticketsadf_model extends CI_Model
	{

		private $table = 'tickets_adf';
		...
		public function editcapa($data = [])
		{
			return $this->db->where('id', $data['id'])
				->update('ticket_message_adf', $data);
		}
		...
		public function addressedtickets()
		{
			...
			foreach ($result as $row) {
				...
				if (isset($_SESSION['ward']) && $_SESSION['ward'] != 'ALL') {
					$this->db->where('bf_feedback_adf.ward', $_SESSION['ward']);
				}
				...
				if ($pos !== false && count($this->session->userdata['department_access'][$department[0]->type]) > 0) {
					...
					if (!isset($dataset[$i]->patinet) || $dataset[$i]->patinet === null) {
		...
		public function read()
		{
			...
			foreach ($result as $row) {
				...
				if ($pos !== false && count($this->session->userdata['department_access'][$department[0]->type]) > 0) {
					...
					// 					$reasonsarray[$i]= $feed['reason'];
					// 					print_r($reasonsarray);
					// if($reasonsarray){

					// echo 'hi';
					// 	// $dataset[$i]->patinet->reason= $feed['reason'];
					// }
					$dataset[$i]->department = $department[0];
					if ($row->rating == 2) {
						$dataset[$i]->ratingt = 'Poor';
