...
class Ipd_opt_model extends CI_Model
{
...
// asort($data); 
	...
	//keep
	public function ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action,$alltickets)
	{
	...
	//keep
	public function ticket_resolution_rate($table_tickets, $status, $table_feedback, $alltickets)
    {
        // Filter tickets with the specified status
        $filteredTickets = array_filter($alltickets, function($t) use ($status) {
            return $t->status === $status;
        });
        
        // Calculate total tickets and tickets with the given status
        $totalTickets = count($alltickets);
        $statusTickets = count($filteredTickets);
    
        // Calculate and return the resolution rate
        return $totalTickets > 0 ? round(($statusTickets / $totalTickets) * 100) : 0;
    }
	...
	public function get_tickets($table_feedback, $table_tickets)
	{
		...
		if (isset($_SESSION['ward']) && $_SESSION['ward'] != 'ALL') {
			// Use ward only if floorwise doesn't exist
			$this->db->where($table_feedback . '.ward', $_SESSION['ward']);
		} elseif ($this->session->userdata['floor_ward'] != null && count($this->session->userdata['floor_ward']) > 0) {
			$floorwiseArray = $this->session->userdata['floor_ward'];
			$this->db->where_in($table_feedback . '.ward', $floorwiseArray);
		}
	...
	//keep
	public function nps_analytics($table_feedback, $sorttime,$totalFeedbackList)
	{
