<div class="content">
	...
	?>
		...
		<div class="row">
			...
			// ✅ Move date filter INTO the LEFT JOIN
			$this->db->join(
				'bf_outfeedback bf',
			...
			echo '<th colspan="2" class="text-left h5" style="background: #dadada;">
                  <strong>Feedbacks by Speciality</strong>
                <span data-toggle="tooltip" title="This analysis showcases the number of feedbacks received from various speciality within the hospital premises.">
                  <i class="fa fa-info-circle" style="cursor: pointer; color: green;"></i>
                </span>
              </th>';
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading" style="text-align: right;">
						<div class="btn-group">
							<a class="btn btn-success" data-placement="bottom" data-toggle="tooltip" title="<?php echo lang_loader('op', 'op_download_total_feedback_tooltip'); ?>" href="<?php echo base_url($this->uri->segment(1)) . '/overall_patient_excel' ?>">
								<i class="fa fa-download"></i>
							</a>
						</div>
					</div>
					<div class="panel-body">
						<table class="opallfeedbacks table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								<th><?php echo lang_loader('op', 'op_slno'); ?></th>
								<th><?php echo lang_loader('op', 'op_date'); ?></th>
								<?php if (allfeedbacks_page('feedback_id') == true) { ?>
									<th style="white-space: nowrap;"><?php echo lang_loader('op', 'op_feedback_id'); ?></th>
								<?php } ?>

								<th style="white-space: nowrap;"><?php echo lang_loader('op', 'op_patient_details'); ?></th>

								<?php if (allfeedbacks_page('avg_rating') == true) { ?>
									<th><?php echo lang_loader('op', 'op_average_rating'); ?></th>
								<?php } ?>
								<?php if (allfeedbacks_page('psat_score') == true || allfeedbacks_page('nps_score') == true) { ?>
									<th><?php echo lang_loader('op', 'op_psat'); ?> / <?php echo lang_loader('op', 'op_nps'); ?></th>
								<?php } ?>
								...
								<?php if (allfeedbacks_page('nps_comment') == true) { ?>
									<th><?php echo lang_loader('op', 'op_nps'); ?></th>
								<?php } ?>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? "odd gradeX" : "even gradeC" ?>" onclick="window.location.href='<?php echo $ip_link_patient_feedback . $id; ?>';" style="cursor: pointer;">
										...
										<?php if (allfeedbacks_page('feedback_id') == true) { ?>
											<td>
												<a href="<?php echo  $ip_link_patient_feedback . $id; ?>"><?php echo lang_loader('op', 'op_opf'); ?><?php echo $id; ?></a>
											</td>
										<?php } ?>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl + "/analytics_op/resposnsechart"; // Replace with your API endpoint
