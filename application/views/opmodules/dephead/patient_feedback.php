					...
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('op', 'op_discharge_feedback_id_tooltip'); ?>"> <i class="fa fa-question-circle" aria-hidden="true"></i></a> <?php echo lang_loader('op', 'op_opf'); ?> <?php echo $result->id; ?> </h3>
							</div>
							<div class="panel-body" style="background: #fff;">
								...
								<table class=" table table-striped table-bordered  no-footer dtr-inline ">
									<tr>
										<td><b><?php echo lang_loader('op', 'op_patient_name'); ?></b></td>
										<td><?php echo $param['name']; ?></td>
									</tr>
									<tr>
										<td><b><?php echo lang_loader('op', 'op_patient_id'); ?></b></td>
										<td><?php echo $param['patientid']; ?></td>
									</tr>
									<?php if ($param['ward'] != '') { ?>
										<tr>
											<td><b><?php echo lang_loader('op', 'op_speciality'); ?></b></td>
											<td><?php echo ($param['ward']); ?></td>
										</tr>
									<?php } ?>
									<?php if (($param['bedno']) && $param['bedno'] != '-') { ?>
										<tr>
											<td><b><?php echo lang_loader('op', 'op_room_bedno'); ?></b></td>
											<td><?php echo $param['bedno']; ?></td>
										</tr>
									<?php } ?>
									<?php if ($param['email'] != '') { ?>
										<tr>
											<td><b><?php echo lang_loader('op', 'op_email_id'); ?></b></td>
											<td><?php echo $param['email']; ?></td>
										</tr>
									<?php } ?>
									<?php if ($param['contactnumber'] != '') { ?>
										<tr>
											<td><b><?php echo lang_loader('op', 'op_contact_number'); ?></b></td>
											<td><?php echo $param['contactnumber']; ?></td>
										</tr>
									<?php } ?>
									<tr>
										<td><b><?php echo lang_loader('op', 'op_date_time'); ?></b></td>
									...
									<?php if ($param['admissiondate'] != '') { ?>
										<tr>
											<td><b><?php echo lang_loader('op', 'op_admission_date'); ?></b></td>
											<td><?php echo date('d-M-Y', strtotime($param['admissiondate'])); ?></td>
										</tr>
									<?php } ?>
									<?php if ($result->source != '') { ?>
										<tr>
											<td><b><?php echo lang_loader('op', 'op_source'); ?></b></td>
								...
								if ($this->session->user_role == 4) {
									...
									$this->db->join('setupop', 'setupop.shortkey = department.slug');
									// $this->db->where('department.email', $email);
									$this->db->where('setupop.parent', 1);
									$this->db->where('department.type', 'outpatient');
									$query = $this->db->get();
									...
									$sresult = $this->opf_model->setup_result('setupop');
								}
								...
								<div style="padding:10px; background:#fff;">
									...
									<ul class="feedback">
										...
										<li>
											<?php if (is_mobile() === true) { ?>
												<h6><?php echo lang_loader('op', 'op_average_rating'); ?></h6>
											<? } else { ?>
												<h4> <?php echo lang_loader('op', 'op_average_rating'); ?></h4>
											<?php } ?>
										</li>
										...
										<li>
											<?php if (is_mobile() === true) { ?>
												<h6><?php echo lang_loader('op', 'op_netpromoter_analysis'); ?></h6>
											<? } else { ?>
												<h4><?php echo lang_loader('op', 'op_netpromoter_analysis'); ?></h4>
											<?php } ?>
										</li>
										<li id="feedback">
											<?php if (is_mobile() === true) { ?>
												<h6><?php echo lang_loader('op', 'op_promoters_tooltip'); ?>, <?php echo lang_loader('op', 'op_promoters_tooltip_r'); ?></h6>
											<? } else { ?>
												<h4><?php echo lang_loader('op', 'op_promoters_tooltip'); ?>, <?php echo lang_loader('op', 'op_promoters_tooltip_r'); ?></h4>
											<?php } ?>
											<p><?php $netpromoterscore = $param['recommend1Score'];
												...
												<?php if ($param['recommend1Score'] > 4) { ?>
													<span class="text-success"> <b> <?php echo lang_loader('op', 'op_promoters'); ?></b></span>
												<?php 	} elseif ($param['recommend1Score'] < 3.5) { ?>
													<span class="text-danger"><b><?php echo lang_loader('op', 'op_detractors'); ?></b></span>
												<?php } else { ?>
													<span class="text-warning"><b><?php echo lang_loader('op', 'op_passives'); ?></b></span>
												<?php	} ?>
											</p>
											<p style="overflow: clip; word-break: break-all;">
												<?php if ((ismodule_active('nps_promoter_comment') === true) || (ismodule_active('nps_passive_comment') === true) || (ismodule_active('nps_promoter_comment') === true)) { ?>
													<?php if (ismodule_active('nps_detractor_comment') === true && $param['detractorcomment']) { ?>
														<b><?php echo lang_loader('op', 'op_nps_comment'); ?></b>
										...
										<?php if ($this->session->user_role <= 3) { ?>
											<?php if ((($param['location'] == 1)) || ($param['specificservice'] == 1) || ($param['referred'] == 1) || ($param['friend'] == 1) || ($param['previous'] == 1) ||  ($param['docAvailability'] == 1) || ($param['companyRecommend'] == 1) || ($param['otherReason'] == 1)) { ?>
												<li>
													...
													<?php if (is_mobile() === true) { ?>
														<h6><?php echo lang_loader('op', 'op_close_hospital'); ?></h6>
													<? } else { ?>
														<h4><?php echo lang_loader('op', 'op_close_hospital'); ?></h4>
													<?php } ?>
												</li>
												<p>
													<?php if ($param['location'] == 1) { ?><li id="feedback"><?php echo lang_loader('op', 'op_location_proximity'); ?></li><?php } ?>
													<?php if ($param['specificservice'] == 1) { ?><li id="feedback"><?php echo lang_loader('op', 'op_specific_service'); ?> </li><?php } ?>
													<?php if ($param['referred'] == 1) { ?><li id="feedback"><?php echo lang_loader('op', 'op_referred_by_doctors'); ?> </li><?php } ?>
													<?php if ($param['friend'] == 1) { ?><li id="feedback"><?php echo lang_loader('op', 'op_ff_rocommendation'); ?> </li><?php } ?>
													<?php if ($param['previous'] == 1) { ?><li id="feedback"><?php echo lang_loader('op', 'op_previous_experience'); ?> </li><?php } ?>
													<?php if ($param['docAvailability'] == 1) { ?><li id="feedback"><?php echo lang_loader('op', 'op_insurance_facilities'); ?></li><?php } ?>
													<?php if ($param['companyRecommend'] == 1) { ?><li id="feedback"><?php echo lang_loader('op', 'op_company_recommendation'); ?></li><?php } ?>
													<?php if ($param['otherReason'] == 1) { ?><li id="feedback"><?php echo lang_loader('op', 'op_print_online'); ?></li><?php } ?>
												</p>
											<?php } ?>
											<?php if ($param['staffname'] == "" || $param['staffname'] == NULL) {
												...
												<li>
													<?php if (is_mobile() === true) { ?>
														<h6><?php echo lang_loader('op', 'op_recognized_staff'); ?></h6>
													<? } else { ?>
														<h4><?php echo lang_loader('op', 'op_recognized_staff'); ?></h4>
													<?php } ?>

												<li id="feedback">
													<b><?php echo lang_loader('op', 'op_staff_name'); ?></b><br>
										...
										<li>
											<?php if (is_mobile() === true) { ?>
												<h6><?php echo lang_loader('op', 'op_feedback_suggestions'); ?></h6>
											<? } else { ?>
												<h4><?php echo lang_loader('op', 'op_feedback_suggestions'); ?></h4>
											<?php } ?>
										</li>
										<li id="feedback" style="overflow: clip; word-break: break-all;">
		...
		<?php } else {  ?>
			<div class="row">
				<div class="col-sm-12">
					<div class="panel panel-default ">
						<div class="panel-heading">
							<h3 style="text-align: center; color:tomato;"><?php echo lang_loader('op', 'op_no_record_found'); ?> <br>
			...
			<div class="col-sm-12">
				<div class="panel panel-default ">
					<div class="panel-heading">
						...
						<table class="table">
							<tr>
								<th class="" style="border:none !important;vertical-align: middle; text-align:right;"><?php echo lang_loader('op', 'op_feedback_id'); ?></th>
