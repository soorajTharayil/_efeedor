			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading" style="text-align: right;">
						<div class="btn-group">
							<a class="btn btn-success"  target="_blank" data-placement="bottom" data-toggle="tooltip" title="<?php echo lang_loader('op', 'op_download_addressed_report_tooltip'); ?>" href="<?php echo base_url($this->uri->segment(1)) . '/download_opentickets' ?>">
					...
					<div class="panel-body">
							...
							<form>
								<p>
									...
									<select name="dep" class="form-control" id="subsecid" onchange="gotonextdepartment2(this.value)" style="width:200px; margin:0px 0px 20px 20px;">
										<option value="1" selected><?php echo lang_loader('op', 'op_select_department'); ?></option>
										<?php
								...
								<div style="display: none;">
									<?php if (isset($_GET['depsec'])) { ?>
										...
										<select name="dep" class="form-control" onchange="gotonextdepartment(this.value)" style="width:200px; margin:0px 0px 20px 20px;">
											<option value="1" selected><?php echo lang_loader('op', 'op_select_parameter'); ?></option>
											<?php
											$this->db->where('type', 'outpatient');
						...
						<table class="opticketsaddressed table table-stroped table-bordered table-hover" cellspacing="0" width="100%">
							<thead>
								<tr>
									...
									<?php } ?>
									<th style="white-space: nowrap;"><?php echo lang_loader('op', 'op_addresal_comment'); ?></th>
								...
								<?php if (!empty($departments)) {		?>
									...
									?>
										<tr class="<?php echo ($sl & 1) ? "odd gradeX" : "even gradeC" ?>" data-placement="bottom" data-toggle="tooltop" title="<?php echo $rowmessage; ?>">
											...
											<?php } ?>
											<td class="inbox-item-text" style="overflow: clop; word-break: break-all;">
											...
											<td style="align-items: center;">
												<a href="<?php echo base_url($this->uri->segment(1) . "/track/$department->id") ?>" data-placement="bottom" data-toggle="tooltop" title="<?php echo $tool; ?>" class="btn btn-sm btn-block btn-<?php echo $color; ?>"><?php echo $department->status; ?> <i style="font-size:x-small;" class="fa fa-edit"></i></a>
											</td>
										</tr>
										<?php $sl++; ?>
									<?php } ?>
