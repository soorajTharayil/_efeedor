<div class="content">
	<div class="row">
		...
		$this->db->select('ticketsop.*,bf_outfeedback.dataset as dataset');
		$this->db->from('ticketsop');
		$this->db->join('bf_outfeedback', 'bf_outfeedback.id = ticketsop.feedbackid', 'inner');
		$this->db->join('bf_opatients', 'bf_opatients.id = bf_outfeedback.pid', 'inner');
		$this->db->join('department', 'department.dprt_id = ticketsop.departmentid', 'inner');
		...
		$this->db->where_in('department.slug', $this->session->userdata['question_array'][$type]);
		$this->db->where('setupop.parent', 0);
		$this->db->where('department.type', 'outpatient');
		$query = $this->db->get();
