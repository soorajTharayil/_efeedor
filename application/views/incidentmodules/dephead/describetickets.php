								...
								<?php if (!empty($departments)) {		?>
									...
									<?php foreach ($departments as $department) {
										if ($department->status == 'Described') {
											$this->db->where('ticketid', $department->id)->where('ticket_status', 'Described');
											$query = $this->db->get('ticket_incident_message');
											$ticket = $query->result();
											$rowmessage = 'Incident explained by ' . $ticket[0]->message;
									...
									?>
										<tr class="<?php echo ($sl & 1) ? "odd gradeX" : "even gradeC" ?>" data-placement="bottom" data-toggle="tooltip" title="<?php echo $rowmessage; ?>">
											...
											<td style="overflow: clip; word-break: break-all;"><?php echo $department->feed->name; ?>&nbsp;(<a href="<?php echo $ip_link_patient_feedback . $department->id; ?>"><?php echo $department->feed->patientid; ?></a>)
												<!-- <br>
											...
											if ($department->status == 'Described') {
												...
												$style = 'background-color:#3f1670 !important; color:white !important; font-size:14px !important; border:none !important;';
											} else {
												$tool = '';
												$style = '';
											}
...
<style>
	.btn-reddd {
		background-color: #3f1670 !important;
...
<script>
	function gotonextdepartment(type) {
		...
		var url = "<?php echo base_url($this->uri->segment(1) . "/describetickets?type=") ?>" + type + "&depsec=" + subsecid;
		window.location.href = url;
	}

	function gotonextdepartment2(type) {
		var url = "<?php echo base_url($this->uri->segment(1) . "/describetickets?depsec=") ?>" + type;
		window.location.href = url;
	}
</script>