						...
						<table class="incticketsaddressed table table-striped table-bordered table-hover" cellspacing="0" width="100%">
							<thead>
								<tr>
									...
									<?php } ?>
									<th style="white-space: nowrap;">Rejected comment</th>
								...
								<?php if (!empty($departments)) {		?>
									...
									<?php foreach ($departments as $department) {
										if ($department->status == 'Rejected') {
											$this->db->where('ticketid', $department->id)->where('ticket_status', 'Rejected');
											$query = $this->db->get('ticket_incident_message');
											$ticket = $query->result();
											$rowmessage = 'Rejected by ' . $ticket[0]->message;
