             ...
             <table class="table table-filter" style="width: 100%; margin-left: 53px;margin-right: -53px; padding:10px; vertical-align:middle; ">
                 <tr>
                     <?php if ($this->session->userdata['user_role'] >= 0) { ?>
                         ...
                         <?php   }  ?>
                         <?php if ($this->uri->segment(1) != 'patient' && $this->uri->segment(1) != 'asset') {  ?>
                             <td style="white-space: nowrap;">
                                 <span style="margin: 0px 0px 0px 0px; font-size: 17px; ">
                                     <strong>
                                         <strong><?php echo lang_loader('global', 'global_period'); ?>:</strong>
                                     </strong>
                                 </span>
                             </td>
                                 ...
                                 <div class="btn-group" style="margin: 0px 20px 5px 0px;">
                                     ...
                                     <div class="dropdown-menu" style="width: 100%;">
                                         ...
                                         <?php if (!in_array($this->uri->segment(1), ['audit', 'quality'])): ?>
                                             <a href="<?php echo base_url(); ?><?php echo $this->uri->segment(1); ?>/<?php echo $this->uri->segment(2); ?>/<?php echo $this->uri->segment(3); ?>?today=1&tdate=<?php echo date('d-m-Y'); ?>&fdate=<?php echo date('d-m-Y'); ?>" style="width:100%">
                                                 <button class="btn btn-default" style="width:100%; border:0px; border-bottom:1px solid #ccc;">
                                                     <?php echo lang_loader('global', 'global_last_24_hours'); ?>
                                                 </button>
                                             </a>
                                         <?php endif; ?>
                         ...
                         <?php if (ismodule_active('OP') === true && $this->uri->segment(1) == 'opf') {  ?>
                             <script>
                                 function changefloore(cward) {
                                     window.location = '<?php echo base_url(uri_string()); ?>?cward=' + encodeURIComponent(cward);
                                 }
                             </script>
                             <td style="white-space: nowrap;">
                                 <span style="margin: 0px 0px 0px 0px; font-size: 17px; ">
                             ...
                             <td>
                                 <div class="btn-group" style="margin: 0px 20px 5px 0px;">
                                     <button type="button" style="background:#62c52d;border:none;width:200px"
                                         class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                         <span style="display:inline-block; max-width:160px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; vertical-align:middle;"
                                             title="<?php echo htmlspecialchars($_SESSION['ward'], ENT_QUOTES, 'UTF-8'); ?>">
                                             <?php echo htmlspecialchars($_SESSION['ward'], ENT_QUOTES, 'UTF-8'); ?>
                                         </span>
                                         <i class="fa fa-angle-down" aria-hidden="true" style="margin-left:6px;margin-right:0;"></i>
                                     </button>

                                     <ul class="dropdown-menu" style="text-align: left; width:100%;">
                                         <?php
                                            $this->db->order_by("CASE 
                                            WHEN title = 'EMERGENCY' THEN 1 
                                            WHEN title = 'HEALTH CHECKUP' THEN 2 
                                            WHEN title = 'DIALYSIS' THEN 3 
                                            WHEN title = 'CHEMOTHERAPY' THEN 4 
                                            WHEN title = 'ENDOSCOPY' THEN 5 
                                            WHEN title = 'RADIOLOGY' THEN 6 
                                            WHEN title = 'LABORATORY' THEN 7 
                                            WHEN title = 'PHARMACY' THEN 8 
                                            WHEN title = 'PHYSIOTHERAPY' THEN 9 
                                            ELSE 0 END", 'ASC');
                                            ...
                                            foreach ($ward as $rw) {
                                                if ($_SESSION['ward'] == $rw->title) {
                                                    echo '<li><a class="dropdown-item" href="#" onclick=\'changefloore(' . json_encode($rw->title) . ')\'>'
                                                        . htmlspecialchars($rw->title, ENT_QUOTES, 'UTF-8') . ' </a></li>';
                                            ?><div class="dropdown-divider"></div>
                                         <?php
                                                } else {
                                                    echo '<li><a class="dropdown-item" href="#" onclick=\'changefloore(' . json_encode($rw->title) . ')\'>'
