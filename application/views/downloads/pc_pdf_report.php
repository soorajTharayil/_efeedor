...
$ip_tickets_count = $this->ticketsint_model->alltickets();
$ip_open_tickets = $this->ticketsint_model->read();
$ip_closed_tickets = $this->ticketsint_model->read_close();
$ip_addressed_tickets = $this->ticketsint_model->addressedtickets();

$feedbacktaken = $this->pc_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
$all_tickets = $this->pc_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
$sresult = $this->efeedor_model->setup_result($setup);
$ticket = $this->pc_model->get_tickets($table_feedback, $table_tickets);
...
$html .= '<h2 style="text-align:center;">EFEEDOR PATIENT COMPLAINTS REPORT </h2>';
...
if (ticket_addressal('pc_addressal') === true) {
	$html .= '<tr>
        <td width="80%">ADDRESSED COMPLAINTS</td>
...
$html .= '<tr>
    ...
    <tr>
        <td width="80%">COMPLAINT RESOLUTION RATE</td>
