...
$close_rate = $this->opf_model->ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action);
$ticket_close_rate = secondsToTime($close_rate);



$feedbacktaken = $this->opf_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
$ip_feedbacks_count = $this->opf_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);

// To see the total tickets count
$ip_tickets_count = $this->ticketsop_model->alltickets();
$ip_open_tickets = $this->ticketsop_model->read();
$ip_closed_tickets = $this->ticketsop_model->read_close();
$ip_addressed_tickets = $this->ticketsop_model->addressedtickets();

// IP ANALYTICS
$ip_nps = $this->opf_model->nps_analytics($table_feedback, $asc, $setup);
$ip_nps_tool = 'Promoters: ' . ($ip_nps['promoters_count']) . ', ' . "Detractors: " . ($ip_nps['detractors_count']) . ', ' . "Passives: " . ($ip_nps['passives_count']);
$ip_psat = $this->opf_model->psat_analytics($table_patients, $table_feedback, $table_tickets, $sorttime);
...
$ip_satisfied_count = $this->opf_model->get_satisfied_count($table_feedback, $table_tickets);
$ip_unsatisfied_count = $this->opf_model->get_unsatisfied_count($table_feedback, $table_tickets);
...
$ticket = $this->opf_model->get_tickets($table_feedback, $table_tickets);
...
$pdf->SetTitle('OP - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . '  ');
$pdf->SetSubject('OP - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . '  ');
...
$html .= '<h2 style="text-align:center;">EFEEDOR OUTPATIENT FEEDBACK REPORT </h2>';
...
$html .= '<table border="1" cellpadding="5">
					<tr>
						...
						<td  style="text-align:center;" width="10%"><b>VERY POOR</b></td>
...
$pdf->Output('OP - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . ' .pdf', 'I');
