...
$close_rate = $this->isr_model->ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action);
$ticket_close_rate = secondsToTime($close_rate);


$ip_tickets_count = $this->ticketsesr_model->alltickets();
$ip_open_tickets = $this->ticketsesr_model->read();
$ip_closed_tickets = $this->ticketsesr_model->read_close();
$ip_addressed_tickets = $this->ticketsesr_model->addressedtickets();

$feedbacktaken = $this->isr_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
$all_tickets = $this->isr_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
$sresult = $this->efeedor_model->setup_result($setup);
$ticket = $this->isr_model->get_tickets($table_feedback, $table_tickets);
...
$pdf->SetTitle('INTERNAL REQUESTS REPORT  - ' . $d1 . ' to ' . $d2 . '  ');
$pdf->SetSubject('INTERNAL REQUESTS REPORT - ' . $d1 . ' to ' . $d2 . '  ');
...
$html .= '<h2 style="text-align:center;">EFEEDOR STAFF SERVICE REQUESTS REPORT </h2>';
$html .= '<p><span style="text-align:left;">SHOWING DATA FROM ' . $ward . '- ' . $d1 . ' TO ' . $d2 . '</span></p>';


$html .= '<table border="1" cellpadding="5" >
    <tr>
        <td colspan="2" style="text-align:center;"><b>SERVICE REPORT</b></td>
    </tr>
    <tr>
        <td width="80%">TOTAL REQUESTS</td>
        <td width="20%">' . count($ip_tickets_count) . '</td>
    </tr>
    <tr>
        <td width="80%">OPEN REQUESTS</td>
        <td width="20%">' . count($ip_open_tickets) . '</td>
    </tr>';

if (ticket_addressal('isr_addressal') === true) {
	$html .= '<tr>
        <td width="80%">ADDRESSED REQUESTS</td>
        <td width="20%">' . count($ip_addressed_tickets) . '</td>
    </tr>';
}

$html .= '<tr>
        <td width="80%">CLOSED REQUESTS</td>
        <td width="20%">' . count($ip_closed_tickets) . '</td>
    </tr>
    <tr>
        <td width="80%">REQUEST RESOLUTION RATE</td>
...
$html .= '<table border="1" cellpadding="5">
					<tr>
						<td  colspan="3" style="text-align:center;" ><b>REQUESTS RECEIVED BY CATEGORY</b></td>
					</tr>
					<tr>
						<td style="width:60%">CATEGORY </td>
						<td style="width:20%">PERCENTAGE </td>
						<td style="width:20%">BY NO. OF REQUESTS </td>
					</tr>';
$highest_complain = '';
$lowest_complain = '';
$i = 0;
...
$html .= '<table border="1" cellpadding="5">
					<tr>
						<td  colspan="3" style="text-align:center;"><b>REQUESTS ANALYSIS</b></td>
					</tr>
					<tr>
						<td>HIGHEST REQUESTS RECEIVED </td>
						<td>' . $ticketbydepartmentname[$highest_complain] . '</td>
						<td>' . $ticketbydepartment_percentage[$highest_complain] . '%</td>
					</tr>
					<tr>
						<td>LEAST REQUESTS RECEIVED</td>
...
$pdf->Output('INTERNAL REQUESTS REPORT  - ' . $d1 . ' to ' . $d2 . ' .pdf', 'I');
