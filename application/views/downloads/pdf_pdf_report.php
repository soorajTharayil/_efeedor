...
$close_rate = $this->post_model->ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action);
...
$feedbacktaken = $this->post_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
...
$ip_satisfied_count = $this->post_model->get_satisfied_count($table_feedback, $table_tickets);
$ip_unsatisfied_count = $this->post_model->get_unsatisfied_count($table_feedback, $table_tickets);
...
$ticket = $this->post_model->get_tickets($table_feedback, $table_tickets);
...
$pdf->SetTitle('POST - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . '  ');
$pdf->SetSubject('POST - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . '  ');
...
$pdf->Output('POST - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . ' .pdf', 'I');
