...
$close_rate = $this->admissionfeedback_model->ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action);
...
$feedbacktaken = $this->admissionfeedback_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
...
$key_highlights = $this->admissionfeedback_model->key_highlights($table_patients, $table_feedback, $sorttime, $setup);


$ip_satisfied_count = $this->admissionfeedback_model->get_satisfied_count($table_feedback, $table_tickets);
$ip_unsatisfied_count = $this->admissionfeedback_model->get_unsatisfied_count($table_feedback, $table_tickets);
...
$ticket = $this->admissionfeedback_model->get_tickets($table_feedback, $table_tickets);
...
$pdf->SetTitle('ADF - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . '  ');
$pdf->SetSubject('ADF - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . '  ');
...
$html .= '<h2 style="text-align:center;">EFEEDOR ADMISSION FEEDBACK REPORT </h2>';
...
$pdf->Output('ADF - OVERALL PATIENTS FEEDBACK REPORT  - ' . $d1 . ' to ' . $d2 . ' .pdf', 'I');
