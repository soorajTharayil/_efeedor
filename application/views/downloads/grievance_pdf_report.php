...
$close_rate = $this->grievance_model->ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action);
$ticket_close_rate = secondsToTime($close_rate);

$ip_tickets_count = $this->ticketsgrievance_model->alltickets();
$ip_open_tickets = $this->ticketsgrievance_model->read();
$ip_closed_tickets = $this->ticketsgrievance_model->read_close();
$ip_addressed_tickets = $this->ticketsgrievance_model->addressedtickets();


$feedbacktaken = $this->grievance_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
$all_tickets = $this->grievance_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
$sresult = $this->efeedor_model->setup_result($setup);
$ticket = $this->grievance_model->get_tickets($table_feedback, $table_tickets);
...
$pdf->SetTitle('GRIEVANCE REPORT  - ' . $d1 . ' to ' . $d2 . '  ');
$pdf->SetSubject('GRIEVANCE REPORT - ' . $d1 . ' to ' . $d2 . '  ');
...
$html .= '<h2 style="text-align:center;">EFEEDOR STAFF GRIEVANCES REPORT </h2>';
$html .= '<p><span style="text-align:left;">SHOWING DATA FROM ' . $ward . '- ' . $d1 . ' TO ' . $d2 . '</span></p>';

$html .= '<table border="1" cellpadding="5" >
    <tr>
        <td colspan="2" style="text-align:center;"><b>GRIEVANCES REPORT</b></td>
    </tr>
    <tr>
        <td width="80%">TOTAL GRIEVANCES</td>
        <td width="20%">' . count($ip_tickets_count) . '</td>
    </tr>
    <tr>
        <td width="80%">OPEN GRIEVANCES</td>
        <td width="20%">' . count($ip_open_tickets) . '</td>
    </tr>';

if (ticket_addressal('grievance_addressal') === true) {
	$html .= '<tr>
        <td width="80%">ADDRESSED GRIEVANCES</td>
        <td width="20%">' . count($ip_addressed_tickets) . '</td>
    </tr>';
}

$html .= '<tr>
        <td width="80%">CLOSED GRIEVANCES</td>
        <td width="20%">' . count($ip_closed_tickets) . '</td>
    </tr>
    <tr>
        <td width="80%">GRIEVANCE RESOLUTION RATE</td>
...
$html .= '<table border="1" cellpadding="5">
					<tr>
						<td  colspan="3" style="text-align:center;" ><b>GRIEVANCES RECEIVED BY CATEGORY</b></td>
					</tr>
					<tr>
						<td style="width:60%">CATEGORY </td>
						<td style="width:20%">PERCENTAGE </td>
						<td style="width:20%">BY NO. OF GRIEVANCES </td>
					</tr>';
$highest_complain = '';
$lowest_complain = '';
$i = 0;
...
$html .= '<table border="1" cellpadding="5">
					<tr>
						<td  colspan="3" style="text-align:center;"><b>GRIEVANCE ANALYSIS</b></td>
					</tr>
					<tr>
						<td>HIGHEST GRIEVANCES RECEIVED </td>
						<td>' . $ticketbydepartmentname[$highest_complain] . '</td>
						<td>' . $ticketbydepartment_percentage[$highest_complain] . '%</td>
					</tr>
					<tr>
						<td>LEAST GRIEVANCES RECEIVED</td>
...
$pdf->Output('GRIEVANCE REPORT  - ' . $d1 . ' to ' . $d2 . ' .pdf', 'I');
