<div class="content">
							...
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip"
										title="<?php echo lang_loader('ip', 'ip_discharge_feedback_id_tooltip'); ?>"> <i
											class="fa fa-question-circle" aria-hidden="true"></i></a>Percentage of bronchiolitis
									patients treated inappropriately (Pediatrics) - <?php echo $result->id; ?> </h3>
							</div>
							<?php if (ismodule_active('QUALITY') === true && isfeature_active('QUALITY-EDIT-PERMISSION') === true) { ?>
								<div class="btn-group no-print" style="float: right;">
									<a class="btn btn-danger" style="margin-top:-40px;margin-right:10px;"
										href="<?php echo base_url($this->uri->segment(1) . "/edit_feedback_CQI3a16/$id") ?>"> <i
	...
	<script>
		...
		var myChart = new Chart(ctx, {
			...
			responsive: true,
			data: {
				labels: ['Number of patients treated inappropriately', 'Total number of patients with bronchiolitis'],
				datasets: [{
					label: 'Percentage of bronchiolitis  patients treated inappropriately',
			...
			options: {
				plugins: {
					...
					title: {
						display: true,
						text: 'Percentage of bronchiolitis  patients treated inappropriately - ' + monthyear,
						font: {
