<div class="content">
						...
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip"
										title="<?php echo lang_loader('ip', 'ip_discharge_feedback_id_tooltip'); ?>"> <i
											class="fa fa-question-circle" aria-hidden="true"></i></a>Percentage of ischemic stroke
									patients who develop a symptomatic intracranial hemorrhage (i.e., clinical deterioration ≥
									4-point increase on NIHSS and brain image finding of parenchymal hematoma, or subarachnoid
									hemorrhage, or intraventricular hemorrhage) within (≤) 36 hours after the onset of treatment
									with intra-venous (IV) or intra-arterial (IA) thrombolytic (t-PA) therapy, or mechanical
									endovascular reperfusion procedure (i.e., mechanical endovascular thrombectomy with a clot
									retrieval dev (Stroke Unit) <?php echo $result->id; ?> </h3>
							</div>
							<?php if (ismodule_active('QUALITY') === true && isfeature_active('QUALITY-EDIT-PERMISSION') === true) { ?>
								<div class="btn-group no-print" style="float: right;">
									<a class="btn btn-danger" style="margin-top:-40px;margin-right:10px;"
										href="<?php echo base_url($this->uri->segment(1) . "/edit_feedback_CQI4j15/$id") ?>"> <i
											class="fa fa-pencil" style="font-size:18px;"></i> Edit </a>
								</div>
							<?php } ?>
							<div class="panel-body" style="background: #fff;">
								...
								<table class=" table table-striped table-bordered  no-footer dtr-inline " style="font-size: 16px;">
									...
									<tr>
										<td><b>Number of patients with ischemic stroke developing SICH </b></td>

										<td><?php echo $param['initial_assessment_hr']; ?></td>
									</tr>
									<tr>
										<td><b>Total number of patients receiving recanalization treatment(IVT/EVT /IVT+EVT) </b>
										</td>

										<td><?php echo $param['total_admission']; ?></td>
									</tr>
									<tr>
										<td><b>Percentage of ischemic stroke patients who develop a symptomatic intracranial
												hemorrhage (i.e., clinical deterioration ≥ 4-point increase on NIHSS and brain image
												finding of parenchymal hematoma, or subarachnoid hemorrhage, or intraventricular
												hemorrhage) within (≤) 36 hours after the onset of treatment with intra-venous (IV)
												or intra-arterial (IA) thrombolytic (t-PA) therapy, or mechanical endovascular
												reperfusion procedure (i.e., mechanical endovascular thrombectomy with a clot
												retrieval dev (Stroke Unit)</b></td>
									...
									<tr>
										...
										<td>
											...
											// $param is an object → use ->files_name
											if (!empty($param->files_name) && is_array($param->files_name)) {

												foreach ($param->files_name as $file) {
													// $file is also an object → use ->
													$files[] = [
														'url' => $file->url,
														'name' => $file->name
													];
									...
									<div class="panel panel-default ">
										<div class="panel-heading">
											<h3 style="text-align: center; color:tomato;">
												<?php echo lang_loader('ip', 'ip_no_record_found'); ?> <br>
												<a href="<?php echo base_url(uri_string(1)); ?>">
													<button type="button" href="javascript:void()" data-toggle="tooltip"
														title="Back" class="btn btn-sm btn-success" style="text-align: center;">
								...
								<div class="panel panel-default ">
									<div class="panel-heading">
										...
										<table class="table">
											<tr>
												<th class=""
													style="border:none !important;vertical-align: middle; text-align:right;">
													<?php echo lang_loader('ip', 'ip_feedback_id'); ?></th>
												<td class="" style="border:none !important;">
													<input type="text" class="form-control" placeholder="Enter Feedback ID"
														maxlength="15" size="10" name="pid">
												</td>
												<th class="" style="text-align:left;">
													<p style="text-align:left;"><a href="javascript:void()"
															data-toggle="tooltip" title="Search"><button type="submit"
	...
	<script>
		...
		var myChart = new Chart(ctx, {
			...
			responsive: true,
			data: {
				labels: ['Number of patients with ischemic stroke developing SICH', 'Total number of patients receiving recanalization treatment(IVT/EVT /IVT+EVT)'],
				datasets: [{
					label: 'Percentage of ischemic stroke patients who develop a  symptomatic intracranial hemorrhage',
			...
			options: {
				plugins: {
					...
					title: {
						display: true,
						text: 'Percentage of ischemic stroke patients who develop a  symptomatic intracranial hemorrhage - ' + monthyear,
						font: {
