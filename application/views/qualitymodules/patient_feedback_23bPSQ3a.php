<div class="content">
	...
	?>
					...
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_id_tooltip'); ?>"> <i class="fa fa-question-circle" aria-hidden="true"></i></a> 23b. PSQ4c - <?php echo $result->id; ?> </h3>
							</div>
							<?php if (ismodule_active('QUALITY') === true  && isfeature_active('QUALITY-EDIT-PERMISSION') === true) { ?>
								<div class="btn-group no-print" style="float: right;">
									<a class="btn btn-danger" style="margin-top:-40px;margin-right:10px;" href="<?php echo base_url($this->uri->segment(1) . "/edit_feedback_23bPSQ3a/$id") ?>"> <i class="fa fa-pencil" style="font-size:18px;"></i> Edit </a>
								</div>
							<?php } ?>
							<div class="panel-body" style="background: #fff;">
								...
								<table class=" table table-striped table-bordered  no-footer dtr-inline " style="font-size: 16px;">
									...
									<tr>
										<td><b>Number of patients reported in X-ray</b></td>
										<td>
											<?php echo $result->no_of_patients_in_diagnostics; ?>
										</td>
									</tr>
									<tr>
										<td><b>Avg. waiting time for X-ray</b></td>
										<td>
											<?php
											// Benchmark time (50 minutes) in seconds
											$benchmarkSeconds = 15 * 60;

											// Convert the sum_of_reporting_time to seconds
											list($calculatedHours, $calculatedMinutes, $calculatedSeconds) = explode(':', $result->xray_wait_time);
											...
											<span style="color: <?php echo $color; ?>">
												<?php echo $result->xray_wait_time; ?>
											</span>
										</td>
									</tr>
									<tr>
				...
				<script>
					...
					var calculated = "<?php echo $result->xray_wait_time; ?>"; // Calculated value
					...
					var myChart = new Chart(ctx, {
						...
						responsive: true,
						data: {
							labels: ['Benchmark Time', 'Avg. waiting time for X-ray'],
							datasets: [{
								label: 'Benchmark Time compared with Avg. waiting time for X-ray',
						...
						options: {
							plugins: {
								...
								title: {
									display: true,
									text: 'Avg. waiting time for X-ray for ' + monthyear,
									font: {
		...
		<body>
			<h3>23b.PSQ4c- Waiting time for X-ray diagnostics</h3>
	...
	`;
				...
				<script>
					function downloadChartImage() {
						...
						link.href = image;
						link.download = '23b.PSQ4c- Waiting time for X-ray diagnostics.png'; // Name of downloaded file
