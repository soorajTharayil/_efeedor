<div class="content">
							...
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip"
										title="<?php echo lang_loader('ip', 'ip_discharge_feedback_id_tooltip'); ?>"> <i
											class="fa fa-question-circle" aria-hidden="true"></i></a>Incidence of patient
									identification error(Quality Office) <?php echo $result->id; ?> </h3>
							</div>
							<?php if (ismodule_active('QUALITY') === true && isfeature_active('QUALITY-EDIT-PERMISSION') === true) { ?>
								<div class="btn-group no-print" style="float: right;">
									<a class="btn btn-danger" style="margin-top:-40px;margin-right:10px;"
										href="<?php echo base_url($this->uri->segment(1) . "/edit_feedback_CQI3j4/$id") ?>"> <i
	...
	<script>
		...
		var myChart = new Chart(ctx, {
			...
			responsive: true,
			data: {
				labels: ['Number of patient identification errors', 'Total number of patients'],
				datasets: [{
					label: 'Incidence of patient identification error',
			...
			options: {
				plugins: {
					...
					title: {
						display: true,
						text: 'Incidence of patient identification error - ' + monthyear,
						font: {
