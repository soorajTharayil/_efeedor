<div class="content">
	...
	?>
					...
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_id_tooltip'); ?>"> <i class="fa fa-question-circle" aria-hidden="true"></i></a> 23c. PSQ4c - <?php echo $result->id; ?> </h3>
							</div>
							<?php if (ismodule_active('QUALITY') === true  && isfeature_active('QUALITY-EDIT-PERMISSION') === true) { ?>
								<div class="btn-group no-print" style="float: right;">
									<a class="btn btn-danger" style="margin-top:-40px;margin-right:10px;" href="<?php echo base_url($this->uri->segment(1) . "/edit_feedback_23cPSQ3a/$id") ?>"> <i class="fa fa-pencil" style="font-size:18px;"></i> Edit </a>
								</div>
							<?php } ?>
							<div class="panel-body" style="background: #fff;">
								...
								<table class=" table table-striped table-bordered  no-footer dtr-inline " style="font-size: 16px;">
									...
									<tr>
										<td><b>Number of patients reported in USG</b></td>
										<td>
									...
									<tr>
										...
										<td>
											...
											list($calculatedHours, $calculatedMinutes, $calculatedSeconds) = explode(':', $result->usg_wait_time);
				...
				<script>
					...
					var calculated = "<?php echo $result->usg_wait_time; ?>"; // Calculated value
					...
					var myChart = new Chart(ctx, {
						...
						responsive: true,
						data: {
							labels: ['Benchmark Time', 'Avg. waiting time for USG'],
							datasets: [{
								label: 'Benchmark Time compared with Avg. waiting time for USG',
						...
						options: {
							plugins: {
								...
								title: {
									display: true,
									text: 'Avg. waiting time for USG for ' + monthyear,
									font: {
		...
		<body>
			<h3>23c.PSQ4c- Waiting time for USG diagnostics</h3>
	...
	`;
				...
				<script>
					function downloadChartImage() {
						...
						link.href = image;
						link.download = '23c.PSQ4c- Waiting time for USG diagnostics.png'; // Name of downloaded file
