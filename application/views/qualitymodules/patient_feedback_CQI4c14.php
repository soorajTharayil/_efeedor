					...
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip" title="QUALITY INDICATOR MANAGER <KPI ID>"> <i class="fa fa-question-circle" aria-hidden="true"></i></a>Engineering Critical equipment downtime (Engineering & Maintenance) - <?php echo $result->id; ?> </h3>
							</div>
							<?php if (ismodule_active('QUALITY') === true  && isfeature_active('QUALITY-EDIT-PERMISSION') === true) { ?>
								<div class="btn-group no-print" style="float: right;">
									<a class="btn btn-danger" style="margin-top:-40px;margin-right:10px;" href="<?php echo base_url($this->uri->segment(1) . "/edit_feedback_CQI4c14/$id") ?>"> <i class="fa fa-pencil" style="font-size:18px;"></i> Edit </a>
								</div>
							<?php } ?>
							<div class="panel-body" style="background: #fff;">
								...
								<table class=" table table-striped table-bordered  no-footer dtr-inline " style="font-size: 16px;">
									...
									<tr>
										<td><b>Sum of down time of all critical equipment(in Hrs)</b></td>
										<td>
											<?php
											$hr  = $param->initial_assessment_hr ?? '';
											$min = $param->initial_assessment_min ?? '';
											$sec = $param->initial_assessment_sec ?? '';

											// If all empty → show nothing
											if ($hr === '' && $min === '' && $sec === '') {
												echo '';
											} else {
												echo trim("$hr:$min:$sec", ":");
											}
											?>
										</td>
									</tr>
									...
									<tr>
										...
										<td>
											<?php
											// Convert files_name (object array) to pure array format
											$files = [];

											if (!empty($param->files_name) && is_array($param->files_name)) {
												foreach ($param->files_name as $file) {

													// Convert each $file object into an associative array
													$files[] = [
