<div class="content">
	...
	?>
					...
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_id_tooltip'); ?>"> <i class="fa fa-question-circle" aria-hidden="true"></i></a> 7. PSQ3a - <?php echo $result->id; ?> </h3>
							</div>
							<?php if (ismodule_active('QUALITY') === true  && isfeature_active('QUALITY-EDIT-PERMISSION') === true) { ?>
								<div class="btn-group np-print" style="float: right;">
									<a class="btn btn-danger" style="margin-top:-40px;margin-right:10px;" href="<?php echo base_url($this->uri->segment(1) . "/edit_feedback_7PSQ3a/$id") ?>"> <i class="fa fa-pencil" style="font-size:18px;"></i> Edit </a>
								</div>
							<?php } ?>
							<div class="panel-body" style="background: #fff;">
								...
								<table class=" table table-striped table-bordered  no-footer dtr-inline " style="font-size: 16px;">
									...
									<tr>
										<td><b>Number of unplanned return to OT</b></td>
										<td>
											<?php echo $result->no_unplanned_return_ot; ?>
										</td>
									</tr>
									<tr>
										<td><b>Number of patients who underwent surgeries in the OT</b></td>
										<td>
											<?php echo $result->no_patients_in_ot; ?>
										</td>
									</tr>
									<tr>
										<td><b>Percentage of unplanned return to OT</b></td>
										<td>

											<?php echo $result->percentage_of_unplanned_return_ot; ?>
										</td>
				...
				<script>
					// Data
					var benchmark = "<?php echo $result->no_unplanned_return_ot;  ?>"; // Benchmark value
					var calculated = "<?php echo  $result->no_patients_in_ot; ?>"; // Calculated value
					...
					var myChart = new Chart(ctx, {
						...
						responsive: true,
						data: {
							labels: ['Number of unplanned return to OT', 'Number of patients who underwent surgeries in the OT'],
							datasets: [{
								label: 'Number of unplanned return to OT compare with Number of patients who underwent surgeries in the OT',
						...
						options: {
							plugins: {
								...
								title: {
									display: true,
									text: 'Percentage of unplanned return to OT for ' + monthyear,
									font: {
		...
		<body>
			<h3>7.PSQ3a- Percentage of unplanned return to OT</h3>
	...
	`;
				...
				<script>
					function downloadChartImage() {
						...
						link.href = image;
						link.download = '7.PSQ3a- Percentage of unplanned return to OT.png'; // Name of downloaded file
