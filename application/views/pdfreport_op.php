...
$feedbacktaken = $this->efeedor_model->get_feedback('bf_opatients','bf_outfeedback',$fdatet,$tdate);
$all_tickets = $this->efeedor_model->get_tickets('bf_outfeedback', 'ticketsop', $fdatet, $tdate);
...
$this->db->select('ticketsop.*,bf_opatients.name as pname,bf_outfeedback.datet');
$this->db->from('ticketsop');
$this->db->join('bf_outfeedback','bf_outfeedback.id = ticketsop.feedbackid','inner');
...
$sresult = $this->efeedor_model->setup_result('setupop');
...
$d2 = date('d/m/Y',strtotime($fdatet)-86400);
...
//$pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);// set auto page breaks
...
$html .= '<table border="1" cellpadding="5">
			<tr>
				<td  style="text-align:center;" width="80%">RATING PARAMETERS</td>
				<td  style="text-align:center;" width="20%">PERFORMANCE</td>
			</tr>';
			foreach($scoreseto  as $key => $value){
			    $html .='<tr>
...
$html .= '<table border="1" cellpadding="5">
			<tr>
				<td  style="text-align:center;">OVERALL RATING BREAKDOWN</td>
				<td  style="text-align:center;">BY PERCENTAGE</td>
				<td  style="text-align:center;">BY NO. OF PATIENTS</td>
			</tr>
			<tr>
				<td>EXCELLENT(5)</td>
				<td>'.round($value5).'%</td>
