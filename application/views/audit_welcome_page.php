                    ...
                    <div class="panel-body" style="height:100px;">
                        <div class="statistic-box">
                            <h2 style="font-size:25px;"><span class="count-number"><?php echo count($ip_feedbacks_count3); ?></span> 
                                <span class="slight"><i class="fa fa-play fa-rotate-270 text-warning"></i></span></h2>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
      ...
      const categories = [
        { start: 1, end: 29, heading: "MRD & MDC" },
        { start: 30, end: 44, heading: "Nursing & IPSG" },
        { start: 45, end: 70, heading: "Clinical Outcome" },
        { start: 71, end: 72, heading: "Clinical KPI" },
        { start: 73, end: 89, heading: "Infection Control & PCI" },
        { start: 90, end: 100, heading: "Clinical Pathways" }
      ];
            ...
            if (match) {
              ...
              categories.forEach(cat => {
                if (num >= cat.start && num <= cat.end) {
                  visibleCategories.add(cat.heading);
                }
              });
            }
          }
      ...
      function updateHeadings(searchActive) {
        const visibleCats = getVisibleCategories();
    
        categories.forEach(cat => {
          document.querySelectorAll(".heading h2").forEach(h => {
            if (h.textContent.trim().toLowerCase() === cat.heading.toLowerCase()) {
              const block = h.closest(".heading-block") || h.closest(".col-12");
              if (block) {
                if (!searchActive) {
                  block.style.display = "block";
                } else {
                  block.style.display = visibleCats.has(cat.heading) ? "block" : "none";
                }
              }
            }
          });
      ...
      document.querySelectorAll(".heading-block, .col-12").forEach(b => b.style.display = "block");
      setTimeout(() => updateHeadings(false), 300);
