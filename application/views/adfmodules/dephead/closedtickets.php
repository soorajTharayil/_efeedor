		...
		<div class="row">
			...
			<div class="col-lg-12">
				...
				<div class="panel-heading" style="text-align: right;">
						<div class="btn-group">
							<a class="btn btn-success" data-placement="bottom" data-toggle="tooltip" title="<?php echo lang_loader('adf','adf_download_capa_report_tooltip'); ?>" href="<?php echo base_url($this->uri->segment(1)) . '/download_capa_report' ; ?>">
					...
					<div class="panel-body">
							...
							<form>
								<p> 
									...
									<select name="dep" class="form-control" id="subsecid" onchange="gotonextdepartment2(this.value)" style="width:200px; margin:0px 0px 20px 20px;">
										<option value="1" selected><?php echo lang_loader('adf','adf_select_department'); ?></option>
										<?php
									...
									<?php if (isset($_GET['depsec'])) { ?>
										...
										<select name="dep" class="form-control" onchange="gotonextdepartment(this.value)" style="width:200px; margin:0px 0px 20px 20px;">
											<option value="1" selected><?php echo lang_loader('adf','adf_select_parameter'); ?></option>
											<?php
											$this->db->where('type', 'adf');
						...
						<table class="ipticketsclose table table-striped table-bordered table-hover" cellspacing="0" width="100%">
							<thead>
								<tr>
									<th><?php echo lang_loader('adf','adf_slno'); ?></th>
									<th style="white-space: nowrap;"><?php echo lang_loader('adf','adf_ticket_id'); ?></th>
									<th style="white-space: nowrap;"><?php echo lang_loader('adf','adf_patient_details'); ?></th>
									<th style="white-space: nowrap;"><?php echo lang_loader('adf','adf_rating'); ?></th>
									<th style="white-space: nowrap;"><?php echo lang_loader('adf','adf_concern'); ?></th>
									<th style="white-space: nowrap;"><?php echo lang_loader('adf','adf_rca'); ?></th>
									<th style="white-space: nowrap;"><?php echo lang_loader('adf','adf_capa'); ?></th>
									<?php if (close_comment('adf_close_comment') === true) { ?> 
										<th style="white-space: nowrap;"><?php echo lang_loader('adf','adf_comment'); ?></th>
									<?php } ?>
									<?php if (adf_tat('close_ticket') === true) { ?>
									<th style="white-space: nowrap;"><?php echo lang_loader('adf','adf_turn_around'); ?><br><?php echo lang_loader('adf','adf_tat'); ?></th>
									<?php } ?>
								...
								<?php if (!empty($departments)) {		?>
									...
									<?php foreach ($departments as $department) {
										...
										if ($department->status == 'Closed') {
											...
											$query = $this->db->get('ticket_message_adf');
											$ticket = $query->result();
									...
									?>
										<tr class="<?php echo ($sl & 1) ? "odd gradeX" : "even gradeC" ?>" data-placement="bottom" data-toggle="tooltip" title="<?php echo $rowmessage; ?>">
											<td><?php echo $sl; ?></td>
											<td><?php echo lang_loader('adf','adf_adft'); ?><?php echo $department->id; ?></td>
