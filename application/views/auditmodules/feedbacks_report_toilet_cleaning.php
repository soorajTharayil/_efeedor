			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="toilet table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								<th><?php echo lang_loader('ip', 'ip_slno'); ?></th>
								<th>Audited by</th>
								<th>Date</th>
								<th>Room/ Area</th>
								<th>Is the toilet cleaned properly?</th>
								<th>Are all surfaces disinfected?</th>
								<th>Are mirrors cleaned thoroughly?</th>
								<th>Is the floor cleaned?</th>
								<th>Are supplies restocked adequately?</th>
								<th>Is the trash emptied?</th>
								<th>Are toilets free from bad odor?</th>
								<th>Is the exhaust fan cleaned properly?</th>
								<th>Is there any water leakage or damage observed?</th>
								<th>Are floor mats clean and properly placed?</th>
								<th>Are housekeeping equipment arranged in the designated spaces?</th>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $toilet_cleaning_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_toilet_cleaning"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>TOILET CLEANING CHECKLIST</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'TOILET CLEANING CHECKLIST.png';
			link.click();
		};
		img.src = chartImage;
	}
