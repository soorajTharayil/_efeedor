			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="urinary table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								...
								<th>Staff name</th>
								<th>Is the balloon size adequate?</th>
								<th>Are appropriate hand hygiene and gloves used while handling the catheter or drainage bag?</th>
								<th>Are catheters properly secured to prevent movement and urethral traction?</th>
								<th>Is a sterile closed drainage system maintained?</th>
								<th>Is good hygiene maintained at the catheter-urethral interface?</th>
								<th>Is urine flow unobstructed?</th>
								<th>Is the drainage bag maintained below the level of the bladder at all times?</th>
								<th>Are indwelling catheters or drainage bags changed only when clinically indicated and not at arbitrary fixed intervals?</th>
								<th>If there are breaks in aseptic technique, disconnection, or leakage, is the catheter replaced using aseptic technique and sterile equipment?</th>
								<th>Is periodical perineal and catheter care performed?</th>
								<th>Is the urobag emptied every 6 hours?</th>
								<th>Is the patient assessed for any pain or discomfort?</th>
								<th>Is the meatus inspected for redness, irritation, drainage, and documented?</th>
								<th>Is the catheter inspected where it enters the meatus for encrusted material and drainage?</th>
								<th>Are any encrusted materials on the tubing removed?</th>
								<th>Is the meatus cleaned during daily bathing without using antiseptics?</th>
								<th>Is it ensured that the tubing does not go in and out of the urethra during cleaning?</th>
								<th>Is urine collected for culture using a sterile needle and syringe?</th>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $urinary_catheter_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_urinary_catheter"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>URINARY CATHETER MAINTENANCE CHECKLIST</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'URINARY CATHETER MAINTENANCE CHECKLIST.png';
			link.click();
		};
		img.src = chartImage;
	}
