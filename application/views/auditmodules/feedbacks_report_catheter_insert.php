			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="catheter table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								...
								<th>Staff name</th>
								<th>Has an alternative for the indwelling catheterization been considered and documented?</th>
								<th>Is the patient confirmed using two identifiers?</th>
								<th>Are the clinical reasons for insertion specified, communicated to the patient/bystander, and documented?</th>
								<th>Has the need, possible complications, potential outcomes, and procedures been explained, and consent obtained from the patient/bystander by the performing staff?</th>
								<th>Did the person performing the procedure perform hand hygiene?</th>
								<th>Is the person performing the procedure wearing PPE?</th>
								<th>Does the person performing the procedure use sterile gloves?</th>
								<th>Is the catheterization kit available with a sterile catheter, sterile gloves, antiseptic solution, saline and syringe, dressing packs, scissors and forceps, lubricant, tray, and hand rub?</th>
								<th>Is the smallest gauge of the catheter selected for effective drainage?</th>
								<th>Has perineal care been performed, and has hand hygiene been re-performed?</th>
								<th>Is the catheter lubricated with sterile lubricant?</th>
								<th>Is aseptic technique maintained throughout the insertion, ensuring the appropriate length is inserted, and has hand hygiene been re-performed?</th>
								<th>Is urine allowed to drain before the catheter balloon is inflated?</th>
								<th>Is the balloon inflated correctly?</th>
								<th>Is the catheter connected to a sterile drainage bag, placed safely below the level of the bladder, and has hand hygiene been re-performed?</th>
								<th>Is urine flow checked, and is it ensured that there is no urethral trauma?</th>
								<th>Is a urine sample sent to the laboratory, if required/advised, and has hand hygiene been re-performed?</th>
								<th>Has the patient/bystander been educated regarding catheter care, the frequency of follow-ups, and has the procedure been documented?</th>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $catheter_insert_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
										...
										<td style="overflow: clip;">
											<?php echo $r->patientname . " (" . $r->patientid . ")<br>Age: " . $r->age; ?>
										</td>

										<td><?php echo $r->staffname; ?></td>
										<td><?php echo $r->identification_details; ?></td>
										<td><?php echo $r->vital_signs; ?></td>
										<td><?php echo $r->surgery; ?></td>
										<td><?php echo $r->complaints_communicated; ?></td>
										<td><?php echo $r->intake; ?></td>
										<td><?php echo $r->output; ?></td>
										<td><?php echo $r->allergies; ?></td>
										<td><?php echo $r->medication; ?></td>
										<td><?php echo $r->diagnostic; ?></td>
										<td><?php echo $r->lab_results; ?></td>
										<td><?php echo $r->pending_investigation; ?></td>
										<td><?php echo $r->medicine_order; ?></td>
										<td><?php echo $r->facility_communicated; ?></td>
										<td><?php echo $r->health_education; ?></td>
										<td><?php echo $r->risk_assessment; ?></td>
										<td><?php echo $r->urethral; ?></td>
										<td><?php echo $r->urine_sample; ?></td>
										<td><?php echo $r->bystander; ?></td>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_catheter_insert"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>CATHETER INSERTION CHECKLIST</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'CATHETER INSERTION CHECKLIST.png';
			link.click();
		};
		img.src = chartImage;
	}
