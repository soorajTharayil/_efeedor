					...
					<div class="panel-body" style="font-size:14px; line-height:1.5;">
						...
						if (!empty($users)) {
							foreach ($users as $user) {
								...
								foreach ($perms as $p) {
									if (strcasecmp(trim($p->feature_name), 'AUDIT-FORM1') === 0) {
						...
						<table class="table table-bordered table-condensed" style="margin:0;">
							<tbody>
								<tr>
									...
									<td>Ensures record completeness, accuracy & compliance (times, consent, discharge) per NABH, WHO, CAHO, ICMR, JCI, CDC standards.</td>
								</tr>
								<tr>
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading" style="display: flex; justify-content: space-between; align-items: center;">
						<div>
							<strong>Surface cleaning and disinfection effectiveness monitoring record - Audit Summary</strong>
						</div>
						<div>
							<a class="btn btn-success" target="_blank" data-placement="bottom" data-toggle="tooltip" title="Download detailed audit report" href="<?php echo base_url($this->uri->segment(1)) . '/overall_infection_control_surface_cleaning_audit' ?>">
					...
					<div class="panel-body">
						<table class="infection_control_surface_cleaning_audit table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $infection_control_surface_cleaning_audit_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
										...
										<td>
											<?php echo !empty($param->initial_assessment_hr6)
												? date("d-m-Y H:i", strtotime($param->initial_assessment_hr6))
												: ''; ?>
										</td>
										...
										<td>
											<a href="<?php echo $infection_control_surface_cleaning_audit_feedback . $id; ?>"
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = "https://" + domain + "/analytics_audit_quality/resposnsechart_infection_control_surface_cleaning_audit"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>Active Cases MRD Audit- IP</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'Active Cases MRD Audit- IP.png';
			link.click();
		};
		img.src = chartImage;
	}
