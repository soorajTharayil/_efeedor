			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="mrdauditallfeedbacks table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								<th><?php echo lang_loader('ip', 'ip_slno'); ?></th>
								<th>Audited by</th>
								<th>MRD Audit Date</th>
								...
								<th>Admitted time</th>
								<th>Initial assessment time</th>
								<th>Time taken for assessment</th>
								<th>Consent</th>

								<th>Consent comment</th>
								<th>Discharge summary</th>
								<th>Error prone abbreviation</th>
								<th>Error prone comment</th>

								<th>Discharge advice time</th>
								<th>Billing time </th>
								<th>Time taken for discharge </th>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $mrd_audit_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
										...
										<?php if (allfeedbacks_page('feedback_id') == true) { ?>
											<td>
												<a href="<?php echo  $mrd_audit_feedback . $id; ?>">IPDF-<?php echo $id; ?></a>
											</td>
										<?php } ?>
										...
										<td style="overflow: clip;">
											...
											<br>
											<?php echo $r->patient_category; ?>
										</td>


										<td><?php echo $r->patient_got_admitted; ?></td>
										<td><?php echo $r->doctor_completed_assessment; ?></td>
										<td><?php echo date("H:i:s", strtotime($r->initial_assessment)); ?></td>
										<td><?php echo $r->consent_verified; ?></td>
										<?php if ($r->consent_comment) { ?>
											<td><?php echo $r->consent_comment; ?></td>
										<?php } else { ?>
											<td><?php echo "-" ?></td>
										<?php } ?>
										<td><?php echo $r->discharge_summary; ?></td>
										<td><?php echo $r->error_prone; ?></td>
										<?php if ($r->error_prone_comment) { ?>
											<td><?php echo $r->error_prone_comment; ?></td>
										<?php } else { ?>
											<td><?php echo "-" ?></td>
										<?php } ?>
										<td><?php echo $r->doctor_adviced_discharge; ?></td>
										<td><?php echo $r->bill_paid_time; ?></td>
										<td><?php echo date("H:i:s", strtotime($r->time_taken_for_discharge)); ?></td>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_mrd_audit"; // Replace with your API endpoint
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'MRD AUDIT REPORT.png';
			link.click();
		};
		img.src = chartImage;
	}
