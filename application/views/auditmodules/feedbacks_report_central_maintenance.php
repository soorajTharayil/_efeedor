			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="centralline table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								...
								<th>Staff name</th>
								<th>Are hand hygiene requirements being complied with?</th>
								<th>Are patients bathed daily with a chlorhexidine preparation?</th>
								<th>Is the access port or hub scrubbed with friction immediately prior to each use with an appropriate antiseptic (chlorhexidine, povidone-iodine, iodophor, or 70% alcohol)?</th>
								<th>Are only sterile devices used to access catheters?</th>
								<th>Are dressings that are wet, soiled, or dislodged immediately replaced?</th>
								<th>Are routine dressing changes performed using aseptic technique with clean or sterile gloves?</th>
								<th>Are administration sets for continuous infusions changed at the frequency set by the hospital?</th>
								<th>Is the need for each central line assessed daily?</th>
								<th>Is the site inspected for redness, irritation, or infection, and is this inspection documented?</th>
								<th>Is hand hygiene performed before and after each contact?</th>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $central_maintenance_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_central_maintenance"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>CENTRAL LINE MAINTENANCE CHECKLIST</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'CENTRAL LINE MAINTENANCE CHECKLIST.png';
			link.click();
		};
		img.src = chartImage;
	}
