			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="surgicalsafety table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								...
								<th>Surgery name</th>
								<th>Surgery date</th>
								<th>Was prophylactic antibiotic given?</th>
								<th>Was safety checklist followed?</th>
								<th>Was SSI bundle care followed?</th>
								<th>Was time-out performed?</th>
								<th>Was unplanned return to OT?</th>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $surgical_safety_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
										...
										<?php if (allfeedbacks_page('feedback_id') == true) { ?>
											<td>
												<a href="<?php echo  $surgical_safety_feedback . $id; ?>">IPDF-<?php echo $id; ?></a>
											</td>
										<?php } ?>

										<td style="overflow: clip;"><?php echo $r->patientid; ?></td>
										<td><?php echo $r->surgeryname; ?></td>
										<td><?php echo $r->date_of_surgery; ?></td>
										<td><?php echo $r->antibiotic; ?></td>
										<td><?php echo $r->checklist; ?></td>
										<td><?php echo $r->bundle_care; ?></td>
										<td><?php echo $r->time_out; ?></td>
										<td><?php echo $r->unplanned_return; ?></td>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_surgical_safety"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>SURGICAL SAFETY AUDIT REPORT</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'SURGICAL SAFETY AUDIT REPORT.png';
			link.click();
		};
		img.src = chartImage;
	}
