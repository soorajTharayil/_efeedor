			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="npratio table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $nurse_patient_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
										...
										<td style="overflow: clip;"><?php echo $r->site; ?></td>
										
										<td><?php echo !empty($r->icu) ? $r->icu : '-'; ?></td>
										<td><?php echo !empty($r->patient_status) ? $r->patient_status : '-'; ?></td>
										<td><?php echo $r->beds; ?></td>
										<td><?php echo $r->nurses; ?></td>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_nurse_patients_ratio"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>NURSE-PATIENT RATIO REPORT</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'NURSE-PATIENT RATIO REPORT.png';
			link.click();
		};
		img.src = chartImage;
	}
