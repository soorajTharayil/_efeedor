			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="returntoemr table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								...
								<th>Patient UHID</th>
								<th>Initial consultaion date</th>
								<th>Has same OP number been recorded in last 24 hours?</th>
								<th>Patient condition at initial consultation in last 24 hours</th>
								<th>Has same OP number been recorded in last 48 hours?</th>
								<th>Patient condition at initial consultation in last 48 hours</th>
								<th>Has same OP number been recorded in last 72 hours?</th>
								<th>Patient condition at initial consultation in last 72 hours</th>
								<th>Patient condition at re-consultation</th>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $return_to_emr_dv_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
										...
										<td><?php echo $r->initial_consultaion; ?></td>

										<td><?php echo $r->same_op_tf; ?></td>
										<?php if ($r->same_op_tf_comment) { ?>
											<td><?php echo $r->same_op_tf_comment; ?></td>
										<?php } else { ?>
											<td><?php echo "-" ?></td>
										<?php } ?>

										<td><?php echo $r->same_op_fe; ?></td>
										<?php if ($r->same_op_fe_comment) { ?>
											<td><?php echo $r->same_op_fe_comment; ?></td>
										<?php } else { ?>
											<td><?php echo "-" ?></td>
										<?php } ?>

										<td><?php echo $r->same_op_st; ?></td>
										<?php if ($r->same_op_st_comment) { ?>
											<td><?php echo $r->same_op_st_comment; ?></td>
										<?php } else { ?>
											<td><?php echo "-" ?></td>
										<?php } ?>

										<td><?php echo $r->re_consultation; ?></td>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_return_to_emr"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>EMERGENCY RETURN REPORT- DATA VERIFICATION</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'EMERGENCY RETURN REPORT- DATA VERIFICATION.png';
			link.click();
		};
		img.src = chartImage;
	}
