			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table class="returntoicu table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							<thead>
								...
								<th>Was the patient re-admitted within 24 hours of discharge?</th>
								<th>Reason for readmission within 24 hours</th>
								<th>Was the patient re-admitted within 48 hours of discharge?</th>
								<th>Reason for readmission within 48 hours</th>
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $return_to_icu_dv_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
										...
										<td><?php echo $r->re_admitted_tf; ?></td>

										<?php if ($r->re_admitted_tf_comment) { ?>
											<td><?php echo $r->re_admitted_tf_comment; ?></td>
										<?php } else { ?>
											<td><?php echo "-" ?></td>
										<?php } ?>
										<td><?php echo $r->re_admitted_fe; ?></td>

										<?php if ($r->re_admitted_fe_comment) { ?>
											<td><?php echo $r->re_admitted_fe_comment; ?></td>
										<?php } else { ?>
											<td><?php echo "-" ?></td>
										<?php } ?>
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = baseUrl +  "/analytics_audit_quality/resposnsechart_return_to_icu"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>ICU RETURN REPORT- DATA VERIFICATION</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'ICU RETURN REPORT- DATA VERIFICATION.png';
			link.click();
		};
		img.src = chartImage;
	}
