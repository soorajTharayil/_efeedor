            ...
            <div class="col-lg-12">
                <div class="panel panel-default">
                    <div class="panel-heading" style="text-align: right;">
                        <div class="btn-group">
                            <a class="btn btn-success" data-placement="bottom" style="margin-right: 10px;" data-toggle="tooltip" title="Register an asset" href="<?php echo base_url() . '/add_asset?user_id=' . $this->session->userdata['user_id']; ?>" target="_blank">
                                <i class="fa fa-building"></i> Register Asset
                            </a>

                            <button class="btn btn-success" style="margin-right: 10px;" data-toggle="modal" data-target="#dateModal" data-placement="bottom" data-toggle="tooltip" title="Download all assets with valuation date">
                                <i class="fa fa-calendar"></i> Asset Booking
                            </button>

                            <a class="btn btn-success" target="_blank" data-placement="bottom" data-toggle="tooltip" title="Download all assets" href="<?php echo base_url($this->uri->segment(1)) . '/download_' . ($this->uri->segment(2)) ?>">
                    ...
                    <div class="modal fade" id="dateModal" tabindex="-1" role="dialog" aria-labelledby="dateModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <form method="post" action="<?php echo base_url($this->uri->segment(1) . '/download_' . $this->uri->segment(2)); ?>">

                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="dateModalLabel">Select Valuation Date</h5>
                                    ...
                                    <div class="modal-body">
                                        <input type="date" class="form-control" id="valuation_date_picker" required>
                                    </div>

                                    <div class="modal-footer">
                                        <a id="downloadLink" target="_blank" class="btn btn-success" onclick="downloadWithValuationDate()">
                    ...
                    <div class="panel-body">
                        <?php if ($this->session->userdata('user_role') != 4) { ?>
                            <form>
                                <p> <span style="font-size:15px; font-weight:bold;">Filter By : </span>
                                    ...
                                    <select name="dep" class="form-control" id="subsecid" onchange="gotonextdepartment2(this.value)" style="width:230px; margin:0px 20px 0px 0px;">
                                        <option value="1" selected>Asset Group/ Category</option>
                                        <?php
                                    ...
                                    <select name="dep" class="form-control" id="subsecid2" onchange="gotonextdepartment3(this.value)" style="width:200px;">
                                        <option value="1" selected>Asset Department</option>
                                        <?php
                                        $this->db->select('title');
                        ...
                        <table class="assetticketsall table table-striped table-bordered table-hover" cellspacing="0" width="100%" style=" white-space: nowrap; width: 100%;">
                            <thead>
                                <tr>
                                    <th>Sl. <br> No</th>
                                    <th>Asset Details</th>
                                        ...
                                        <tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" data-placement="bottom" data-toggle="tooltip" title="<?php echo htmlspecialchars($rowmessage, ENT_QUOTES, 'UTF-8'); ?>" style="cursor: pointer;">
                                            ...
                                            <?php if (ismodule_active('ASSET') === true && isfeature_active('ASSET-DASHBOARD') === true) { ?>
                                                <td style="text-align: center; vertical-align: middle; padding: 10px;">
                                                    ...
                                                    <a class="btn btn-sm "
                                                        href="<?php echo  base_url() .  '/add_subasset?user_id=' . $this->session->userdata['user_id'] . '&assetname=' . urlencode($department->assetname) . '&assetcode=' . urlencode($department->patientid); ?>"
...
<script>
    ...
    function gotonextdepartment2(groupType) {
        ...
        var url = "<?php echo base_url($this->uri->segment(1) . '/alltickets?depsec=') ?>" + groupType;
    ...
    function gotonextdepartment3(departmentType) {
        ...
        var url = "<?php echo base_url($this->uri->segment(1) . '/alltickets?dep=') ?>" + departmentType;
...
<script>
    function downloadWithValuationDate() {
        const selectedDate = document.getElementById('valuation_date_picker').value;
        if (!selectedDate) {
            alert("Please select valuation date");
            return;
        }
        const url = "<?php echo base_url($this->uri->segment(1) . '/download_' . $this->uri->segment(2)); ?>?valuation_date=" + selectedDate;
