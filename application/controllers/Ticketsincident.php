	...
	public function create($dprt_id = null)
	{
		...
		if ($this->input->post('deparment') != 0) {
			...
			if ($this->input->post('status') == 'Closed') {
				...
				$action = 'Incident Explained by ' . $this->session->userdata['fullname'];
				$message = $this->session->userdata['fullname'];

				$dataset = array(
					'reply' => $this->input->post('reply'),
					'message' => $message,
					'rootcause_describtion' => $this->input->post('rootcause_describtion'),
					'corrective_description' => $this->input->post('corrective_description'),
					'action' => $action,
