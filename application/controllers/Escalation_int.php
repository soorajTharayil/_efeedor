...
class Escalation_int extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = 'Patient Complaint - Escalation User Assignment';
		#-------------------------------#
		
		$data['escalation'] = $this->escalation_model->read('INTERIM');
		
		$data['content'] = $this->load->view('escalation_int/index',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	
	public function action(){
		if($this->uri->segment(3) == 'active'){
			$this->escalation_model->change_status('INTERIM','ACTIVE');
		}elseif($this->uri->segment(3) == 'inactive'){
			$this->escalation_model->change_status('INTERIM','INACTIVE');
		}
