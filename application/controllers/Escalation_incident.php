...
class Escalation_incident extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['title'] = 'Incident - Escalation User Assignment';
		#-------------------------------#

		$data['escalation'] = $this->escalation_model->read('INCIDENT');

		$data['content'] = $this->load->view('escalation_incident/index', $data, true);
	...
	public function action()
	{
		if ($this->uri->segment(3) == 'active') {
			$this->escalation_model->change_status('INCIDENT', 'ACTIVE');
		} elseif ($this->uri->segment(3) == 'inactive') {
			$this->escalation_model->change_status('INCIDENT', 'INACTIVE');
		}
