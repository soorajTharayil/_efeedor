...
class Analytics_op extends CI_Controller
{
    ...
    public function patient_feedback_analysis()
    {
		...
		$departmentQuestionMapping = [
			"CARDIOLOGY" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"ENT" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Laboratory", "Other Facilities"],
			"GASTROENTEROLOGY" => ["Appointment Booking", "Doctor Consultation", "Laboratory", "Pharmacy", "Other Facilities"],
			"GENERAL MEDICINE" => ["Appointment Booking", "Doctor Consultation", "Laboratory", "Pharmacy", "Other Facilities"],
			"GENERAL SURGERY" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"NEUROLOGY" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Laboratory", "Other Facilities"],
			"ONCOLOGY" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"ORTHOPAEDICS" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Pharmacy", "Other Facilities"],
			"PEDIATRICS" => ["Appointment Booking", "Doctor Consultation", "Laboratory", "Pharmacy", "Other Facilities"],
			"PHYSIOTHERAPY" => ["Appointment Booking", "Doctor Consultation", "Other Facilities"],
			"PHYSIOTHERAPY & REHABILITATION" => ["Appointment Booking", "Doctor Consultation", "Other Facilities"],
			"PLASTIC SURGERY" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"UROLOGY" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"],
			"VASCULAR SURGERY" => ["Appointment Booking", "Doctor Consultation", "Radiology", "Laboratory", "Pharmacy", "Other Facilities"]
		];
		$allowSet = [];

        if (isset($_SESSION['ward'])) {
            $ward = $_SESSION['ward'];
            if (isset($departmentQuestionMapping[$ward]) && is_array($departmentQuestionMapping[$ward])) {
                $allowSet = $departmentQuestionMapping[$ward];
            }
        }
        foreach ($question_list_parent as $row) {
			if(count($allowSet) > 0 ){
				if(in_array($row->title,$allowSet)){
...
//     }
    public function resposnsechart()
    {
        $feedback_data = $this->opf_model->patient_and_feedback(
            $this->table_patients,
            $this->table_feedback,
            $this->sorttime,
            $this->setup
        ...
        // Safety guards
        if (!is_iterable($feedback_data)) {
            echo json_encode([]);
            exit;
        }
    
        $days  = isset($_SESSION['days']) ? (int)$_SESSION['days'] : 0;
        $y     = date('Y');
        $fdate = isset($_SESSION['from_date']) ? date('Y-m-d', strtotime($_SESSION['from_date'])) : date('Y-m-d');
        $tdate = isset($_SESSION['to_date'])   ? date('Y-m-d', strtotime($_SESSION['to_date']))   : date('Y-m-d');
        ...
        foreach ($feedback_data as $row) {
            ...
            // Increment count for this bucket (no need to count($row))
            $report[$mon]['count']++;
        }
    
        $overall = is_array($feedback_data) ? count($feedback_data) : iterator_count((function($it){ foreach($it as $_) yield; })($feedback_data));
    
        $response = [];
        foreach ($report as $label => $data) {
            $pct = $overall > 0 ? round(($data['count'] / $overall) * 100) : 0;
            $response[] = [
                'label_field' => $label,
                'data_field'  => $pct,
                'all_detail'  => [
    ...
    public function tickets_recived_by_department()
    {
        ...
        foreach ($department as $key => $row) {
            $set = array();
			if(count($allowSet) > 0 ){
				if(in_array($row->description,$allowSet)){
