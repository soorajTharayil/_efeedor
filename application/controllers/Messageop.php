...
class Messageop extends CI_Controller
{
	...
	public function inweeklyreport()
	{
		...
		$ip_feedbacks_count = $this->opd_opt_model_weekly->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
		$totalFeedbackList = $ip_feedbacks_count;
		$allTickets = $this->opd_opt_model_weekly->get_tickets($table_feedback, $table_tickets);
		$ip_psat = $this->opd_opt_model_weekly->psat_analytics($table_patients, $table_feedback, $table_tickets, $sorttime, $allTickets, $totalFeedbackList);
		$ip_psat_tool = 'Satisfied Patients: ' . ($ip_psat['satisfied_count']) . ', ' . "Unsatisfied Patients: " . ($ip_psat['unsatisfied_count']) . '. ';

		// IP ANALYTICS
		$ip_nps = $this->opd_opt_model_weekly->nps_analytics($table_feedback, $asc, $setup);
		$ip_nps_tool = 'Promoters: ' . ($ip_nps['promoters_count']) . ', ' . "Detractors: " . ($ip_nps['detractors_count']) . ', ' . "Passives: " . ($ip_nps['passives_count']);
		$ip_tickets_count = $this->opd_opt_model_weekly->feedback_and_ticket($table_feedback, $table_tickets, $sorttime);
		$total_ticket_count = count($ip_tickets_count);
		$key_highlights = $this->opd_opt_model_weekly->key_highlights($table_patients, $table_feedback, $sorttime, $setup, $totalFeedbackList);

		$alltickets = $this->opd_opt_model_weekly->alltickets();
		$opentickets = $this->opd_opt_model_weekly->read();
		$closedtickets = $this->opd_opt_model_weekly->read_close();
		$addressedtickets = $this->opd_opt_model_weekly->addressedtickets();

		$ticket_resolution_rate = $this->opd_opt_model_weekly->ticket_resolution_rate($table_tickets, $closed, $table_feedback, $allTickets);
		$close_rate = $this->opd_opt_model_weekly->ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action, $allTickets);
		...
		$user = $this->get_user_by_sms_activity('OP-WEEKLY-FEEDBACKS-REPORT-WHATSAPP');
	...
	public function sendsms_whatsapp($mobile,$fdate,$tdate, $v1, $v2, $v3, $v4, $v5, $v6, $v7, $v8, $v9, $v10, $v11, $v12, $v13, $v14, $v15)
	{
		...
		$templateParamsArray = [
			...
			$val12, // addressd
			$val13,  //closed
			$val14,  //	Ticket Resolution Rate
			$val15,  //Average Resolution Time
		...
		$insert_notification_query = "INSERT INTO notifications_whatsapp 
		  ...
		  VALUES (
			  ...
			  'weeklyoverallsummary_outpatient_feedbackreports', 
