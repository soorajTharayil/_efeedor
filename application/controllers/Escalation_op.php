...
class Escalation_op extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['title'] = 'Outpatient Feedback - Escalation User Assignment';
		#-------------------------------#
		$data['escalation'] = $this->escalation_model->read('OP');

		$data['content'] = $this->load->view('escalation_op/index', $data, true);
	...
	public function action()
	{
		if ($this->uri->segment(3) == 'active') {
			$this->escalation_model->change_status('OP', 'ACTIVE');
		} elseif ($this->uri->segment(3) == 'inactive') {
			$this->escalation_model->change_status('OP', 'INACTIVE');
		}
