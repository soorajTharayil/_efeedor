...
class Asset extends CI_Controller
{
    ...
    public function download_alltickets()
    {
        if (ismodule_active('ASSET') === true) {
            ...
            //Get valuation date
            if (!empty($_GET['valuation_date'])) {
                $currentDate = new DateTime($_GET['valuation_date']);
            } else {
                $currentDate = new DateTime();
            }


            $valuationDateStr = $currentDate->format('d-m-y');

            // Fetch data from bf_feedback_asset_creation
            $this->db->select('fc.*, da.method');
            $this->db->from('bf_feedback_asset_creation as fc');
            $this->db->join('bf_departmentasset as da', 'TRIM(fc.ward) = TRIM(da.title)', 'left');
            //$this->db->where_not_in('fc.assignstatus', ['Asset Lost', 'Asset Dispose', 'Asset Sold']);

            $this->db->where('fc.installDate <=', $currentDate->format('Y-m-d'));

            $query = $this->db->get();
            $departments = $query->result();

            // Calculate FY labels dynamically:
            $currentFYStart = ((int)$currentDate->format('m') <= 3)
                ? ((int)$currentDate->format('Y') - 1)
                : (int)$currentDate->format('Y');

            $fyLabelPrev1 = 'FY ' . ($currentFYStart - 1) . '-' . substr($currentFYStart, -2);
            $fyLabelPrev2 = 'FY ' . ($currentFYStart - 2) . '-' . substr($currentFYStart - 1, -2);
            ...
            $header = [
                ...
                'CURRENT ASSET VALUE',
                'VALUATION DATE',
                "ASSET VALUE- {$fyLabelPrev1}",
                "ASSET VALUE- {$fyLabelPrev2}",
                'CONTRACT TYPE',
                'CONTRACT START DATE',
                'CONTRACT END DATE',
            ...
            if (!empty($departments)) {
                ...
                foreach ($departments as $department) {
                    ...
                    if (!empty($department->assignstatus) && in_array($department->assignstatus, ['Asset Lost', 'Asset Dispose', 'Asset Sold'])) {
                        if (!empty($department->replymessage) && is_object($department->replymessage)) {
                            $status = $department->replymessage->ticket_status ?? '';
                            $soldDateStr = $department->replymessage->sold_start_date_time ?? '';
                            $createdOnStr = $department->replymessage->created_on ?? '';

                            if ($status === 'Asset Sold' && !empty($soldDateStr)) {
                                $statusChangeDate = new DateTime(substr($soldDateStr, 0, 10));
                                if ($statusChangeDate <= $currentDate) {
                                    $hideAsset = true;
                                }
                            } elseif (in_array($status, ['Asset Lost', 'Asset Dispose']) && !empty($createdOnStr)) {
                                $statusChangeDate = new DateTime(substr($createdOnStr, 0, 10));
                                if ($statusChangeDate <= $currentDate) {
                                    $hideAsset = true;
                                }
                            } else {
                    ...
                    if (!empty($department->installDate) && !empty($department->unitprice) && !empty($department->method)) {

                        $method = trim($department->method);
                        $rate = floatval($department->depreciation);

                        if ($method) {
                            $fyAssetValues = $this->calculateDepreciationValues(
                                $department->installDate,
                                $department->unitprice,
                                $rate,
                                $method,
                                $currentDate
                            );
                            $currentValue = end($fyAssetValues);
                        }
                    } else {
                        $rate = floatval($department->depreciation);
                    }

                    $fyValuePrev1 = $fyAssetValues[$fyLabelPrev1] ?? 'N/A';
                    $fyValuePrev2 = $fyAssetValues[$fyLabelPrev2] ?? 'N/A';


                    $dataexport[$i] = [
                        'row1' => $sl,
                        'row2' => $department->patientid,
                        'row3' => $department->assetname,
                        'row4' => $department->ward,
                        'row5' => $department->brand,
                        'row6' => $department->model,
                        'row7' => $department->serial,
                        'row8' => $department->locationsite,
                        'row9' => $department->bedno,
                        'row10' => $department->locationsite,
                        'row11' => $department->bedno,
                        'row12' => $department->depart,
                        'row13' => $department->assignee,
                        'row14' => date('d-m-y', strtotime($department->purchaseDate)),
                        'row15' => $department->porder,
                        'row16' => $department->invoice,
                        'row17' => $department->grn_no,
                        'row18' => date('d-m-y', strtotime($department->installDate)),
                        'row19' => date('d-m-y', strtotime($department->warrenty)),
                        'row20' => date('d-m-y', strtotime($department->warrenty_end)),
                        'row21' => $department->assetquantity,
                        'row22' => $department->unitprice,
                        'row23' => $department->totalprice,
                        'row24' => $rate,
                        'row25' => $currentValue,
                        'row26' => $valuationDateStr,
                        'row27' => $fyValuePrev1,
                        'row28' => $fyValuePrev2,
                        'row29' => !empty($department->contract) ? $department->contract : 'N/A',
                        'row30' => !empty($department->contract_start_date) ? date('d-m-y', strtotime($department->contract_start_date)) : 'N/A',
                        'row31' => !empty($department->contract_end_date) ? date('d-m-y', strtotime($department->contract_end_date)) : 'N/A',
                        'row32' => !empty($department->contract_service_charges) ? $department->contract_service_charges : 'N/A',
                        'row33' => (!empty($department->supplierinfo) ? "Supplier Name: " . $department->supplierinfo : 'N/A') . ", " .
            ...
            $fileName = 'EF-ALL-ASSET MASTER REPORT-' . $valuationDateStr . '.csv';
    ...
    private function calculateDepreciationValues($installDateStr, $unitPrice, $depreciationRate, $method, $currentDate)
    {
        $installDate = new DateTime($installDateStr);
        $baseValue = floatval($unitPrice);
        $rate = floatval($depreciationRate);
        $fyAssetValues = [];
        ...
        // Start from FY two years before current FY
        $startFY = $currentFYStart - 2;
        $fyStart = new DateTime($startFY . '-04-01');

        while ($fyStart <= $currentDate) {
            $fyYear = (int)$fyStart->format('Y');
            $fyLabel = 'FY ' . $fyYear . '-' . substr($fyYear + 1, -2);

            // Calculate period within FY
            $periodStart = ($installDate > $fyStart) ? clone $installDate : clone $fyStart;
            $periodEnd = new DateTime(($fyYear + 1) . '-03-31');
            if ($periodEnd > $currentDate) {
                $periodEnd = clone $currentDate;
            }

            if ($periodStart > $periodEnd) {
                $fyAssetValues[$fyLabel] = number_format($baseValue, 2, '.', '');
                $fyStart->modify('+1 year');
                continue;
            }

            $daysDiff = $periodEnd->diff($periodStart)->days + 1;
            $yearFraction = $daysDiff / 365;

            if ($method == 'SLM') {
                $depreciation = $unitPrice * ($rate / 100) * $yearFraction;
                $baseValue = max(0, $unitPrice - $depreciation);
            } elseif ($method == 'WDV') {
                $effectiveRate = pow(1 - ($rate / 100), $yearFraction);
                $baseValue = max(0, $baseValue * $effectiveRate);
            }
