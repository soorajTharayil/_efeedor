...
class Messageincident extends CI_Controller
{
	...
	public function inweeklyreport()
	{
		...
		$ip_feedbacks_count = $this->incident_opt_model_weekly->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
		$totalFeedbackList = $ip_feedbacks_count;
		$allTickets = $this->incident_opt_model_weekly->get_tickets($table_feedback, $table_tickets);
		$ip_psat = $this->incident_opt_model_weekly->psat_analytics($table_patients, $table_feedback, $table_tickets, $sorttime, $allTickets, $totalFeedbackList);
		$ip_psat_tool = 'Satisfied Patients: ' . ($ip_psat['satisfied_count']) . ', ' . "Unsatisfied Patients: " . ($ip_psat['unsatisfied_count']) . '. ';

		// IP ANALYTICS
		$ip_nps = $this->incident_opt_model_weekly->nps_analytics($table_feedback, $asc, $setup);
		$ip_nps_tool = 'Promoters: ' . ($ip_nps['promoters_count']) . ', ' . "Detractors: " . ($ip_nps['detractors_count']) . ', ' . "Passives: " . ($ip_nps['passives_count']);
		$ip_tickets_count = $this->incident_opt_model_weekly->feedback_and_ticket($table_feedback, $table_tickets, $sorttime);
		$total_ticket_count = count($ip_tickets_count);
		$key_highlights = $this->incident_opt_model_weekly->key_highlights($table_patients, $table_feedback, $sorttime, $setup, $totalFeedbackList);

		$alltickets = $this->incident_opt_model_weekly->alltickets();
		$opentickets = $this->incident_opt_model_weekly->read();
		$closedtickets = $this->incident_opt_model_weekly->read_close();
		$addressedtickets = $this->incident_opt_model_weekly->addressedtickets();
		$ticket = $this->incident_model->tickets_recived_by_department_interim($type, $table_feedback, $table_tickets);

		$ticket_resolution_rate = $this->incident_opt_model_weekly->ticket_resolution_rate($table_tickets, $closed, $table_feedback, $allTickets);
		$close_rate = $this->incident_opt_model_weekly->ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action, $allTickets);
...
if ($minDepartment && $maxDepartment != null) {
	...
	$highest_complain = '-';
		...
		$user = $this->get_user_by_sms_activity('INC-WEEKLY-FEEDBACKS-REPORT-WHATSAPP');
	...
	public function sendsms_whatsapp($mobile,$fdate,$tdate, $v1, $v2, $v3, $v4, $v5, $v6, $v7)
	{
		...
		$insert_notification_query = "INSERT INTO notifications_whatsapp 
		  ...
		  VALUES (
			  ...
			  'weeklyoverallsummary_incident_reports', 
