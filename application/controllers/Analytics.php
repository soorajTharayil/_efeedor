...
class Analytics extends CI_Controller
{
    ...
    public function patient_satisfaction_analysis()
    {
        ...
        //print_r($dates); exit;
        $all_tickets = $this->ipd_model->get_tickets($this->table_feedback, $this->table_tickets, $this->sorttime);
        //print_r($all_tickets);
        $feedback_data = $this->ipd_model->patient_and_feedback($this->table_patients, $this->table_feedback, $this->sorttime, $this->setup);
        $days = $_SESSION['days'];
        $set = array();
...
//     }
    ...
    public function resposnsechart()
    {
        ...
        $y     = date('Y');
        $fdate = date('Y-m-d', strtotime($_SESSION['from_date'] ?? 'now'));
        $tdate = date('Y-m-d', strtotime($_SESSION['to_date'] ?? 'now'));
        ...
        foreach ($feedback_data as $row) {
            ...
            if (!isset($report[$mon])) {
                $report[$mon] = ['count' => 0, 'overall' => 0];
            }
    
            // FIX: just count the row
            $report[$mon]['count']++;
        }
    
        $overall = count($feedback_data);
        foreach ($report as $k => $v) {
            $report[$k]['overall'] = $overall;
        }
        ...
        foreach ($report as $key => $row) {
            $response[] = [
                'label_field' => $key,
                'data_field'  => $overall ? round(($row['count'] / $overall) * 100) : 0,
                'all_detail'  => $row,
            ];
        }
    ...
    {
        //$dates = get_from_to_date();
        $department = $this->ipd_model->get_department($this->type, false);
        $get_tickes = $this->ipd_model->get_tickets($this->table_feedback, $this->table_tickets);
        //  print_r($get_tickes); 
        $report = array();
        $response = array();
    ...
    public function trend_analysis()
    {
        ...
        $sresult = $this->ipd_model->setup_result($this->setup);
        $ip_feedbacks_count = $this->ipd_model->patient_and_feedback($this->table_patients, $this->table_feedback, $this->sorttime, $this->setup);
	...
	/*public function tickets_recived_by_department_set(){
		...
		print_r($parameterTicketCount); exit;
		exit;
	}*/
