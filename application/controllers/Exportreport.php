...
class Exportreport extends CI_Controller {
	...
	public function expormainip(){
		...
		ob_end_clean();
		$fileName = 'IP- PATIENTS WISE FEEDBACK REPORT - '.date('d-m-Y',strtotime($_GET['tdate'])).' to '.date('d-m-Y',strtotime($_GET['fdate'])).'.csv';
	...
	public function explortopcomment(){
		...
		//print_r($comments);
		ob_end_clean();
		$fileName = 'IP- PATIENTS COMMENTS - '.date('d-m-Y',strtotime($_GET['tdate'])).' to '.date('d-m-Y',strtotime($_GET['fdate'])).'.csv';
		header('Pragma: public');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Cache-Control: private', false);
	...
	public function exportmainipdepartment(){
		...
		ob_end_clean();
		$fileName = 'IP- DEPARTMENT WISE REPORT - '.date('d-m-Y',strtotime($_GET['tdate'])).' to '.date('d-m-Y',strtotime($_GET['fdate'])).'.csv';
		header('Pragma: public');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Cache-Control: private', false);
	...
	public function expormainop(){
		...
		$dataexport[$i]['patient_id'] = (int)$for5['overallScore'];
		$dataexport[$i]['mobile'] = round(($for5['overallScore']/$d[$k])*100).'%';
		
		
		$i++;
		$dataexport[$i]['name'] = '';
		$dataexport[$i]['name'] = 'GOOD(4)';
		$dataexport[$i]['patient_id'] = (int)$for4['overallScore'];
		...
		$dataexport[$i]['name'] = 'AVERAGE(3)';
		$dataexport[$i]['patient_id'] = (int)$for3['overallScore'];
		...
		$dataexport[$i]['name'] = 'POOR(2)';
		$dataexport[$i]['patient_id'] = (int)$for2['overallScore'];
		$dataexport[$i]['mobile'] = round(($for2['overallScore']/$d[$k])*100).'%';
		
		
		$i++;
		
		$dataexport[$i]['name'] = 'WORST(1)';
		$dataexport[$i]['patient_id'] = (int)$for1['overallScore'];
	...
	public function expormainfull(){
		...
		foreach($feedbacktaken as $r){
			...
			if((int)$param->overallScore  == 1){
				$patientrating1 = $patientrating1 +1;
			}
			if((int)$param->overallScore  == 2){
				$patientrating2 = $patientrating2 +1;
			}
			if((int)$param->overallScore  == 3){
				$patientrating3 = $patientrating3 +1;
			}
			if((int)$param->overallScore  == 4){
				$patientrating4 = $patientrating4 +1;
			}
			if((int)$param->overallScore  == 5){
		...
		$dataexport[$i]['row2'] =  round((($promoter-$depromoter)/($depromoter+$promoter+$passive))*100).'%';
		$dataexport[$i]['row3'] = '';
		$dataexport[$i]['row4'] = '';
		$i++;
		...
		ob_end_clean();
		$fileName = 'IP - OVERALL PATIENTS FEEDBACKS REPORT  - '.date('d-m-Y',strtotime($_GET['tdate'])).' to '.date('d-m-Y',strtotime($_GET['fdate'])).'.csv';
		header('Pragma: public');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Cache-Control: private', false);
	...
	public function expormainint(){
		...
		$this->db->select('tickets_int.*,bf_patients.name as pname');
		$this->db->from('tickets_int');
		
		$this->db->join('bf_feedback_int','bf_feedback_int.id = tickets_int.feedbackid','left');
		$this->db->join('bf_patients','bf_patients.patient_id = bf_feedback_int.patientid','left');
		if(isset($_SESSION['ward']) && $_SESSION['ward'] != 'ALL'){
			$this->db->where('bf_feedback_int.ward',$_SESSION['ward']);
		}
		...
		ob_end_clean();
		$fileName = 'PATIENT COMPLAINT - DEPARTMENT WISE COMPLAINT REPORT  - '.date('d-m-Y',strtotime($_GET['tdate'])).' to '.date('d-m-Y',strtotime($_GET['fdate'])).'.csv';
		header('Pragma: public');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Cache-Control: private', false);
	...
	public function expormainpermoter(){
		...
		ob_end_clean();
		$fileName = 'IP-Promoters List Report - '.date('d-m-Y',strtotime($_GET['tdate'])).' to '.date('d-m-Y',strtotime($_GET['fdate'])).'.csv';
		header('Pragma: public');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Cache-Control: private', false);
	...
	public function expormaindemoter(){
		...
		ob_end_clean();
		$fileName = 'IP-Demoters List REPORT - '.date('d-m-Y',strtotime($_GET['tdate'])).' to '.date('d-m-Y',strtotime($_GET['fdate'])).'.csv';
		header('Pragma: public');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Cache-Control: private', false);
	...
	public function expormainbysection(){
		...
		ob_end_clean();
		$fileName = 'IP- Hospital Chose for Various Reasons - '.date('d-m-Y',strtotime($_GET['tdate'])).' to '.date('d-m-Y',strtotime($_GET['fdate'])).'.csv';
		header('Pragma: public');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Cache-Control: private', false);
	...
	public function expormainfullop(){
		...
		foreach($feedbacktaken as $r){
			...
			$param = json_decode($r->dataset);
			if((int)$param->recommend1Score > 0){
				if ($param->recommend1Score > 4){ 
					$promoter = $promoter + 1;
				}else{
					$depromoter = $depromoter + 1;
