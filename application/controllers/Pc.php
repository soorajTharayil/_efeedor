 ...
 public function escalation_report()
    {
        ...
        if (ismodule_active('PCF') === true) {
            ...
            $data['title'] = 'PC- ESCALATION REPORT-PATIENT COMPLAINT TICKETS';
            $dates = get_from_to_date();
            $data['departments'] = $this->ticketsint_model->alltickets();

            $data['content'] = $this->load->view('complaintsmodules/escalation_report', $data, true);
    ...
    public function dep_tat_edit()
    {
        ...
        if (ismodule_active('PCF') === true) {
            ...
            $data['title'] = 'Patient Complaints - Manage Escalation Turn Around Time';
    ...
    public function download_capa_report_pdf()
    {
        if (ismodule_active('PCF') === true) {
                ...
                foreach ($departments as $department) {
                    if ($department->status == 'Closed') {
                        ...
                        $dataexport[] = array(
                            ...
                            'TICKET DETAILS' => $department->department->description . '<br>' .
                            ...
                            'RCA/CAPA' => 'RCA: ' . (!empty($root) ? implode(", ", $root) : 'NA') . '<br>' .
                                'CAPA: ' . (!empty($corrective) ? implode(", ", $corrective) : 'NA') . '<br>' .
            ...
            $pdf->SetTitle('INPATIENTS COMPLAINTS - CAPA REPORT - ' . $tdate . ' to ' . $fdate);
            $pdf->SetSubject('INPATIENTS COMPLAINTS - CAPA REPORT - ' . $tdate . ' to ' . $fdate);
            ...
            $html .= '<h2 style="text-align:center;">INPATIENTS COMPLAINTS- CAPA REPORT </h2>';
            ...
            foreach ($dataexport as $data) {
                ...
                // RCA/CAPA
                $html .= '<tr>
                <td style="font-weight: bold;">RCA/ CAPA/ Resolution Comment</td>
                <td>' . $data['RCA/CAPA'] . '</td>
              </tr>';
            ...
            $fileName = 'EF- PCF CAPA REPORT - ' . $tdate . ' to ' . $fdate . '.pdf';
