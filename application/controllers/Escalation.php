...
class Escalation extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = 'IP Discharge Feedback - Escalation User Assignment';
		#-------------------------------#
		
		$data['escalation'] = $this->escalation_model->read('IP');
		
		$data['content'] = $this->load->view('escalation/index',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	
	public function action(){
		if($this->uri->segment(3) == 'active'){
			$this->escalation_model->change_status('IP','ACTIVE');
		}elseif($this->uri->segment(3) == 'inactive'){
			$this->escalation_model->change_status('IP','INACTIVE');
		}
		redirect('escalation');
	}
