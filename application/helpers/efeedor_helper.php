...
function show_filter($segment_1, $segment_2)
{
	...
	if ($segment_1 == 'auditdepartment') {
		$show = false;
	}

	if ($segment_1 == 'audit_patient_category') {
		$show = false;
	}
	...
	if ($segment_1 == 'audit_safety_adherence_dept') {
		$show = false;
	}

	if ($segment_1 == 'audit_np_ratio_icu') {
		$show = false;
	}

	if ($segment_1 == 'audit_np_ratio_ward') {
		$show = false;
	}

	if ($segment_1 == 'audit_hand_designation') {
		$show = false;
	}

	if ($segment_1 == 'audit_hand_indication') {
		$show = false;
	}

	if ($segment_1 == 'audit_hand_action') {
		$show = false;
	}

	if ($segment_1 == 'audit_hand_compliance') {
		$show = false;
	}
	...
	if ($segment_1 == 'audit_transfusion_type') {
		$show = false;
	}

	if ($segment_1 == 'audit_patient_status') {
		$show = false;
	}

	if ($segment_1 == 'audit_emergency_code') {
		$show = false;
	}

	if ($segment_1 == 'qualitybenchmark') {
		$show = false;
	}
...
{
	if ($show == 'open_ticket') {
		return true; //to show tat in open ticket page
	} elseif ($show == 'tat_exists') {
		return true; //if tat exists
	} elseif ($show == 'close_ticket') {
		return true; //to show tat in close ticket page
	} elseif ($show == 'admin_link') {
		return true; //to show tat status in admin tracking link
	} elseif ($show == 'department_link') {
		return true; //to show tat status in department head tracking link
	} else {
		return false;
	}
}
...
function int_tat($show)
{
	if ($show == 'open_ticket') {
		...
		return true; //to show tat in department head tracking link
	} elseif ($show == 'patient_link') {
		return true; //to show tat in patient tracking link
	} elseif ($show == 'department_rating') {
		return true; //to show star in patient link after the tickect is closed
	} elseif ($show == 'department_information') {
		return true; //to show department head details in patient tracking link
	} else {
		return false;
	}
}
