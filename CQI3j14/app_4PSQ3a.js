...
$scope.user_id = ehandor.userid;
    ...
    // Validate: Empty or negative values not allowed
    if (isNaN(medicationErrors) || medicationErrors < 0) {
        alert("Please enter a valid number of appropriate handovers (non-negative).");
        return;
    }

    if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
        alert("Please enter a valid number of opportunities for handovers (non-negative).");
        return;
    }

    // Allow both zero case
    ...
    // Format output (whole or 2 decimal)
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3j14')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3j14.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
