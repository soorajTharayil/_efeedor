...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateMedicationErrorRate = function () {
      ...
      // Case: denominator = 0 but numerator > 0 → invalid
      if (opportunitiesForErrors === 0) {
        alert(
          "Number of handover opportunities Doctors cannot be zero unless both are zero"
        );
        return;
      }

      // Allow calculation even if numerator > denominator (just compute normally)
      var errorRatePercentage =
        (medicationErrors / opportunitiesForErrors) * 100;

      // Format: whole number or two decimals
