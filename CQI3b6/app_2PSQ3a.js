...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    // Get inputs and parse as float to allow decimal inputs
    ...
    if (reportingErrors < 0) {
        alert("Please enter the number of reporting errors (cannot be negative).");
        return;
    }

    if (testsPerformed < 0) {
        alert("Please enter the number of tests performed (cannot be negative).");
        return;
    }

    

    // Calculate errors per 1000 investigations (0 if denominator is 0)
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI3b6')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI3b6.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
