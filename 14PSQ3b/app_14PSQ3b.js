...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate  function
    $scope.calculateVAPRate = function () {
      // Get the number of ventilator associated pneumonia in a month
      var ventilatorAssociatedPneumonia = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the number of ventilator days in that month
      var ventilatorDays = parseInt(
        document.getElementById("formula_para2").value
      );

      // Validate inputs for ventilatorAssociatedPneumonia and ventilatorDays
      // if (isNaN(ventilatorAssociatedPneumonia) || ventilatorAssociatedPneumonia < 0) {
      // 	alert("Enter the number of ventilator associated pneumonia in a month.");
      // 	return;
      // }

      // if (isNaN(ventilatorDays) || ventilatorDays <= 0) {
      // 	alert("Enter the number of ventilator days in that month.");
      // 	return;
      // }

      if (ventilatorAssociatedPneumonia > ventilatorDays) {
        alert(
          "Number of ventilator associated pneumonia must be less than the number of ventilator days."
        );
        return;
      }

      if (ventilatorAssociatedPneumonia === 0 && ventilatorDays === 0) {
        $scope.calculatedResult = 0;
      } else {
        // Calculate the VAP rate
        $scope.calculatedResult = Math.round(
          (ventilatorAssociatedPneumonia / ventilatorDays) * 1000
        );
      }
      ...
      console.log("Calculated VAP rate", $scope.calculatedResult);
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi14.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
