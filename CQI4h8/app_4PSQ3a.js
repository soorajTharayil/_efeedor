...
$scope.calculateMedicationErrorRate = function () {
	// Get the number of planned discharges (numerator)
	var medicationErrors = parseFloat(document.getElementById('formula_para1').value);

	// Get the total number of discharges (denominator)
	...
	if (isNaN(medicationErrors) || medicationErrors < 0) {
		alert("Please enter total number of planned discharges");
		return;
	}

	if (isNaN(opportunitiesForErrors) || opportunitiesForErrors < 0) {
		alert("Please enter total number of discharges");
		return;
	}

	// ✅ Allow zero and both zero
	...
	if (opportunitiesForErrors === 0 && medicationErrors === 0) {
		...
		// Denominator zero → take numerator * 100
...
// Menu bar start
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_CQI4h8'
		)
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/save_CQI4h8.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
