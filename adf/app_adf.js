...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/adf.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
	...
	$scope.savefeedback = function () {
		...
		$http.post($rootScope.baseurl_main + '/savepatientfeedback_adf.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId, $scope.feedback).then(function (responsedata) {
