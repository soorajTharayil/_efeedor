...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window) {
    ...
    //Calculate Incidence of hospital associated pressure ulcers  function
    $scope.calculatePressureUlcersKPI = function () {
      // Get the number of patients who develop new or worsening pressure ulcers
      var pressureUlcers = parseInt(
        document.getElementById("formula_para1").value
      );

      // Get the total number of patient days
      var totalPatientDays = parseInt(
      ...
      if (isNaN(pressureUlcers) || pressureUlcers < 0) {
        alert(
          "Enter the number of patients who develop new or worsening pressure ulcers."
        );
        return;
      }
      ...
      if (isNaN(totalPatientDays) || totalPatientDays <= 0) {
        alert(
          "Enter the total number of patient days (must be greater than zero)."
        );
        return;
      }
      ...
      if (pressureUlcers > totalPatientDays) {
        alert(
          "Number of pressure ulcers must be less than the total number of patient days."
        );
        return;
      }
      ...
      if (pressureUlcers === 0) {
        ...
        pressureUlcersKPI = Math.round(
          (pressureUlcers / totalPatientDays) * 1000
        );
      }

      // Assign the calculated result to $scope.calculatedResult
      $scope.calculatedResult = pressureUlcersKPI;
          ...
          function (response) {
            if (response.data.status === "exists") {
              ...
              $http
                .post(
                  $rootScope.baseurl_main +
                    "/savepatientfeedback_kpi12.php?patient_id=" +
                    $rootScope.patientid +
                    "&administratorId=" +
                    $rootScope.adminId +
                    "&month=" +
