...
app.controller("PatientFeedbackCtrl", function ($rootScope, $scope, $http, $timeout) {
  $scope.typel = "english";
  $scope.type2 = "English";
  $scope.feedback = {};
  ...
  $scope.next0 = function () {
    ...
    grecaptcha.ready(function () {
      grecaptcha.execute('6Lc8CkcqAAAAACv2WebgYioIVJhljcDhqJk5AbAz', { action: 'submit_mobile_number' })
        .then(function (token) {
          // HTTP Request: Fetch data based on the contact number and reCAPTCHA token
          $http.get($rootScope.baseurl_main + '/mobile_interim_captcha.php?mobile=' + $scope.feedback.contactnumber + '&recaptcha_token=' + token, { timeout: 20000 })
            .then(function (responsedata) {
              // Log the reCAPTCHA score for debugging
              console.log("reCAPTCHA score:", responsedata.data.recaptcha_score);
              ...
              if (responsedata.data.pinfo == "NO") {
                ...
                $scope.feedback.name = "";
                $scope.feedback.section = "INTERIM";
  ...
  $scope.setupapplication = function () {
    ...
    $http
      .get($rootScope.baseurl_main + "/ward2.php?patientid=" + id, {
        timeout: 20000,
      })
      .then(
  ...
  $scope.setupapplication1 = function () {
    ...
    $scope.feedback.ward = locationValue;

    var url = window.location.href;
    var id = url.substring(url.lastIndexOf('=') + 1);

    $http.get($rootScope.baseurl_main + '/ward2.php?patientid=' + id, { timeout: 20000 })
      .then(function (responsedata) {
        $scope.wardlist = responsedata.data;
        // ✅ Load setup into global scope if exists
        if (responsedata.data && responsedata.data.setup) {
        ...
        if (locationValue && $scope.wardlist.ward) {
          ...
          if (matchingWard) {
            ...
            // always assign step2 once ward is selected
            $scope.step0 = false;
            $scope.step1 = false;
            $scope.step2 = true;

            // If site (bedno) exists, try to assign it, otherwise ignore
            if (siteValue && Array.isArray($scope.bed_no)) {
              $timeout(function () {
                var matchingEntry = Object.entries($scope.bed_no).find(function ([key, value]) {
                  return value.trim().toLowerCase() === siteValue.trim().toLowerCase();
                });

                if (matchingEntry) {
                  var matchedValue = matchingEntry[1].trim(); // the actual bed name
                  $scope.feedback.bedno = matchedValue;
                  console.log("Auto-selected Bed Value:", matchedValue);
                  $scope.isWardPreselected_site = true;
                } else {
                  console.warn("No matching bed found for site:", siteValue);
                }
              }, 300);
            }
        ...
        $rootScope.loader = false;
        console.error("Error fetching ward data:", response);
      });
  };
  ...
  $scope.savefeedback = function () {
    ...
    // Set default valaue
    if (!$scope.feedback.bedno || $scope.feedback.bedno.trim() === '' || $scope.feedback.bedno.trim() === 'Room/Bed number') {
      alert("Please select bed number")
      return;
    }
    if (!$scope.feedback.name || $scope.feedback.name.trim() === '') {
      $scope.feedback.name = "Not Available";
    }
    if (!$scope.feedback.patientid || $scope.feedback.patientid.trim() === '') {
      $scope.feedback.patientid = "Not Available";
    }
    if (!$scope.feedback.contactnumber || $scope.feedback.contactnumber.trim() === '') {
      $scope.feedback.contactnumber = "Not Available";
    }


    $rootScope.loader = true;

    $scope.feedback.patientType = "INTERIM";
    ...
    $http
      .post(
        ...
        "/savepatientfeedback_int.php?patient_id=" +
        $scope.feedback.patientid +
        "&administratorId=" +
        $rootScope.adminId,
        $scope.feedback
